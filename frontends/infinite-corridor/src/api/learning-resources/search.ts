import type { RequiredBy } from "ol-util"
import { intersection } from "lodash"
import { QueryClient, useInfiniteQuery } from "react-query"
import type { LearningResourceSearchResult } from "ol-search-ui"
import type { Aggregations, SearchQueryParams, Facets } from "@mitodl/course-search-utils"
...
const doSearch = async (params: SearchQueryParams): Promise<TransformedSearchResponse> => {
...
const modifyCachedSearchResource = (queryClient: QueryClient, patch: RequiredBy<Partial<LearningResourceSearchResult>, "id">) => {
  ...
  searchQueries.forEach(query => {
    ...
    const newPages = data.pages.map(page => {
      ...
      const index = results.findIndex(result => result.id === patch.id)
      if (index === -1) return page
      match = true
      const newResult = { ...results[index], ...patch }
      const newResults = results.map((res, i) => i === index ? newResult : res)
      return {
        ...page,
        hits: {
          ...page.hits,
