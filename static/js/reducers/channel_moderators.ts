...
import { ChannelModerators, Moderator } from "../flow/discussionTypes";
...
const addModerator = ({
  channelName,
  moderator
}: AddChannelModerator, data: Map<string, ChannelModerators>): Map<string, ChannelModerators> => {
  const update = new Map(data);
  let moderators = update.get(channelName) || [];
  const existingModerator = R.find(R.eqProps("moderator_name", moderator), moderators);
...
const deleteModerator = ({
  channelName,
  username
}: DeleteChannelModerator, data: Map<string, ChannelModerators>): Map<string, ChannelModerators> => {
  const update = new Map(data);
  const moderators = update.get(channelName) || [];
  update.set(channelName, moderators.filter(moderator => moderator.moderator_name !== username));
  return update;
};
...
export const channelModeratorsEndpoint = {
  ...
  getSuccessHandler: ({
    channelName,
    response
  }: ChannelModeratorsEndpointResponse, data: Map<string, ChannelModerators>): Map<string, ChannelModerators> => {
    const update = new Map(data);
    update.set(channelName, response);
    return update;
  },
  postFunc: async (channelName: string, email: string) => {
    const moderator = await moderationAPI.addChannelModerator(channelName, email);
