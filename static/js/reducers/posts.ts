...
import { GET, POST, PATCH, DELETE, INITIAL_STATE } from "redux-hammock/constants";
import R from "ramda";

import { SET_POST_DATA, CLEAR_POST_ERROR } from "../actions/post";

import { CreatePostPayload, Post } from "../flow/discussionTypes";
...
export const postsEndpoint = {
  ...
  postFunc: (name: string, payload: CreatePostPayload) => postAPI.createPost(name, payload),
  ...
  extraActions: {
    [SET_POST_DATA]: (state: Object, action: Action<string, any>) => {
      const update = action.payload instanceof Array ? mergeMultiplePosts(action.payload, state.data) : mergePostData(action.payload, state.data);
