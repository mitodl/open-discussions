...
import R from "ramda";
import { GET, PATCH, POST, DELETE, INITIAL_STATE } from "redux-hammock/constants";
...
import { GenericComment, CommentFromAPI, CommentInTree, MoreCommentsFromAPI, MoreCommentsInTree, ReplaceMoreCommentsPayload } from "../flow/discussionTypes";
 ...
 */
const removeMoreComments = (oldTree: Array<GenericComment>, parentId: string) => {
  ...
  if (parentId) {
    ...
    const newReplies = comment.replies.filter(comment => comment.comment_type === "comment");
...
const deleteComment = (oldTree: Array<GenericComment>, commentId: string) => R.over(findComment(oldTree, commentId), R.evolve({
  author_name: deleted,
  text: deleted,
  author_id: deleted,
  deleted: true
 ...
 */
const replaceMoreComments = (oldTree: Array<GenericComment>, parentId: string, comments: Array<CommentFromAPI | MoreCommentsFromAPI>) => {
  ...
  for (const comment of comments) {
    filteredTree = appendCommentToTree(filteredTree, comment, comment.parent_id, true);
  }

  return filteredTree;
};

const addReply = (replies: Array<GenericComment>, comment: GenericComment, appendToEnd: boolean): Array<GenericComment> => appendToEnd ? [...replies, comment] : [comment, ...replies];

/**
 * Append a comment to the tree in the right place
 */
const appendCommentToTree = (tree: Array<GenericComment>, comment: CommentFromAPI | MoreCommentsFromAPI, parentId: string | null | undefined, appendToEnd: boolean): Array<GenericComment> => {
 ...
 */
const updateCommentTree = (tree: Array<GenericComment>, updatedComment: CommentFromAPI): Array<GenericComment> => {
  ...
  updatedComment = ({ ...updatedComment, replies } as CommentInTree);
 ...
 */
export const createCommentTree = (responseData: Array<CommentFromAPI | MoreCommentsFromAPI>): Array<GenericComment> => {
...
export const commentsEndpoint = {
  ...
  getFunc: async (postId: string, commentId?: string, params?: Object): Promise<GetCommentsPayload> => {
    const comments = commentId ? await commentsAPI.getComment(commentId) : await commentsAPI.getComments(postId, params || {});
  ...
  deleteFunc: async (postId: string, commentId: string): Promise<any> => {
  ...
  deleteSuccessHandler: ({
    commentId,
    postId
  }: DeleteCommentPayload, data: CommentData): CommentData => {
  ...
  getSuccessHandler: (response: GetCommentsPayload, data: CommentData): CommentData => {
    const update = new Map(data);
    update.set(response.postId, createCommentTree(response.comments));
    return update;
  },
  postFunc: async (postId: string, text: string, commentId: string | null | undefined): Promise<CreateCommentPayload> => {
  ...
  postSuccessHandler: ({
    ...
    postId,
    comment
  }: CreateCommentPayload, data: CommentData): CommentData => {
    const update = new Map(data);
    const oldTree = data.get(postId);
    if (oldTree) {
      update.set(postId, appendCommentToTree(oldTree, comment, commentId, false));
    }
    return update;
  },
  patchFunc: (commentId: string, payload: Object) => commentsAPI.updateComment(commentId, payload),
  patchSuccessHandler: (response: CommentFromAPI, data: CommentData): CommentData => {
    const update = new Map(data);
  ...
  extraActions: {
    [REPLACE_MORE_COMMENTS]: (state: Object, action: Action<ReplaceMoreCommentsPayload, any>) => {
...
export const moreCommentsEndpoint = {
  name: "morecomments",
  verbs: [GET],
  getFunc: (postId: string, parentId: string | null, children: Array<string>) => commentsAPI.getMoreComments(postId, parentId, children),
  initialState: { ...INITIAL_STATE },
  getSuccessHandler: (response: Array<CommentFromAPI | MoreCommentsFromAPI>, data: Map<string, Array<GenericComment>>) => {
    // no changes to state.
