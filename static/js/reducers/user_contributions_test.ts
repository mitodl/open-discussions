...
describe("userContributions reducer", () => {
  ...
  });[[POSTS_OBJECT_TYPE, postsResult, "getUserPostsStub"], [COMMENTS_OBJECT_TYPE, commentsResult, "getUserCommentsStub"]].forEach(([objType, expResultArray, expStubCalled]) => {
    it(`should be able to fetch ${objType} and correctly manage the endpoint state`, async () => {
      const {
        requestType,
        successType
      } = actions.userContributions.get;
      const {
        data
      } = await dispatchThen(actions.userContributions.get(objType, username, dummyPagination), [requestType, successType]);
  ...
  it("should pass an empty object to the API method by default for pagination params", async () => {
    ...
    await dispatchThen(actions.userContributions.get(POSTS_OBJECT_TYPE, username), [requestType, successType]);
  ...
  it("should append new results to existing endpoint data", async () => {
    ...
    assert.equal(userContribution.data.length, postsResult.length + newPostsResult.length);
