...
import { makeComment, makeCommentsResponse, makeMoreComments, makeMoreCommentsResponse } from "../factories/comments";
import { findComment } from "../lib/comments";

describe("comments reducers", () => {
  let helper, store, dispatchThen, listenForActions, post, response, newCommentResponse;
  ...
  it("should let you get the comments for a Post", async () => {
    const {
      requestType,
      successType
    } = actions.comments.get;
    const {
      data
    } = await dispatchThen(actions.comments.get(post.id), [requestType, successType]);
  ...
  it("should handle a comment whose parent we don't have", async () => {
    ...
    const {
      data
    } = await dispatchThen(actions.comments.get(post.id, response[0].id), [requestType, successType]);
    const comments = data.get(post.id);
  ...
  it("should handle an empty response ok", () => {
    ...
    return dispatchThen(actions.comments.get(post.id), [requestType, successType]).then(({
      data
    }) => {
  ...
  it("should let you reply to a post", () => {
    ...
    return listenForActions([actions.comments.get.successType, actions.comments.get.requestType, requestType, successType], () => {
      ...
      const comment = R.view(findComment(stateTree, newCommentResponse.id), stateTree);
  ...
  it("should let you reply to a comment", () => {
    ...
    return listenForActions([actions.comments.get.successType, actions.comments.get.requestType, requestType, successType], () => {
      store.dispatch(actions.comments.get(post.id));
      store.dispatch(actions.comments.post(post.id, "comment text", response[0].id));
    }).then(state => {
      assert.isTrue(state.loaded);
      const stateTree = state.data.get(post.id);
      const comment = R.view(findComment(stateTree, newCommentResponse.id), stateTree);
      ...
      assert.equal(comment.text, "comment text");
      assert.equal(state.postStatus, FETCH_SUCCESS);
      assert.ok(helper.createCommentStub.calledWith(post.id, "comment text", response[0].id));
    });
  });
  ...
  it("should let you update a comment", async () => {
    ...
    const state = await listenForActions([actions.comments.get.requestType, actions.comments.get.successType, actions.comments.patch.requestType, actions.comments.patch.successType], async () => {
      await store.dispatch(actions.comments.get(post.id));
      await store.dispatch(actions.comments.patch(comment.id, {
        upvoted: true
      }));
    });
    assert.isTrue(state.loaded);
  ...
  it("should let you update a comment without a tree in state", async () => {
    ...
    const state = await listenForActions([actions.comments.patch.requestType, actions.comments.patch.successType], async () => {
      await store.dispatch(actions.comments.patch(comment.id, {
        upvoted: true
      }));
    });
  ...
  it("should let you delete a comment", async () => {
    ...
    const state = await listenForActions([actions.comments.get.requestType, actions.comments.get.successType, actions.comments["delete"].requestType, actions.comments["delete"].successType], async () => {
      await store.dispatch(actions.comments.get(post.id));
  ...
  describe("more_comments", () => {
    ...
    it("swaps a more_comments object with some comments at the root level", async () => {
      ...
      await listenForActions([actions.comments.get.requestType, actions.comments.get.successType], () => {
        store.dispatch(actions.comments.get(post.id));
      });
      ...
      const newComments = [firstNewComment, firstNewCommentReply, secondNewComment, newMoreComments];
      store.dispatch(replaceMoreComments({
        postId: post.id,
        parentId: null,
    ...
    it("does not update the state when fetching more comments, except to mark it as loaded", async () => {
      ...
      let resp;
      await listenForActions([actions.morecomments.get.requestType, actions.morecomments.get.successType], async () => {
