...
describe("forms reducer", () => {
  ...
  it("should update the form state", () => {
    const prom = dispatchThen(actions.forms.formBeginEdit(payload), [actions.forms.FORM_BEGIN_EDIT, actions.forms.FORM_UPDATE]).then(forms => {
      assert.deepEqual(forms, {
        [formKey]: {
          value: { prop: "prop2" },
          errors: {}
  ...
  it("should clear the form state", () => {
    const prom = dispatchThen(actions.forms.formBeginEdit(payload), [actions.forms.FORM_BEGIN_EDIT, actions.forms.FORM_BEGIN_EDIT, actions.forms.FORM_END_EDIT]).then(forms => {
      assert.deepEqual(forms, {
        otherkey: {
          value: value,
          errors: {}
