...
import { Post, UserFeedComment, PostListPaginationParams, UserContributionResponse } from "../flow/discussionTypes";
...
const getObjectKeyFromResponse = R.compose(R.head, R.intersection([POSTS_OBJECT_TYPE, COMMENTS_OBJECT_TYPE]), R.keys);
...
export const userContributionsEndpoint = {
  ...
  getFunc: async (requestType: string, username: string, params: PostListPaginationParams) => {
    const fetchFunc = requestType === POSTS_OBJECT_TYPE ? api.getUserPosts : api.getUserComments;
    const response = await fetchFunc(username, params || {});
    return { username, response };
  },
  getSuccessHandler: (payload: ChannelEndpointResponse, oldData: ContributionMap): ContributionMap => {
    const {
      username,
      response
    } = payload;
    ...
    const userState = usernameContributionMap.get(username) || R.clone(USER_INITIAL_STATE);
    ...
    userState[objectKey].data = userState[objectKey].data.concat(response[objectKey]);
