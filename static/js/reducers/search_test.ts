...
describe("search reducers", () => {
  ...
  it("should fetch search results", async () => {
    const {
      data
    } = await dispatchThen(actions.search.post({ from: 0, size: 5 }), [actions.search.post.requestType, actions.search.post.successType]);
  ...
  it("should truncate search results based on the from argument", async () => {
    ...
    const {
      data
    } = await dispatchThen(actions.search.post({ from: 2, size: 5 }), [actions.search.post.requestType, actions.search.post.successType]);
