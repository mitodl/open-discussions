...
describe("widgets reducer", () => {
  ...
  it("should let you fetch a widget list", async () => {
    ...
    await helper.dispatchThen(actions.widgets.get(response.id), [requestType, successType]);
  ...
  it("should patch a widget list", async () => {
    ...
    const {
      requestType,
      successType
    } = actions.widgets.patch;
    await helper.dispatchThen(actions.widgets.patch(response.id, payload), [requestType, successType]);
