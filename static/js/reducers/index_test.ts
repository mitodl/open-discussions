...
describe("reducers", () => {
  ...
  describe("posts reducer", () => {
    ...
    it("should allow for multiple posts to coexist", () => {
      return Promise.all([store.dispatch(actions.posts.get("first")), store.dispatch(actions.posts.get("second"))]).then(() => {
        const {
          posts: {
            data
          }
  ...
  describe("channels reducer", () => {
    ...
    it("should support multiple channels", () => {
      return Promise.all([store.dispatch(actions.channels.get("first")), store.dispatch(actions.channels.get("second"))]).then(() => {
        const {
          channels: {
            data
          }
    ...
    it("should let you create a channel", async () => {
      ...
      const channel = makeChannel();
      const channels = await dispatchThen(actions.channels.post(channel), [requestType, successType]);
    ...
    it("should let you update a channel", async () => {
      const {
        requestType,
        successType
      } = actions.channels.patch;
      const channel = makeChannel();
      const channels = await dispatchThen(actions.channels.patch(channel), [requestType, successType]);
      assert.deepEqual(channel, channels.data.get(channel.name));
    });
  });
  ...
  describe("frontpage reducer", () => {
    ...
    it("should let you get the frontpage", async () => {
      ...
      const {
        data
      } = await dispatchThen(actions.frontpage.get(), [requestType, successType]);
  ...
  describe("subscribed channels (getChannels) reducer", () => {
    ...
    it("should let you get subscribed channels", async () => {
      const {
        requestType,
        successType
      } = actions.subscribedChannels.get;
      const {
        data
