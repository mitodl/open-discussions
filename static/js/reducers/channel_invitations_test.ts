...
describe("channelInvitations reducers", () => {
  ...
  beforeEach(() => {
    ...
    dispatchThen = helper.store.createDispatchThen(state => state.channelInvitations);
  ...
  it("should add a new contributor to the list", async () => {
    ...
    const {
      data
    } = await dispatchThen(actions.channelInvitations.post(channelName, newInvite.email), [actions.channelInvitations.post.requestType, actions.channelInvitations.post.successType]);
