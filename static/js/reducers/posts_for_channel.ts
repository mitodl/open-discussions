...
import { PostListResponse, PostListData, PostListPaginationParams } from "../flow/discussionTypes";
...
export const postsForChannelEndpoint = {
  ...
  getSuccessHandler: (payload: ChannelEndpointResponse, data: Map<string, PostListData>): Map<string, PostListData> => {
    const {
      channelName,
      response
    } = payload;
  ...
  extraActions: {
    [EVICT_POSTS_FOR_CHANNEL]: (state: Object, action: Action<string, any>) => {
