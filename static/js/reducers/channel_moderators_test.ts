...
describe("channelModerators reducers", () => {
  ...
  beforeEach(() => {
    ...
    dispatchThen = helper.store.createDispatchThen(state => state.channelModerators);
  ...
  it("should handle a response with data", () => {
    const {
      requestType,
      successType
    } = actions.channelModerators.get;
    return dispatchThen(actions.channelModerators.get("channelname"), [requestType, successType]).then(({
      data
    }) => {
  ...
  it("should add a new moderator to the list", async () => {
    ...
    await dispatchThen(actions.channelModerators.get(channelName), [actions.channelModerators.get.requestType, actions.channelModerators.get.successType]);

    const {
      data
    } = await dispatchThen(actions.channelModerators.post(channelName, newModerator.moderator_name), [actions.channelModerators.post.requestType, actions.channelModerators.post.successType]);
  ...
  it("should filter out duplicates when adding a new moderator", async () => {
    ...
    const {
      data
    } = await dispatchThen(actions.channelModerators.post(channelName, existingModerator.moderator_name), [actions.channelModerators.post.requestType, actions.channelModerators.post.successType]);
  ...
  it("should remove a moderator from the list", async () => {
    ...
    const {
      data
    } = await dispatchThen(actions.channelModerators.delete(channelName, existingModerator.moderator_name), [actions.channelModerators.delete.requestType, actions.channelModerators.delete.successType]);
    assert.deepEqual(data.get(channelName), moderators.filter(moderator => moderator.moderator_name !== existingModerator.moderator_name));
  });
});