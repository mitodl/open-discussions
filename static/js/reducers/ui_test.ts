...
import { SET_SHOW_DRAWER_DESKTOP, SET_SHOW_DRAWER_MOBILE, SET_SHOW_DRAWER_HOVER, SET_SNACKBAR_MESSAGE, SHOW_DIALOG, HIDE_DIALOG, SET_DIALOG_DATA, SHOW_DROPDOWN, HIDE_DROPDOWN, SET_BANNER_MESSAGE, HIDE_BANNER, PUSH_LR_HISTORY, POP_LR_HISTORY, CLEAR_LR_HISTORY, setShowDrawerDesktop, setShowDrawerMobile, setShowDrawerHover, setSnackbarMessage, showDialog, hideDialog, setDialogData, showDropdown, hideDropdown, setBannerMessage, hideBanner, pushLRHistory, popLRHistory, clearLRHistory } from "../actions/ui";
...
describe("ui reducer", () => {
  ...
  it("should let you toggle desktop drawer visibility", async () => {
    let state = await dispatchThen(setShowDrawerDesktop(true), [SET_SHOW_DRAWER_DESKTOP]);
    assert.isTrue(state.showDrawerDesktop);
    state = await dispatchThen(setShowDrawerDesktop(false), [SET_SHOW_DRAWER_DESKTOP]);
    assert.isFalse(state.showDrawerDesktop);
  });

  it("should let you toggle mobile drawer visibility", async () => {
    let state = await dispatchThen(setShowDrawerMobile(true), [SET_SHOW_DRAWER_MOBILE]);
    assert.isTrue(state.showDrawerMobile);
    state = await dispatchThen(setShowDrawerMobile(false), [SET_SHOW_DRAWER_MOBILE]);
    assert.isFalse(state.showDrawerMobile);
  });

  it("should let you toggle desktop drawer hover state", async () => {
    let state = await dispatchThen(setShowDrawerHover(true), [SET_SHOW_DRAWER_HOVER]);
    assert.isTrue(state.showDrawerHover);
    state = await dispatchThen(setShowDrawerHover(false), [SET_SHOW_DRAWER_HOVER]);
  ...
  it("should set snackbar message", async () => {
    ...
    let state = await dispatchThen(setSnackbarMessage(payload), [SET_SNACKBAR_MESSAGE]);
    ...
    state = await dispatchThen(setSnackbarMessage(payload), [SET_SNACKBAR_MESSAGE]);
    ...
    it(`should let you ${show ? "show" : "hide"} a dialog`, async () => {
      ...
      let state = await dispatchThen(setDialogData({ dialogKey, data }), [SET_DIALOG_DATA]);
  ...
  it("should set data for the dialog", async () => {
    ...
    const data = [1, 2, 3];
    await dispatchThen(showDialog(dialogKey), [SHOW_DIALOG]);
    const state = await dispatchThen(setDialogData({ dialogKey, data }), [SET_DIALOG_DATA]);
    // make sure we start with dialog state so we can assert that it's cleared
    assert.deepEqual(state.dialogs.get(dialogKey), data);
  });
  ...
  it("should let you set the banner message", async () => {
    ...
    let state = await dispatchThen(setBannerMessage("some message"), [SET_BANNER_MESSAGE]);
  ...
  it("should let you push to the LR history", async () => {
    const state = await dispatchThen(pushLRHistory({ objectId: "HEY!", objectType: "best", runId: 1 }), [PUSH_LR_HISTORY]);
    assert.deepEqual(state.LRDrawerHistory, [{ objectId: "HEY!", objectType: "best", runId: 1 }]);
  });

  it("should let you pop an entry off the LR history", async () => {
    await dispatchThen(pushLRHistory({ objectId: "HEY!", objectType: "best", runId: 1 }), [PUSH_LR_HISTORY]);
    await dispatchThen(pushLRHistory({ objectId: "second one", objectType: "worst", runId: 1 }), [PUSH_LR_HISTORY]);
