...
import { ChannelContributors, Contributor } from "../flow/discussionTypes";
...
const addContributor = ({
  channelName,
  contributor
}: AddChannelContributor, data: Map<string, ChannelContributors>): Map<string, ChannelContributors> => {
  const update = new Map(data);
  let contributors = update.get(channelName) || [];
  const existingContributor = R.find(R.eqProps("contributor_name", contributor), contributors);
...
const deleteContributor = ({
  channelName,
  username
}: DeleteChannelContributor, data: Map<string, ChannelContributors>): Map<string, ChannelContributors> => {
  const update = new Map(data);
  const contributors = update.get(channelName) || [];
  update.set(channelName, contributors.filter(contributor => contributor.contributor_name !== username));
  return update;
};
...
export const channelContributorsEndpoint = {
  ...
  getSuccessHandler: ({
    channelName,
    response
  }: ChannelContributorsEndpointResponse, data: Map<string, ChannelContributors>): Map<string, ChannelContributors> => {
    const update = new Map(data);
    update.set(channelName, response);
    return update;
  },
  postFunc: async (channelName: string, email: string) => {
    const contributor = await channelAPI.addChannelContributor(channelName, email);
