...
describe("postsForChannel reducer", () => {
  ...
  it("should let you get the posts for a channel", async () => {
    const {
      requestType,
      successType
    } = actions.postsForChannel.get;
    const {
      data
    } = await dispatchThen(actions.postsForChannel.get("channel"), [requestType, successType]);
  ...
  it("should support multiple channels", async () => {
    await Promise.all([store.dispatch(actions.postsForChannel.get("first")), store.dispatch(actions.postsForChannel.get("second"))]);
    const {
      postsForChannel: {
        data
      }
  ...
  it(`uses mapPostListResponse to update existing posts`, async () => {
    ...
    sinon.assert.calledWith(mapStub, { posts: posts }, {
      pagination: null,
      postIds: []
    });
