...
export const useCommentModeration = (shouldGetReports: boolean | null | undefined, channelName: string | null | undefined) => {
  const dispatch = useDispatch();

  const approveComment = useCallback(async (comment: CommentInTree) => {
  ...
  const removeComment = useCallback(async (comment: CommentInTree) => {
  ...
  const ignoreReports = useCallback(async (comment: CommentInTree) => {
    await dispatch(actions.comments.patch(comment.id, { ignore_reports: true }));
    if (shouldGetReports) {
      await dispatch(actions.reports.get(channelName));
    }
  }, [dispatch, channelName, shouldGetReports]);
  ...
  const deleteComment = useCallback(async (comment: CommentInTree, post: Post) => {
