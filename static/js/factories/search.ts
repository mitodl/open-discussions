...
import { platforms, offeredBys, LR_TYPE_COURSE, LR_TYPE_ALL, LR_TYPE_PROGRAM, LR_TYPE_VIDEO, LR_TYPE_USERLIST, LR_TYPE_LEARNINGPATH, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE, OPEN, PROFESSIONAL, CERTIFICATE } from "../lib/constants";

import { CommentResult, LearningResourceResult, PostResult, ProfileResult } from "../flow/searchTypes";
...
export const makeCourseResult = (): LearningResourceResult => ({
  ...
  platform: casual.random_element([platforms.edX, platforms.OCW, platforms.bootcamps]),
  ...
  audience: casual.random_element([[], [OPEN], [PROFESSIONAL], [OPEN, PROFESSIONAL]]),
...
export const makeProgramResult = (): LearningResourceResult => ({
  ...
  offered_by: [casual.random_element([offeredBys.xpro, offeredBys.micromasters])],
  runs: [makeRun()],
  is_favorite: casual.coin_flip,
  lists: casual.random_element([[], [100, 200]]),
  audience: casual.random_element([[], [OPEN], [PROFESSIONAL], [OPEN, PROFESSIONAL]]),
...
export const makeUserListResult = (listType: string): LearningResourceResult => ({
  id: casual.integer(1, 1000),
  title: casual.title,
  url: casual.url,
  image_src: "http://image.medium.url",
...
export const makeSearchResult = (type: string | null | undefined) => {
  if (!type) {
...
export const makeSearchFacetResult = () => {
  return {
    ...
    topics: {
      buckets: [{ key: "Science", doc_count: 172 }, { key: "Physics", doc_count: 32 }]
    },
    cost: {
...
export const makeSearchResponse = (pageSize: number = SETTINGS.search_page_size, total: number = SETTINGS.search_page_size * 2, type: string | null | undefined, withFacets: boolean = false) => {
