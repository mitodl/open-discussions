...
import { WIDGET_FIELD_TYPE_MARKDOWN, WIDGET_FIELD_TYPE_NUMBER, WIDGET_FIELD_TYPE_PEOPLE, WIDGET_FIELD_TYPE_URL, WIDGET_FIELD_TYPE_CHECKBOX, WIDGET_TYPE_MARKDOWN, WIDGET_TYPE_PEOPLE, WIDGET_TYPE_RSS, WIDGET_TYPE_URL } from "../lib/constants";
import { incrementer } from "../lib/util";
import { validWidgetRenderers } from "../lib/widgets";

import { WidgetFieldSpec, WidgetInstance, WidgetListResponse, WidgetSpec, RSSWidgetJson } from "../flow/widgetTypes";
...
export const makeWidgetJson = (widgetType: string): Object | null | undefined => {
  switch (widgetType) {
    case WIDGET_TYPE_MARKDOWN:
      return null;
    case WIDGET_TYPE_URL:
...
export const makeWidgetInstance = (widgetType: string | null | undefined = null): WidgetInstance => {
  if (!widgetType) {
...
export const makeFieldSpec = (specType?: string, name?: string): WidgetFieldSpec => {
  if (!specType) {
...
export const makeWidgetSpec = (widgetType: string | null | undefined = null): WidgetSpec => {
  if (!widgetType) {
    widgetType = casual.random_element(Object.keys(validWidgetRenderers));
  }
  ...
  if (widgetType === WIDGET_TYPE_URL) {
    ...
    fieldSpecs = [makeFieldSpec(WIDGET_FIELD_TYPE_URL, "url"), makeFieldSpec(WIDGET_FIELD_TYPE_NUMBER, "feed_display_limit")];
  } else if (widgetType === WIDGET_TYPE_PEOPLE) {
    fieldSpecs = [makeFieldSpec(WIDGET_FIELD_TYPE_PEOPLE, "people"), makeFieldSpec(WIDGET_FIELD_TYPE_CHECKBOX, "show_all_members_link")];
  } else {
...
export const makeWidgetListResponse = (numWidgets: number = 3): WidgetListResponse => ({
  // $FlowFixMe
  id: listIncr.next().value,
  widgets: R.range(1, numWidgets).map(() => makeWidgetInstance()),
  available_widgets: Object.keys(validWidgetRenderers).map(widgetType => makeWidgetSpec(widgetType))
});