...
import { makeChannel, makeModerators, makeContributors, makeChannelInvite } from "./channels";
...
describe("channels related factories", () => {
  ...
  ;[true, false].forEach(specifyUsername => {
    [true, false].forEach(isModerator => {
      it(`should make a moderators list as a ${isModerator ? "moderator" : "regular user"} ${specifyUsername ? "with a special username" : ""}`, () => {
        const moderators = makeModerators(specifyUsername ? "username" : null, isModerator);
        if (specifyUsername) {
  ...
  ;[true, false].forEach(specifyUsername => {
    it(`should make a contributors list ${specifyUsername ? "with a special username" : ""}`, () => {
  ...
  ;[true, false].forEach(specifyEmail => {
    it(`should make a channel invitation ${specifyEmail ? "with a specific email" : ""}`, () => {
