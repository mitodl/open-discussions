...
import { COURSE_ARCHIVED, COURSE_CURRENT, platforms, offeredBys, LR_TYPE_COURSE, LR_TYPE_PROGRAM, LR_TYPE_USERLIST, LR_TYPE_LEARNINGPATH, LR_TYPE_VIDEO, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE, OBJECT_TYPE_MAPPING, DATE_FORMAT } from "../lib/constants";
import { makePodcast, makePodcastEpisode } from "./podcasts";

import { Course, CourseTopic, LearningResourceRun, Program, UserList, ListItem, Video } from "../flow/discussionTypes";
...
export const makeProgram = (): Program => ({
  ...
  is_favorite: casual.boolean,
  items: [// $FlowFixMe: flow seems confused here
  makeUserListItem(LR_TYPE_COURSE), makeUserListItem(LR_TYPE_COURSE)],
  object_type: "program",
  runs: [makeRun()],
  lists: [],
  audience: [],
...
export const makeUserListItems = (count: number = 3): Array<ListItem> => R.times(() => makeUserListItem(casual.random_element([LR_TYPE_VIDEO, LR_TYPE_COURSE, LR_TYPE_PROGRAM, LR_TYPE_USERLIST])), count);
...
const LR_FACTORY_MAPPING = {
  ...
  [LR_TYPE_USERLIST]: R.compose(R.merge({ list_type: LR_TYPE_USERLIST }), makeUserList),
  [LR_TYPE_LEARNINGPATH]: R.compose(R.merge({ list_type: LR_TYPE_LEARNINGPATH }), makeUserList),
...
export const makeLearningResource = (object_type: string): Object => R.merge({ object_type }, LR_FACTORY_MAPPING[object_type]());

export const makeLearningPath = R.compose(R.merge({
...
export const makeFavoritesResponse = () => {
  ...
  return [...courses.map(formatFavorite(LR_TYPE_COURSE)), ...programs.map(formatFavorite(LR_TYPE_PROGRAM))];
};

export const makeRandomResource = () => casual.random_element([makeCourse, makeVideo, makeProgram, makeUserList])();
