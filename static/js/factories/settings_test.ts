...
import { FRONTPAGE_NOTIFICATION, COMMENT_NOTIFICATION, FRONTPAGE_FREQUENCY_CHOICES, FREQUENCY_IMMEDIATE, FREQUENCY_NEVER } from "../reducers/settings";
...
describe("settings factory", () => {
  it("should make a frontpage setting object", () => {
    ...
    assert.include(R.pluck("value", FRONTPAGE_FREQUENCY_CHOICES), setting.trigger_frequency);
  });
  ...
  it("should make a comment setting object", () => {
    ...
    assert.include([FREQUENCY_IMMEDIATE, FREQUENCY_NEVER], setting.trigger_frequency);
  });
});