...
import { FormProps, WithFormProps } from "../flow/formTypes";
...
describe("withForm", () => {
  ...
  let sandbox, formData: any, formEndEditStub, formBeginEditStub, formUpdateStub, formValidateStub, validateFormStub, onSubmitStub, onSubmitResultStub;

  const renderPage = ({ ...props
  }) => mount(<WrappedPage form={formData} processing={true} validateForm={validateFormStub} formBeginEdit={formBeginEditStub} formEndEdit={formEndEditStub} formUpdate={formUpdateStub} formValidate={formValidateStub} onSubmit={onSubmitStub} onSubmitResult={onSubmitResultStub} onSubmitFailure={sandbox.stub()} onUpdate={sandbox.stub()} renderForm={sandbox.stub()} {...props} />);
  ...
  ;[true, false].forEach(checked => {
    it(`should update the form for a checked == ${checked.toString()} checkbox`, () => {
      ...
      form.find("input").simulate("change", { target: { name, type: "checkbox", checked } });
  ...
  describe("onSubmitFailure", () => {
    ...
    ;[[onSubmitFailureStub, true, 2, "is defined"], [undefined, false, 1, "is not defined"]].forEach(([onSubmitFailureProp, expSubmitFailureCall, expValidateCallCount, desc]) => {
      it(`${shouldIf(expSubmitFailureCall)} call onSubmitFailure when onSubmit fails and the prop ${desc}`, async () => {
        const wrapper = renderPage({
  ...
  describe("getSubmitResultErrors", () => {
    ...
    ;[[getSubmitResultErrorsStub, { field: ["error text"] }, true, "is defined and returns an error object"], [getSubmitResultErrorsStub, undefined, false, "is defined and returns nothing"], [undefined, undefined, false, "is not defined"]].forEach(([getSubmitResultErrorsProp, stubReturnValue, expValidationFail, desc]) => {
      it(`${shouldIf(expValidationFail)} add a validation message if the function ${desc}`, async () => {
