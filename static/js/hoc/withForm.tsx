...
import { FormComponentCls, WrappedComponentCls, WithFormProps } from "../flow/formTypes";

const withForm = <T>(FormComponent: FormComponentCls<T>) => (WrappedComponent: WrappedComponentCls<T>) => {
  class withForm extends React.Component<WithFormProps<T>> {
    ...
    onSubmit = async (e?: Object) => {
      ...
      if (R.isEmpty(validation)) {
        try {
          ...
          const submitResultFieldErrors = getSubmitResultErrors ? getSubmitResultErrors(result) : null;
    ...
    render() {
      ...
      const renderForm = props => form ? <FormComponent onUpdate={this.onUpdate} onSubmit={this.onSubmit} onRecaptcha={useRecaptcha ? this.onRecaptcha : null} processing={processing} form={form.value} validation={form.errors} {...props} /> : null;
