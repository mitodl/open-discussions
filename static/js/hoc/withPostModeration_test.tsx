...
import { withPostModeration, postModerationSelector, REPORT_POST_DIALOG } from "./withPostModeration";

import { setSnackbarMessage, showDialog, hideDialog, DIALOG_REMOVE_POST, DIALOG_DELETE_POST } from "../actions/ui";
...
describe("withPostModeration", () => {
  let helper, render, post, getChannelNameStub, mockStoreState, channel, channelName, mockStore, subscribedChannelsStub, anyErrorExcept404Stub;
  ...
  beforeEach(() => {
    ...
    getChannelNameStub = helper.sandbox.stub(utils, "getChannelName").returns(channelName);
    subscribedChannelsStub = helper.sandbox.stub(selectors, "getSubscribedChannels").returns([]);
    anyErrorExcept404Stub = helper.sandbox.stub(restUtils, "anyErrorExcept404").returns(false);
    ...
    render = () => shallow(<WrappedComponent dispatch={mockStore.dispatch} {...getPostModerationSelector()} />);
  });
  ...
  it("should delete a post and refresh the post list", async () => {
    ...
    const wrapper = await render();
    const refreshStub = helper.sandbox.stub(wrapper.instance(), "refreshPostList");
    const hideDeleteStub = helper.sandbox.stub(wrapper.instance(), "hideDeleteDialog");
    ...
    assert.deepEqual(mockStore.getLastAction(), setSnackbarMessage({
  ...
  describe("postModerationSelector", () => {
    ...
    it("should include the reports", () => {
      assert.deepEqual(mockStoreState.reports.data.reports, getPostModerationSelector().reports);
    }) //
    ;[[404, undefined, true, true, true, true, false], [403, undefined, true, true, true, false, true], [undefined, 500, false, false, true, false, false], [404, 500, true, false, true, true, false], [403, 500, false, true, true, false, true], [undefined, undefined, true, true, true, false, false], [undefined, undefined, false, true, false, false, false], [undefined, undefined, true, false, false, false, false]].forEach(([channelError, reportsError, channelsLoaded, reportsLoaded, shouldBeLoaded, shouldBeNotFound, shouldBeNotAuthorized]) => {
      it(`${shouldIf(shouldBeLoaded)} be loaded when we want it that way :)`, () => {
        if (channelError) {
    ...
    ;[true, false].forEach(userIsModerator => {
      it(`${shouldIf(userIsModerator)} indicate user is moderator if ${userIsModerator ? "is" : "is not"} moderator`, () => {
    ...
    ;[["showRemovePostDialog", DIALOG_REMOVE_POST], ["showDeletePostDialog", DIALOG_DELETE_POST], ["reportPostDialogVisible", REPORT_POST_DIALOG]].forEach(([propName, dialogConstant]) => {
      [true, false].forEach(dialogOpen => {
        it(`should include ${propName}=${String(dialogOpen)} if ${dialogConstant} is ${dialogOpen ? "open" : "closed"}`, () => {
          if (dialogOpen) {
    ...
    ;[true, false].forEach(hasError => {
      it(`should have errored=${String(hasError)} when there ${hasError ? "is" : "is not"} an error`, () => {
