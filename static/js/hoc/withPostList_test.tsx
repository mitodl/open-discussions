...
import { makeChannelPostList, makePagination, makePost } from "../factories/posts";
...
describe("withPostList", () => {
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(WrappedPage, Page, {}, {
      posts: postList,
      loadPosts: loadPosts,
      location: { search: {} }
    });
  ...
  describe("pagination", () => {
    [{ sort: "new" }, null].forEach(pagination => {
      it(`has PostList and ${pagination ? "" : "no "}InfiniteScroll if there is ${pagination ? "" : "no "}pagination`, async () => {
        const {
          inner
        } = await render({}, {
          pagination
      ...
      it(`sets hasMore=${String(!!afterUrl)} if it ${afterUrl ? "has" : "doesn't have"} an after url`, async () => {
        const {
          inner
        } = await render({}, {
          pagination: {
    ...
    describe("loadMore", () => {
      ...
      });[["?sort=some_sort&after=different_after", { sort: "some_sort" }], ["", {}]].forEach(([search, expectedExtraParams]) => {
        it(`${search ? "has" : "doesn't have"} query parameters from the URL for the pagination`, async () => {
          const {
            inner
          } = await render({}, {
            canLoadMore: true,
    ...
    describe("PostList", () => {
      ...
      });[["reportPost", "showReportPost", true], ["removePost", "showRemovePost", true], ["reportPost", "showReportPost", false], ["removePost", "showRemovePost", false]].forEach(([prop, showProp, showPropValue]) => {
        it(`${showProp ? "passes" : "doesn't pass"} through ${prop} when ${String(showProp)}=${String(showPropValue)}`, async () => {
          const {
            inner
          } = await render({}, {
            [prop]: prop,
          ...
          assert.equal(inner.find("PostList").prop(prop), showPropValue ? prop : null);
        });
      ...
      it("toggles pinning a post", async () => {
        ...
        assert.deepEqual([actions.posts.patch.requestType, actions.posts.patch.successType, EVICT_POSTS_FOR_CHANNEL], store.getActions().map(R.prop("type")));
      });
    });
  });
});