...
import { getReportForm, onReportUpdate, REPORT_CONTENT_PAYLOAD, REPORT_CONTENT_NEW_FORM } from "../lib/reports";
...
export const withPostModeration = (WrappedComponent: Class<React.Component<any, any>>) => {
  class WithPostModeration extends React.Component<any, any> {
    ...
    reportPost = async () => {
      ...
      if (!R.isEmpty(validation)) {
        ...
        await dispatch(actions.reports.post({
    ...
    render() {
      ...
      return <React.Fragment>
          <Dialog open={reportPostDialogVisible} hideDialog={preventDefaultAndInvoke(this.hideReportPostDialog)} onCancel={this.hideReportPostDialog} onAccept={this.reportPost} validateOnClick={true} title="Report Post" submitText="Yes, Report" id="report-post-dialog">
            {reportForm ? <ReportForm reportForm={reportForm.value} validation={reportForm.errors} onUpdate={onReportUpdate(dispatch)} description="Are you sure you want to report this post for violating the rules of this site?" label="Why are you reporting this post?" /> : null}
          </Dialog>
          <Dialog id="remove-post-dialog" open={showRemovePostDialog} onAccept={this.removePost} hideDialog={this.hideRemoveDialog} submitText="Yes, remove" title="Remove Post">
            <p>
          ...
          <Dialog id="delete-post-dialog" open={showDeletePostDialog} onAccept={this.deletePost} hideDialog={this.hideDeleteDialog} submitText="Yes, delete" title="Delete Post">
            <p>
              Are you sure you want to delete this post? This is a permanent
              action and cannot be reversed.
            </p>
          </Dialog>
          <WrappedComponent {...this.props} removePost={this.openRemovePostDialog} deletePost={this.openDeletePostDialog} approvePost={this.approvePost} ignorePostReports={this.ignoreReports} reportPost={this.openReportPostDialog} />
        </React.Fragment>;
    }
  }
...
export const postModerationSelector = (state: Object, ownProps: Object) => {
  ...
  const loaded = channels.error || reports.error ? true : channels.loaded && reports.loaded;

  const notFound = loaded ? channels.error && channels.error.errorStatusCode === 404 : false;

  const notAuthorized = loaded ? channels.error && channels.error.errorStatusCode === 403 : false;
  ...
  return {
    ...
    errored: anyErrorExcept404([channels, state.posts, state.subscribedChannels])
  };
};