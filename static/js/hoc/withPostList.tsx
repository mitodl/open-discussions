...
import { Post, PostListPagination } from "../flow/discussionTypes";
...
const withPostList = (WrappedComponent: Class<React.Component<any, any>>) => {
  class withPostList extends React.Component<any, any> {
    ...
    renderPosts = () => {
      ...
      let postList = <PostList posts={posts} isModerator={isModerator} togglePinPost={showTogglePinPost ? this.togglePinPost : null} reportPost={showReportPost ? reportPost : null} deletePost={showDeletePost ? deletePost : null} removePost={showRemovePost ? removePost : null} showPinUI={showPinUI} showChannelLinks={showChannelLinks} />;

      if (pagination) {
        postList = <InfiniteScroll hasMore={!!pagination.after} loadMore={this.loadMore} initialLoad={false} loader={<Loading className="infinite" key="loader" />}>
            {postList}
          </InfiniteScroll>;
      }
