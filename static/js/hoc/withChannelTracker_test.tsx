...
describe("withTracker", () => {
  ...
  it("should call GA create and pageview if channel has a tracking id", async () => {
    ...
    assert.ok(gaGaStub.calledWith("create", channel.ga_tracking_id, "auto", {
      name: "UA_FAKE_01"
    }));
    assert.ok(gaGaStub.calledWith("UA_FAKE_01.send", "pageview", window.location.pathname));
  });
  ...
  ;[[true, 4], [false, 2]].forEach(([missingPrevChannel, gaCalls]) => {
    it(`${shouldIf(missingPrevChannel)} call google analytics on componentDidUpdate`, async () => {
      channel.ga_tracking_id = "UA-FAKE-01";
      const prevChannel = missingPrevChannel ? null : channel;
      window.location = "http://fake/c/path";
      const wrapper = await render({}, { location: window.location, channel: channel });
      const prevProps = {
