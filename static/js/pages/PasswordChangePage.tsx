...
import { PwChangeForm } from "../flow/authTypes";
...
const {
  getForm,
  actionCreators
} = configureForm(FORM_KEY, passwordChangeForm);

const onSubmit = (form: PwChangeForm) => actions.passwordChange.post(form.current_password, form.new_password);
...
const mapStateToProps = state => {
  ...
  const invalidPwError = R.view(R.lensPath(["error", "current_password"]))(passwordChange);
  const successfullySubmitted = passwordChange && passwordChange.loaded && !passwordChange.processing && passwordChange.postStatus === FETCH_SUCCESS;
...
export default R.compose(connect(mapStateToProps, {
  ...
  onSubmit,
  ...actionCreators
}, mergeProps), withForm(PasswordChangeForm))(PasswordChangePage);