...
class ChannelRouter extends React.Component<Props> {
  ...
  render() {
    ...
    return <BannerPageWrapper>
        {channel ? <ChannelHeader channel={channel} history={history} isModerator={channel.user_is_moderator}>
        ...
        <Switch>
          ...
          <Route path={`${match.url}/members/`} render={this.renderMembersPage} />
          <Route path={`${match.url}/about/`} render={this.renderAboutPage} />
          <Route exact path={`${match.url}/:postID/:postSlug?`} render={this.renderPostPage} />
          <Route exact path={`${match.url}/:postID/:postSlug?/comment/:commentID`} component={this.renderPostPage} />
        </Switch>
      </BannerPageWrapper>;
  }
}
...
export default R.compose(connect(mapStateToProps, mapDispatchToProps))(ChannelRouter);