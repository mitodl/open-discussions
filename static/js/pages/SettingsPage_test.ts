...
describe("SettingsPage", () => {
  ...
  const renderPage = async () => {
    const [wrapper] = await renderComponent("/settings/", [actions.settings.get.requestType, actions.settings.get.successType, FORM_BEGIN_EDIT]);
  ...
  it("should work ok when the frontpage setting comes back first", async () => {
    ...
    assert.equal(wrapper.find(Radio).props().value, settings[0].trigger_frequency);
    assert.equal(wrapper.find(Checkbox).props().checked, settings[1].trigger_frequency === FREQUENCY_IMMEDIATE);
  });
  ...
  it("should work ok if the frontpage setting doesnt come back first", async () => {
    ...
    helper.getSettingsStub.returns(Promise.resolve(settings));
    const wrapper = await renderPage();
    assert.equal(wrapper.find(Radio).props().value, settings[1].trigger_frequency);
    assert.equal(wrapper.find(Checkbox).props().checked, settings[0].trigger_frequency === FREQUENCY_IMMEDIATE);
  });
});