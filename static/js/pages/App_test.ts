...
describe("App", () => {
  ...
  const isAuthRequiredPage = wrapper => wrapper.find("AuthRequiredPage").exists();
  ...
  beforeEach(() => {
    ...
    helper.getSettingsStub.returns(Promise.resolve([makeFrontpageSetting(), makeCommentSetting()]));
  ...
  it("shows messages in the banner on mount", async () => {
    ...
    const [wrapper] = await renderComponent(`/missing?${qs.stringify({ message })}`, [actions.subscribedChannels.get.requestType, actions.subscribedChannels.get.successType, actions.channel.SET_CHANNEL_DATA, actions.ui.SET_BANNER_MESSAGE]);
  ...
  ;[[true, true, 0, false], [true, false, 0, true], [false, false, 1, false], [false, true, 0, false]].forEach(([needsSite, isAnonPath, expLoadCalls, expRedirect]) => {
    describe(`when needsAuthedSite -> ${String(needsSite)} and isAnonAccessiblePath -> ${String(isAnonPath)}`, () => {
      ...
      beforeEach(() => {
        ...
        needsAuthStub = helper.sandbox.stub(authUtils, "needsAuthedSite").returns(needsSite);
      });
      ...
      it(`${shouldIfGt0(expLoadCalls)} load requirements and ${shouldIf(expRedirect)} redirect`, async () => {
  ...
  ;[SETTINGS_URL, `${SETTINGS_URL}tokenbasedauthtokentoken`].forEach(url => {
    it(`loads requirements after navigating away from settings url ${url}`, async () => {
      const [wrapper] = await renderComponent(url, [actions.settings.get.requestType, actions.settings.get.successType]);
      sinon.assert.notCalled(helper.getChannelsStub);

      await helper.listenForActions([actions.subscribedChannels.get.requestType, actions.subscribedChannels.get.successType, actions.livestream.get.requestType, actions.livestream.get.successType, actions.frontpage.get.requestType, actions.frontpage.get.successType], () => {
        const {
          history
        } = wrapper.find(App).props();
        history.push({ pathname: "/" });
      });
    });
  }) //
  ;[["/learn/", true], ["/", false]].forEach(([url, isLearnUrl]) => {
    it(`${shouldIf(!isLearnUrl)} include a Drawer, ${shouldIf(isLearnUrl)} include ContentToolbar if url is ${url}`, async () => {
  ...
  ;[true, false].forEach(courseUIEnabled => {
    it(`${shouldIf(courseUIEnabled)} render something at "/learn"`, async () => {
  ...
  ;[true, false].forEach(podcastFrontpageEnabled => {
    it(`${shouldIf(podcastFrontpageEnabled)} render something at "/podcasts"`, async () => {
      SETTINGS.podcast_frontpage_enabled = podcastFrontpageEnabled;
      const [wrapper] = await renderComponent("/podcasts/", []);
      assert.equal(podcastFrontpageEnabled, wrapper.find("PodcastFrontpage").exists());
    });
  });
});