...
import { FRONTPAGE_FREQUENCY_CHOICES, FREQUENCY_DAILY, FREQUENCY_NEVER, FREQUENCY_IMMEDIATE, FRONTPAGE_NOTIFICATION, COMMENT_NOTIFICATION } from "../reducers/settings";
import { setSnackbarMessage } from "../actions/ui";
import { withRouter } from "react-router";

import { Dispatch } from "redux";
import { FormValue } from "../flow/formTypes";
import { FrontpageFrequency, CommentFrequency } from "../reducers/settings";
...
const getFrontpageFrequency = R.compose(R.prop("trigger_frequency"), R.find(R.propEq("notification_type", FRONTPAGE_NOTIFICATION)));

const getCommentFrequency = R.compose(R.prop("trigger_frequency"), R.find(R.propEq("notification_type", COMMENT_NOTIFICATION)));
...
class SettingsPage extends React.Component<Props> {
  ...
  onCommentsChange = () => {
    ...
    dispatch(actions.forms.formUpdate({
      ...
      value: {
        ...form.value,
        [COMMENTS_INPUT_NAME]: form.value[COMMENTS_INPUT_NAME] === FREQUENCY_IMMEDIATE ? FREQUENCY_NEVER : FREQUENCY_IMMEDIATE
      }
    }));
  };
        ...
        <div className="main-content settings-page">
          ...
          <form onSubmit={this.onSubmit}>
            <Card>
              ...
              <Radio className="radio" name={FRONTPAGE_INPUT_NAME} value={form.value[FRONTPAGE_INPUT_NAME] || FREQUENCY_DAILY} onChange={this.onFrontpageChange} options={FRONTPAGE_FREQUENCY_CHOICES} />
            </Card>
            <Card>
              <label htmlFor="notifications" className="label">
                When do you want to receive an email notification?
              </label>
              <Checkbox checked={form.value[COMMENTS_INPUT_NAME] === FREQUENCY_IMMEDIATE} className="settings-checkbox" name="notifications" onChange={this.onCommentsChange}>
                When someone responds to one of my posts or comments
              </Checkbox>
            </Card>
            <button type="submit" disabled={saving} className={`blue-button ${saving ? "disabled" : ""}`}>
              Save
            </button>
          </form>
        </div>
...
const mapStateToProps = (state, ownProps): StateProps => ({
  ...
  saving: state.settings.processing && state.settings.patchStatus === FETCH_PROCESSING
});
