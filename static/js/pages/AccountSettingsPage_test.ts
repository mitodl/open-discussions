...
describe("AccountSettingsPage", () => {
  ...
  const renderPage = async () => {
    const [wrapper] = await renderComponent("/settings/account", [actions.accountSettings.get.requestType, actions.accountSettings.get.successType]);
  ...
  it("renders the account settings page with lines for each auth type", async () => {
    ...
    helper.getSocialAuthTypesStub.returns(Promise.resolve([{ provider: "email" }, { provider: "micromasters" }, { provider: "saml" }, { provider: "unknown_provider" }]));
  ...
  it("renders the email auth type first", async () => {
    helper.getSocialAuthTypesStub.returns(Promise.resolve([{ provider: "micromasters" }, { provider: "email" }]));
    const wrapper = await renderPage();
