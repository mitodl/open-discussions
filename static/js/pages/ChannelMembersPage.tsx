...
import { SearchParams, Result } from "../flow/searchTypes";
...
function ChannelMembersPage(props: Props) {
  ...
  return <div className="main-content two-column channel-member-page">
      <div className="header-row">
        <div className="picker-label">Sort by:</div>
        <ChannelMembersSortPicker value={sortField} updatePickerParam={param => () => {
        setSortField(param);
        clearSearch();
        setFrom(0);
      ...
      <InfiniteScroll hasMore={from + CHANNEL_MEMBERS_PAGE_SIZE < total} loadMore={() => {
      if (!searchLoaded) {
        return;
      }
      runSearch(channelMemberParams(channelName, from, sortField));
      setFrom(from + CHANNEL_MEMBERS_PAGE_SIZE);
    }} initialLoad={from === 0} loader={<Loading className="infinite" key="loader" />}>
        <Grid>
          {results.map((result, i) => <Cell key={i} width={6}>
...
export default connect<Props, OwnProps, _, _, _, _>(mapStateToProps, mapDispatchToProps)(ChannelMembersPage);