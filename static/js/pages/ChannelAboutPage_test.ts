...
import ChannelAboutPage, { ChannelAboutPage as InnerChannelAboutPage } from "./ChannelAboutPage";
...
describe("ChannelAboutPage", () => {
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(ChannelAboutPage, InnerChannelAboutPage, {
      forms: {}
    }, { channel });
  ...
  it("should render about content, if present", async () => {
    ...
    const {
      readOnly,
      initialData
    } = inner.find("ArticleEditor").props();
    assert.isTrue(readOnly);
    assert.deepEqual(initialData, channel.about);
  }) //
  ;[true, false].forEach(userIsModerator => {
    it(`${shouldIf(userIsModerator)} render editing UI if user is moderator`, async () => {
