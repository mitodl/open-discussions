...
class ProfilePage extends React.Component<Props> {
  ...
  renderUserWebsiteLinks() {
    ...
    const socialSites = profile.user_websites.filter(site => site.site_type !== PERSONAL_SITE_TYPE).map((site, i) => <SocialSiteLogoLink site={site.site_type} url={site.url} key={i} />);
    // $FlowFixMe: profile.user_websites cannot be undefined here
    const personalSite = profile.user_websites.filter(site => site.site_type === PERSONAL_SITE_TYPE).map((site, i) => <SiteLogoLink url={site.url} key={i} />);
    ...
    return <React.Fragment>
        ...
        <Card className="profile-card">
          <div className="row basic-info">
            <ProfileImage profile={profile} userName={userName} editable={false} imageSize={PROFILE_IMAGE_MEDIUM} />
          ...
          {userName === SETTINGS.username ? <div className="row actions">
              <button type="submit" onClick={this.onClick}>
                Edit
              </button>
            </div> : null}
        </Card>
        <ProfileContributionFeed userName={userName} selectedTab={match.params.objectType || defaultTab} />
      </React.Fragment>;
  }
}
...
export default R.compose(connect(mapStateToProps), withSingleColumn("profile-view-page"), withSpinnerLoading)(ProfilePage);