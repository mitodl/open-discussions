...
describe("LoginPage", () => {
  ...
  beforeEach(() => {
    ...
    helper.postEmailLoginStub.returns(Promise.resolve({
      flow: FLOW_LOGIN,
      state: STATE_SUCCESS
    }));
    renderPage = helper.configureHOCRenderer(ConnectedLoginPage, LoginPage, DEFAULT_STATE, { location: {} });
  });
  ...
  describe("getSubmitResultErrors prop", () => {
    ...
    ;[[[errorText], true], [[], false]].forEach(([errors, expectErrorObject]) => {
      it(`${shouldIf(expectErrorObject)} return error object if API response error count == ${String(errors.length)}`, async () => {
        ...
        const expectedResult = expectErrorObject ? { email: errorText } : undefined;
  ...
  it("should contain a link to the signup page", async () => {
    ...
    const link = inner.find("Link").findWhere(c => c.prop("to") === `${REGISTER_URL}?next=%2F`);
    assert.ok(link.exists());
  });
  ...
  it("form onSubmit prop calls api correctly", async () => {
    ...
    const {
      onSubmit
    } = inner.find("AuthEmailForm").props();
