...
import ConnectedRegisterConfirmPage, { RegisterConfirmPage } from "./RegisterConfirmPage";
...
describe("RegisterConfirmPage", () => {
  ...
  beforeEach(() => {
    ...
    helper.postConfirmRegisterStub.returns(Promise.resolve({
      flow: FLOW_REGISTER,
      state: STATE_REGISTER_DETAILS
    }));
    renderPage = helper.configureHOCRenderer(ConnectedRegisterConfirmPage, RegisterConfirmPage, DEFAULT_STATE, {
      location: {
  ...
  it("should render an error message if no code or partial_token", async () => {
    ...
    assert.include(inner.find("p").at(0).text(), "No confirmation code was provided or it has expired");
