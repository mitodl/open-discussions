...
import { AuthResponse, EmailForm } from "../../flow/authTypes";
...
const onSubmit = ({
  email
}: EmailForm, next: string) => actions.auth.loginEmail(FLOW_LOGIN, email, next);

const getSubmitResultErrors = getAuthResponseFieldErrors("email");

const onSubmitResult = R.curry((setAuthUserDetail: Function, history: Object, response: AuthResponse) => {
...
export default R.compose(connect(mapStateToProps, {
  ...
  setAuthUserDetail,
  ...actionCreators
}, mergeProps), withForm(AuthEmailForm))(LoginPage);