...
import { FLOW_REGISTER, STATE_REGISTER_CONFIRM_SENT, getFormErrorSelector } from "../../reducers/auth";
...
import { EmailForm, AuthResponse } from "../../flow/authTypes";
...
export const RegisterPage = ({
  ...
  match,
  next
}: RegisterPageProps) => <div className="auth-page register-page">
...
const {
  getForm,
  actionCreators
} = configureForm(FORM_KEY, newEmailForm);

const onSubmit = (form: EmailForm, next: string) => actions.auth.registerEmail(FLOW_REGISTER, form.email, next, form.recaptcha);
  ...
  return {
    ...
    onSubmitResult: (response: AuthResponse) => {
      ...
      if (response.state === STATE_REGISTER_CONFIRM_SENT && response.email) {
        setBannerMessage(`We sent an email to <${response.email}>, please validate your address to continue.`);
      }
    },
    next
  };
...
export default R.compose(connect(mapStateToProps, {
  ...
  onSubmit,
  ...actionCreators
}, mergeProps), withForm(AuthEmailForm))(RegisterPage);