...
describe("RegisterPage", () => {
  ...
  beforeEach(() => {
    ...
    helper.postEmailRegisterStub.returns(Promise.resolve({
      flow: FLOW_REGISTER,
      state: STATE_REGISTER_CONFIRM_SENT,
      email
    }));
    renderPage = helper.configureHOCRenderer(ConnectedRegisterPage, RegisterPage, DEFAULT_STATE, { location: {} });
  });
  ...
  it("should contain a link to the login page", async () => {
    ...
    const link = inner.find("Link").findWhere(c => c.prop("to") === `${LOGIN_URL}?next=%2F`);
    assert.ok(link.exists());
  });
  ...
  it("form onSubmit prop calls api correctly", async () => {
    ...
    assert.equal(dispatchedActions[2].type, actions.auth.registerEmail.requestType);
    sinon.assert.calledOnce(helper.postEmailRegisterStub);
    sinon.assert.calledWith(helper.postEmailRegisterStub, FLOW_REGISTER, "test@example.com");
  });
