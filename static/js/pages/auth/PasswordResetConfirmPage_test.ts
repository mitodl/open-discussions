...
import ConnectedPasswordResetConfirmPage, { PasswordResetConfirmPage, FORM_KEY } from "./PasswordResetConfirmPage";
...
describe("PasswordResetConfirmPage", () => {
  ...
  beforeEach(() => {
    ...
    renderPage = helper.configureHOCRenderer(ConnectedPasswordResetConfirmPage, PasswordResetConfirmPage, DEFAULT_STATE);
  });
  ...
  it("should render a success message after the form submits successfully", async () => {
    ...
    assert.include(inner.find("h3").at(0).text(), "Your password has been reset");
  });
  ...
  it("form onSubmit prop calls api correctly", async () => {
    const {
      inner,
      store
    } = await renderPage({}, { match: { params: { token: "1234567890", uid: "ABC" } } });

    const {
      onSubmit
    } = inner.find("PasswordResetConfirmForm").props();
    ...
    assert.equal(dispatchedActions[2].type, actions.passwordReset.postNewPassword.requestType);
    sinon.assert.calledOnce(helper.postPasswordResetNewPasswordStub);
    sinon.assert.calledWith(helper.postPasswordResetNewPasswordStub, "abcdefgh", "abcdefgh", "1234567890", "ABC");
  });
