...
import ConnectedPasswordResetPage, { PasswordResetPage, FORM_KEY } from "./PasswordResetPage";
...
describe("PasswordResetPage", () => {
  ...
  beforeEach(() => {
    ...
    renderPage = helper.configureHOCRenderer(ConnectedPasswordResetPage, PasswordResetPage, DEFAULT_STATE);
  });
  ...
  it("form onSubmit prop calls api correctly", async () => {
    ...
    const {
      onSubmit
    } = inner.find("PasswordResetForm").props();
    ...
    assert.equal(dispatchedActions[2].type, actions.passwordReset.postEmail.requestType);
    sinon.assert.calledOnce(helper.postPasswordResetEmailStub);
    sinon.assert.calledWith(helper.postPasswordResetEmailStub, "test@example.com");
  });

  describe("onSubmitFailure prop", () => {
    [[{ email: "error text" }, "error text", "reset API response with an email error"], [{}, "Error resetting password", "empty reset API response"]].forEach(([submitResponse, expErrorText, responseDesc]) => {
