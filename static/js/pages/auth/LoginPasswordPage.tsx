...
import { getAuthPartialTokenSelector, getAuthFlowSelector, isProcessing } from "../../reducers/auth";
import { getAuthUiNameSelector, getAuthUiEmailSelector, getAuthUiImgSelector } from "../../reducers/ui";

import { Match } from "react-router";
import { PasswordForm, AuthFlow } from "../../flow/authTypes";
...
export class LoginPasswordPage extends React.Component<Props> {
  ...
  render() {
    ...
    const onBackButtonClick = preventDefaultAndInvoke(R.partial(goToFirstLoginStep, [history]));

    return <div className="auth-page login-password-page">
        <div className="main-content">
          <Card className="login-card">
            ...
            <div className="form-header">
              <div className="row">
                <LoginGreeting email={email} name={name} profileImageUrl={profileImageUrl} onBackButtonClick={onBackButtonClick} />
              </div>
            </div>
            {renderForm()}
          </Card>
...
const {
  getForm,
  actionCreators
} = configureForm(FORM_KEY, newPasswordForm);

const onSubmit = (authFlow: AuthFlow, partialToken: string, form: PasswordForm) => actions.auth.loginPassword(authFlow, partialToken, form.password);
...
export default R.compose(connect(mapStateToProps, mapDispatchToProps, mergeProps), withForm(AuthPasswordForm))(LoginPasswordPage);