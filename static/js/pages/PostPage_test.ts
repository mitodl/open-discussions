...
import { makeComment, makeCommentsResponse, makeMoreComments } from "../factories/comments";
...
describe("PostPage", function () {
  let helper, render, post, comments, commentsResponse, channel, twitterEmbedStub;
  ...
  beforeEach(() => {
    ...
    helper.getCommentStub.returns(Promise.resolve(R.slice(0, 1, commentsResponse)));
    ...
    render = helper.configureHOCRenderer(PostPage, InnerPostPage, {
      posts: {
  ...
  it("should set the document title", async () => {
    ...
    assert.equal(inner.find("MetaTags").props().children.find(child => child.type === "title").props.children, formatTitle(post.title));
  });
  ...
  it("should set the document meta description", async () => {
    ...
    assert.equal(inner.find("MetaTags").props().children.find(child => child.type === "meta").props["content"], truncate(post.text, 300));
  });
  ...
  it("should set the document meta canonical link to the post detail url", async () => {
    ...
    assert.equal(inner.find("MetaTags").props().children.find(child => child.type === CanonicalLink).props["relativeUrl"], postDetailURL(channel.name, post.id, post.slug));
  });

  it("should set the document meta canonical link to the comment permalink", async () => {
    const commentLink = commentPermalink(channel.name, post.id, post.slug, comments[0].id);
    const {
      inner
    } = await render({}, {
      match: {
    ...
    assert.equal(inner.find("MetaTags").props().children.find(child => child.type === CanonicalLink).props["relativeUrl"], commentLink);
  });
  ...
  it("should show a comment permalink UI if at the right URL", async () => {
    ...
    assert(card.exists());
    assert.include(card.find("div").at(0).text(), "You are viewing a single comment's thread.");
    assert.equal(card.find(Link).props().to, postDetailURL(channel.name, post.id, post.slug));
    assert.equal(card.find(Link).props().children, "View the rest of the comments");
  ...
  it("loads more comments when the function is called", async () => {
    ...
    assert.includeMembers(R.pluck("type")(actionsList), [actions.morecomments.get.requestType, actions.morecomments.get.successType, REPLACE_MORE_COMMENTS]);

    sinon.assert.calledWith(helper.getMoreCommentsStub, post.id, parent.id, moreComments.children);
  });
    ...
    describe("validation", () => {
      describe("posts", () => {
        it("validates post report content", async () => {
          ...
          const validationStub = helper.sandbox.stub(validationFuncs, "validateContentReportForm").returns({ a: "complaint" });
          await dialog.props().onAccept();
