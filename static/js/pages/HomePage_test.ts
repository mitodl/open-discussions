...
import HomePage, { HomePage as InnerHomePage, mapStateToProps } from "./HomePage";
...
describe("HomePage", () => {
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(HomePage, InnerHomePage, state, ownProps);
  });
  ...
  describe("integration", () => {
    ...
    ;[true, false].forEach(courseUIEnabled => {
      it(`${shouldIf(courseUIEnabled)} render a new courses widget when course_ui_enabled === ${String(courseUIEnabled)}`, async () => {
  ...
  describe("mapStateToProps", () => {
    //
    [[true, makeChannelPostList(), true], [true, [], false], [false, makeChannelPostList(), true], [false, [], true]].forEach(([processing, posts, expectedLoaded]) => {
      it(`sets loaded to ${String(expectedLoaded)} when there are ${posts.length} posts and API request processing=${String(processing)}`, () => {
    ...
    ;[true, false].forEach(loaded => {
      it(`sets canLoadMore=${String(loaded)} when loaded=${String(loaded)}`, () => {
        const props = mapStateToProps({
          ...state,
          frontpage: {
            processing: !loaded,
