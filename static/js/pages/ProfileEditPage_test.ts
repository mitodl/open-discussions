...
describe("ProfileEditPage", function () {
  ...
  const setName = (wrapper, name) => wrapper.find(".profile-name input").simulate("change", makeEvent("name", name));

  const setBio = (wrapper, text) => wrapper.find(".bio textarea").simulate("change", makeEvent("bio", text));

  const setHeadline = (wrapper, text) => wrapper.find(".headline input").simulate("change", makeEvent("headline", text));

  const setLocation = (wrapper, json) => wrapper.find(LocationPicker).props().onChange(makeLocationEvent(json));

  const clearLocation = wrapper => wrapper.find(LocationPicker).props().onClear();

  const submitProfile = wrapper => wrapper.find(".save-profile").simulate("click");
  ...
  const basicProfileEditPageActions = [actions.forms.FORM_BEGIN_EDIT, actions.profiles.get.requestType, actions.profiles.get.successType, actions.subscribedChannels.get.requestType, actions.subscribedChannels.get.successType];

  const renderPage = async () => {
    const [wrapper] = await renderComponent(editProfileURL(profile.username), basicProfileEditPageActions);
  ...
  it("should update values and successfully pass them to api on submit", async () => {
    ...
    await listenForActions([actions.forms.FORM_UPDATE, actions.forms.FORM_UPDATE, actions.forms.FORM_UPDATE, actions.forms.FORM_UPDATE], () => {
    ...
    helper.updateProfileStub.returns(Promise.resolve({ ...profile, name, bio, headline, location }));

    await listenForActions([actions.profiles.patch.requestType, actions.profiles.patch.successType], () => {
  ...
  it("should show validation errors when the name is empty", async () => {
    ...
    setName(wrapper, "");
    submitProfile(wrapper);
    assert.equal(wrapper.find(".profile-name .validation-message").text(), "Name is required");
    sinon.assert.notCalled(helper.updateProfileStub);
  });

  it("goes back when cancel is clicked", async () => {
    const wrapper = await renderPage();
    assert.equal(helper.currentLocation.pathname, editProfileURL(profile.username));
