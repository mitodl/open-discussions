...
import { ProfilePayload } from "../flow/discussionTypes";
...
export const getFormValuesFromProfile = (profile: Profile): ProfilePayload => R.pickAll(["name", "bio", "headline"], profile);
...
class ProfileEditPage extends React.Component<Props> {
  ...
  beginFormEdit() {
    ...
    dispatch(actions.forms.formBeginEdit(R.merge(EDIT_PROFILE_PAYLOAD, {
  ...
  onSubmit = (e: Object) => {
    ...
    if (!profileForm || !R.isEmpty(validation)) {
      ...
      dispatch(actions.profiles.patch(profile.username, profileForm.value)).then(profile => {
  ...
  onSubmitWebsite = R.curry((siteType: string, formPropGetter: Function, e: Object) => {
    const {
      dispatch,
      profile
    } = this.props;
    ...
    dispatch(actions.userWebsites.post(profile.username, url, siteType)).then(() => {
      this.loadData();
      dispatch(actions.forms.formEndEdit({
        formKey: SITE_FORM_KEYS[siteType]
      }));
    }).catch(error => {
      dispatch(actions.forms.formValidate({
  ...
  onSubmitSocialSite = this.onSubmitWebsite(SOCIAL_SITE_TYPE, R.prop("socialSiteForm"));
  onUpdatePersonalSite = this.onUpdateWebsite(PERSONAL_SITE_TYPE);
  onSubmitPersonalSite = this.onSubmitWebsite(PERSONAL_SITE_TYPE, R.prop("personalSiteForm"));
  ...
  render() {
    ...
    return profile.username === SETTINGS.username ? <React.Fragment>
        <MetaTags>
          <title>{formatTitle("Edit your profile")}</title>
          <CanonicalLink relativeUrl="profile/edit" />
        </MetaTags>
        <ProfileForm profile={profile} onUpdate={this.onUpdate} onSubmit={this.onSubmit} form={profileForm.value} validation={profileForm.errors} socialSiteFormValues={R.propOr({}, "value", socialSiteForm)} socialSiteFormErrors={R.propOr({}, "errors", socialSiteForm)} personalSiteFormValues={R.propOr({}, "value", personalSiteForm)} personalSiteFormErrors={R.propOr({}, "errors", personalSiteForm)} onUpdateSocialSite={this.onUpdateSocialSite} onSubmitSocialSite={this.onSubmitSocialSite} onUpdatePersonalSite={this.onUpdatePersonalSite} onSubmitPersonalSite={this.onSubmitPersonalSite} onUpdateLocation={this.onUpdateLocation} onClearLocation={this.onClearLocation} onDeleteSite={this.onDeleteWebsite} history={history} processing={processing} />
      </React.Fragment> : <Redirect to={profileURL(profile.username)} />;
  }
}
...
export default R.compose(connect(mapStateToProps), withSingleColumn("profile-edit-page"), withSpinnerLoading)(ProfileEditPage);