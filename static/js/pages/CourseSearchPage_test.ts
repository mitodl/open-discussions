...
describe("CourseSearchPage", () => {
  ...
  beforeEach(() => {
    ...
    const numHits = 10;
    searchResponse = makeSearchResponse(SETTINGS.search_page_size, numHits, "course", true);
    ...
    helper.stubComponent(LRCardModule, "LearningResourceCard", "LearningResourceCard");
    ...
    render = helper.configureReduxQueryRenderer(CourseSearchPage, {}, initialState);
  ...
  it("renders search results", async () => {
    ...
    searchResponse.hits.hits.forEach((result, i) => {
      assert.deepEqual(wrapper.find("SearchResult").at(i).prop("result"), result._source);
    });
    assert.include(wrapper.find(".results-count").text(), searchResponse.hits.total);
  });
  ...
  it("renders the  Resource facet", async () => {
    ...
    wrapper.update();
    assert.equal(wrapper.find(SearchFacet).at(2).prop("title"), "Learning Resource");
  }) //
  ;[["mechical enginr", "mechanical engineer"], ['"mechical enginr"', '"mechanical engineer"']].forEach(([text, suggestion]) => {
    it(`renders suggestion ${suggestion} for query ${text}`, async () => {
      ...
      const [{
        search
      }] = replaceStub.args[replaceStub.args.length - 1];
  ...
  ;[[true, false, false], [false, true, true], [true, true, true]].forEach(([loaded, processing, shouldShowPostloading]) => {
    it(`${shouldIf(shouldShowPostloading)} show the loading UI when processing = ${String(processing)} and loaded = ${String(loaded)}`, async () => {
      let resolver;
      helper.searchStub.returns(new Promise(resolve => {
        resolver = () => {
      ...
      wrapper.update();
      assert.equal(wrapper.find("CourseSearchLoading").exists(), shouldShowPostloading);
  ...
  it("shows a message saying there are no results", async () => {
    ...
    wrapper.update();
    assert.equal(wrapper.find(".empty-list-msg").text(), "There are no results to display.");
  });
  ...
  it("triggers a non-incremental search when the facet changes", async () => {
    ...
    wrapper.find(SearchFacet).at(1).props().onUpdate({
  ...
  it("facetOptions adds any selected facets not in results to the group", async () => {
    ...
    const {
      topics
    } = wrapper.find("CourseFilterDrawer").prop("activeFacets");
  ...
  it("toggleFacet should let you turn off an active facet", async () => {
    ...
    wrapper.update();
    wrapper.find("CourseFilterDrawer").prop("toggleFacet")("topics", "NewTopic", false);
    wrapper.update();
    const {
      topics
    } = wrapper.find("CourseFilterDrawer").prop("activeFacets");
