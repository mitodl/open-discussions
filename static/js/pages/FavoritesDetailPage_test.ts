...
describe("FavoritesDetailPage tests", () => {
  ...
  it("should the items the user has favorited", async () => {
    ...
    assert.deepEqual([...wrapper.find(LearningResourceCard)].map(card => card.props.object.id).sort(), favorites.map(item => item.content_data.id).sort());
  });
  ...
  it("should show an empty message if there aren't any favorites", async () => {
    ...
    assert.equal(wrapper.find(".empty-message").at(0).text(), "You don't have any favorites.");
  });
});