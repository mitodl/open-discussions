...
import ConnectedPasswordChangePage, { PasswordChangePage, FORM_KEY } from "./PasswordChangePage";
...
describe("PasswordChangePage", () => {
  ...
  beforeEach(() => {
    ...
    renderPage = helper.configureHOCRenderer(ConnectedPasswordChangePage, PasswordChangePage, DEFAULT_STATE);
  });
  ...
  it("should render a success message after the form submits successfully", async () => {
    ...
    assert.include(inner.find("h3").at(0).text(), "Your password has been changed successfully!");
  });
  ...
  it("form onSubmit prop calls api correctly", async () => {
    ...
    const {
      onSubmit
    } = inner.find("PasswordChangeForm").props();
    ...
    assert.equal(dispatchedActions[2].type, actions.passwordChange.post.requestType);
