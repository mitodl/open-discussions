...
import { makeUserList, makeUserListItems } from "../factories/learning_resources";
...
describe("UserListDetailPage tests", () => {
  ...
  beforeEach(() => {
    ...
    helper.handleRequestStub.withArgs(userListDetailApiURL.param({ userListId: userList.id }).toString()).onFirstCall().returns({
      status: 200,
      body: userList
    });
    helper.handleRequestStub.withArgs(userListItemsApiURL.param({ userListId: userList.id }).toString()).onFirstCall().returns({
      status: 200,
      body: {
        count: 4,
  ...
  it("should should show the description", async () => {
    ...
    assert.equal(wrapper.find(".list-description").text(), userList.short_description);
  });
  ...
  ;[true, false].forEach(usersOwnList => {
    it(`${shouldIf(usersOwnList)} show some editing controls if ${usersOwnList ? "is" : "is not"} the users own list`, async () => {
      if (usersOwnList) {
  ...
  ;[[0, false], [1, false], [2, true], [3, true]].forEach(([itemCount, shouldShowButton]) => {
    it(`${shouldIf(shouldShowButton)} show the reorder button for LP with ${String(itemCount)} items`, async () => {
      // $FlowFixMe: flow thinks this is invalid for who knows what reason
      userList.author = SETTINGS.user_id;
