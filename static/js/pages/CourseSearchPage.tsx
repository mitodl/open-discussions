...
import { BannerPageWrapper, BannerPageHeader, BannerContainer, BannerImage } from "../components/PageBanner";
import CourseFilterDrawer from "../components/search/CourseFilterDrawer";

import { actions } from "../actions";
import { clearSearch } from "../actions/search";
import { LR_TYPE_ALL, LR_TYPE_LEARNINGPATH, LR_TYPE_USERLIST, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE } from "../lib/constants";
import { emptyOrNil, preventDefaultAndInvoke, GRID_MOBILE_BREAKPOINT, isDoubleQuoted } from "../lib/util";
import { mergeFacetResults, SEARCH_GRID_UI, SEARCH_LIST_UI } from "../lib/search";
import { COURSE_SEARCH_BANNER_URL } from "../lib/url";
import { deserializeSearchParams, serializeSearchParams } from "../lib/course_search";
import { useResponsive, useWidth } from "../hooks/util";

import { SortParam, LearningResourceResult } from "../flow/searchTypes";
import { Match } from "react-router";
import { CellWidth } from "../components/Grid";
...
const courseSearchSelector = createSelector(state => state.search, search => {
  const {
    error,
    results,
    total,
...
export function Results(props: ResultProps) {
  ...
  if (total === 0 && !processing && loaded) {
    return <div className="empty-list-msg">There are no results to display.</div>;
  }

  return <InfiniteScroll hasMore={from + SETTINGS.search_page_size < total} loadMore={loadMore} initialLoad={from === 0} loader={<Loading className="infinite" key="loader" />}>
      <Grid>
        {results.map((result, i) => <Cell width={searchResultCellWidth} key={i}>
            <SearchResult result={result} toggleFacet={toggleFacet} searchResultLayout={searchResultLayout} />
          </Cell>)}
      </Grid>
    </InfiniteScroll>;
}
...
export default function CourseSearchPage(props: Props) {
  ...
  const facetOptions = useCallback((group: string) => {
  ...
  const runSearch = useCallback(async (text, activeFacets, incremental = false) => {
    ...
    await dispatch(actions.search.post({
      channelName: null,
      text,
  ...
  const toggleFacet = useCallback(async (name: string, value: string, isEnabled: boolean) => {
  ...
  const onUpdateFacets = useCallback((e: Object) => {
    toggleFacet(e.target.name, e.target.value, e.target.checked);
  }, [toggleFacet]);

  const updateText = useCallback((event: Object) => {
    const text = event ? event.target.value : "";
    setText(text);
  }, [setText]);

  const clearText = useCallback((event: Object) => {
  ...
  useEffect(() => {
    ...
    } else if (deviceWidth < THREE_CARD_BREAKPOINT && deviceWidth >= GRID_MOBILE_BREAKPOINT) {
  ...
  const suggestions = !emptyOrNil(suggest) && !emptyOrNil(text) ? R.without([text], suggest).map(suggestion => isDoubleQuoted(text) ? `"${suggestion}"` : suggestion) : [];
  ...
  return <BannerPageWrapper>
      ...
      <BannerPageHeader tall compactOnMobile>
        ...
        <CourseSearchbox onChange={updateText} value={text || ""} onClear={clearText} onSubmit={onSubmit} validation={error} autoFocus />
      </BannerPageHeader>
      <Grid className={`main-content ${searchResultLayout === SEARCH_GRID_UI ? "two-column-extrawide" : "two-column"} search-page`}>
        <Cell width={facetColumnWidth} />
        <Cell width={resultsColumnWidth}>
          {!emptyOrNil(suggestions) ? <div className="suggestion">
              Did you mean
              {suggestions.map((suggestion, i) => <span key={i}>
                  <a onClick={preventDefaultAndInvoke(() => acceptSuggestion(suggestion))} onKeyPress={e => {
              if (e.key === "Enter") {
          ...
          <div className="layout-buttons">
            <div onClick={() => setSearchResultLayout(SEARCH_LIST_UI)} onKeyPress={e => {
            if (e.key === "Enter") {
              setSearchResultLayout(SEARCH_LIST_UI);
            }
          }} tabIndex="0" className={`option ${searchResultLayout === SEARCH_LIST_UI ? "active" : ""}`}>
              <i className="material-icons view_list">view_list</i>
            </div>
            <div onClick={() => setSearchResultLayout(SEARCH_GRID_UI)} onKeyPress={e => {
            if (e.key === "Enter") {
              setSearchResultLayout(SEARCH_GRID_UI);
            }
          }} tabIndex="0" className={`option ${searchResultLayout === SEARCH_GRID_UI ? "active" : ""}`}>
              <i className="material-icons view_comfy">view_comfy</i>
            </div>
            {processing ? null : <div className="results-count">
                {total} {total === 1 ? "Result" : "Results"}
              </div>}
          </div>
        </Cell>
        <Cell className="search-filters" width={facetColumnWidth}>
          <CourseFilterDrawer activeFacets={activeFacets} clearAllFilters={clearAllFilters} toggleFacet={toggleFacet} facetOptions={facetOptions} onUpdateFacets={onUpdateFacets} />
        </Cell>
        <Cell width={resultsColumnWidth}>
          {error ? null : <Results from={from} incremental={incremental} loadMore={loadMore} loaded={loaded} processing={processing} results={results} searchResultCellWidth={searchResultCellWidth} searchResultLayout={searchResultLayout} toggleFacet={toggleFacet} total={total} />}
        </Cell>
      </Grid>
    </BannerPageWrapper>;
}