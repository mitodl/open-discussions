...
import ChannelMembersPage, { CHANNEL_MEMBERS_PAGE_SIZE, getSortOption } from "./ChannelMembersPage";
...
describe("ChannelMembersPage", () => {
  ...
  beforeEach(() => {
    channel = makeChannel();
    searchResponse = makeSearchResponse(CHANNEL_MEMBERS_PAGE_SIZE, CHANNEL_MEMBERS_PAGE_SIZE + 40, "profile");
    ...
    renderPage = () => mount(<Provider store={helper.createMockStore(initialState)}>
  ...
  it("should render results using SearchResult in a Grid", () => {
    ...
    R.zip([...page.find(SearchResult)], searchResponse.hits.hits).forEach(([el, result]) => {
  ...
  ;[[MEMBERS_SORT_AUTHOR_NAME, "asc"], [MEMBERS_SORT_JOIN_DATE, {
    order: "desc",
    ...
    it(`getSortOption returns what we want for ${sortField}`, () => {
      assert.deepEqual(getSortOption(sortField, "worst-channel-ever.jpg"), expectation);
    });
  });
});