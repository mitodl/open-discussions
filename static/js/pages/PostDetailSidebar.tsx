...
import { PostResult } from "../flow/searchTypes";
...
export class PostDetailSidebar extends React.Component<Props> {
  ...
  render() {
    ...
    return <Card className="related-posts">
        <h3>Similar Posts</h3>
        {relatedPosts.map((post: PostResult) => <div key={post.post_id} className="related-post-item">
            {post.post_type === LINK_TYPE_LINK ? <a href={post.post_link_url} className="external-link" target="_blank" rel="noopener noreferrer">
                <i className="material-icons open_in_new">open_in_new</i>
              </a> : null}
            <Link to={postDetailURL(post.channel_name, post.post_id, post.post_slug)} className="title-link">
              {post.post_title}
            </Link>
...
export default connect<Props, OwnProps, _, _, _, _>(mapStateToProps)(PostDetailSidebar);