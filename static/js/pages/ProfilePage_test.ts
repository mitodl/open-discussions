...
describe("ProfilePage", function () {
  ...
  const basicProfilePageActions = [actions.profiles.get.requestType, actions.profiles.get.successType, actions.subscribedChannels.get.requestType, actions.subscribedChannels.get.successType, actions.userContributions.get.requestType];

  const renderPage = async (url: string | null | undefined) => {
  ...
  it("should display profile name, bio, and headline", async () => {
    ...
    assert.deepEqual(wrapper.find(".name-headline-location .name").at(0).text(), profile.name);
    assert.deepEqual(wrapper.find(".name-headline-location .headline").at(0).text(), profile.headline);
  ...
  it("should include a ProfileImage equal to profile.profile_image_medium", async () => {
    const wrapper = await renderPage();
    assert.equal(wrapper.find(ProfileImage).find("img").at(1).props().src, profile.profile_image_medium);
  });
  ...
  it("should pass the right selected tab value to the post/comment feed component", async () => {
    const wrapper = await renderPage(profileURL(profile.username, COMMENTS_OBJECT_TYPE));
    const feedComponent = wrapper.find("ProfileContributionFeed");
    assert.equal(feedComponent.prop("selectedTab"), COMMENTS_OBJECT_TYPE);
  });[true, false].forEach(sameUser => {
    it(`should not include an edit profile button on this page when profile is ${sameUser ? "" : "not"} for the logged in user`, async () => {
