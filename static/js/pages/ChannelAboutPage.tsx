...
export class ChannelAboutPage extends React.Component<Props> {
  ...
  renderAboutContent = () => {
    ...
    // assumptions
    return channelForm ? <ArticleEditor key="edit" initialData={channel.about || []} onChange={editorUpdateFormShim("about", updateChannelForm)} /> : <ArticleEditor key="view" readOnly initialData={channel.about || []} />;
  };
  ...
  renderChannelEditUI = () => {
    ...
    return channelForm ? <button className="save-button" onClick={this.saveChannelAbout}>
        Save
      </button> : <button className="edit-button" onClick={() => beginChannelFormEdit(channel)}>
        Edit
      </button>;
  };
  ...
  render() {
    ...
    return <React.Fragment>
        ...
        {channel && channel.user_is_moderator ? this.renderChannelEditUI() : null}
      </React.Fragment>;
  }
}
...
export default R.compose(connect(mapStateToProps, mapDispatchToProps), withSingleColumn("home-page"))(ChannelAboutPage);