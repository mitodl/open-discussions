...
describe("SearchPage", () => {
  let helper, renderPage, searchResponse, channel, initialState, initialProps, upvotedPost;
  ...
  beforeEach(() => {
    ...
    renderPage = helper.configureHOCRenderer(ConnectedSearchPage, SearchPage, initialState, initialProps);
  });
  ...
  it("renders search results", async () => {
    ...
    assert.deepEqual(inner.props().upvotedPosts.get(upvotedPost.id), upvotedPost);
    searchResponse.hits.hits.forEach((result, i) => {
      assert.deepEqual(inner.find("SearchResult").at(i).prop("result"), result);
      assert.deepEqual(inner.find("SearchResult").at(i).prop("upvotedPost"), result.post_id === upvotedPost.id ? upvotedPost : null);
    });
  });
  ...
  ;[0, 5].forEach(from => {
    it(`InfiniteScroll initialLoad ${shouldIf(from > 0)} be false when from is ${from}`, async () => {
      const {
        wrapper,
        inner
      } = await renderPage();
  ...
  ;[[true, false, false], [false, true, false], [false, false, false], [true, true, true]].forEach(([initialLoad, searchProcessing, expected]) => {
    it(`${expected ? "shows" : "doesn't show"} PostLoading when we are ${searchProcessing ? "loading" : "not loading"} search results and initialLoad = ${String(initialLoad)}`, async () => {
      const {
        inner
      } = await renderPage({
        search: {
  ...
  it("shows a message saying there are no results", async () => {
    ...
    assert.equal(inner.find(".empty-list-msg").text(), "There are no results to display.");
  });
  ...
  it("triggers a non-incremental search when the filter type changes", async () => {
    ...
    assert.equal(store.getActions()[store.getActions().length - 2].type, actions.search.clear);
  });
  ...
  ;[[true, true, true, true], [true, true, false, true], [true, false, true, true], [true, false, false, true], [false, true, true, false], [false, true, false, true], [false, false, true, false], [false, false, false, true]].forEach(([channelLoaded, searchLoaded, hasChannel, loaded]) => {
    it(`has loaded=${String(loaded)} when channelLoaded=${String(channelLoaded)} and searchLoaded=${String(searchLoaded)} and it ${hasChannel ? "has" : "doesn't have"} a channel`, async () => {
      const {
        inner
      } = await renderPage({
        channels: {
  ...
  it("calls updateVotedComments when upvote or downvote is triggered", async () => {
    ...
    assert.deepEqual(inner.state().votedComments.get(comments[0].id), comments[0]);
    helper.updateCommentStub.returns(Promise.resolve(comments[1]));
    await inner.instance().downvote(comments[1]);
    assert.deepEqual(inner.state().votedComments.get(comments[1].id), comments[1]);
  });
});