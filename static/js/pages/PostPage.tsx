...
import { withPostModeration, postModerationSelector } from "../hoc/withPostModeration";
...
import { formatCommentsCount, getPostDropdownMenuKey, postMenuDropdownFuncs } from "../lib/posts";
...
import { anyErrorExcept404, anyErrorExcept404or410or500, any404Error, anyNotAuthorizedErrorType } from "../util/rest";
...
import { FormsState } from "../flow/formTypes";
import { Channel, CommentInTree, GenericComment, MoreCommentsInTree, Post, Profile } from "../flow/discussionTypes";
...
type PostPageProps = {
  ...
  channel: Channel;
  isModerator: boolean;
  focusedComment: CommentInTree | null | undefined;
...
const shouldLoadData = R.complement(R.allPass([R.eqProps("postID"), R.eqProps("channelName"), R.eqProps("commentID"), R.eqBy(R.path(["location", "search"]))]));
...
export class PostPage extends React.Component<PostPageProps> {
  ...
  loadData = async () => {
    ...
    try {
      const [post] = await Promise.all([dispatch(actions.posts.get(postID)), dispatch(actions.comments.get(postID, commentID, qs.parse(search)))]);
  ...
  loadMoreComments = async (comment: MoreCommentsInTree) => {
    ...
    const comments = await dispatch(actions.morecomments.get(postId, parentId, children));
    dispatch(replaceMoreComments({
      postId,
      parentId,
      comments
    }));
  ...
  downvote = async (comment: CommentInTree) => {
    ...
    await dispatch(actions.comments.patch(comment.id, {
  ...
  renderCommentSectionHeader = () => {
    ...
    if (commentID) {
      return <Card className="comment-detail-card">
    ...
    if (post.num_comments > 0) {
      return <div className="count-and-sort">
          <div className="comments-count">{formatCommentsCount(post)}</div>
          <CommentSortPicker updatePickerParam={updateCommentSortParam(this.props)} value={qs.parse(search).sort || COMMENT_SORT_BEST} />
        </div>;
    }
    return null;
  };
  ...
  render() {
    ...
    const {
      showPostMenu,
      hidePostMenu
    } = postMenuDropdownFuncs(dispatch, post);
    ...
    return <div>
        <MetaTags>
          ...
          <CanonicalLink relativeUrl={commentID ? commentPermalink(channel.name, post.id, post.slug, commentID) : postDetailURL(channel.name, post.id, post.slug)} />
          <meta name="description" content={truncate(post.text, 300)} />
        </MetaTags>
        <Card className="post-card">
          <div className="post-card-inner">
            <ExpandedPostDisplay post={post} isModerator={isModerator} approvePost={approvePost.bind(this)} removePost={removePost.bind(this)} forms={forms} beginEditing={beginEditing(dispatch)} showPostDeleteDialog={this.showPostDialog(DELETE_POST_DIALOG)} showPostReportDialog={preventDefaultAndInvoke(() => reportPost(post))} showPermalinkUI={showPermalinkUI} toggleFollowPost={toggleFollowPost(dispatch)} embedly={embedly} postDropdownMenuOpen={postDropdownMenuOpen} showPostMenu={showPostMenu} hidePostMenu={hidePostMenu} channel={channel} />
          </div>
        </Card>
        {showPermalinkUI ? null : <CommentForm post={post} profile={profile} />}
        {this.renderCommentSectionHeader()}
        {commentsTree.length > 0 ? <CommentTree comments={commentsTree} isPrivateChannel={isPrivate(channel)} isModerator={isModerator} loadMoreComments={this.loadMoreComments} processing={commentInFlight} post={post} commentPermalink={commentPermalink(channel.name, post.id, post.slug)} /> : null}
        <Dialog open={postDeleteDialogVisible} hideDialog={hidePostDialog} onAccept={async () => {
        await this.deletePost(post);
        hidePostDialog();
      }} title="Delete Post" submitText="Yes, Delete">
...
const mapStateToProps = (state, ownProps) => {
  ...
  const loaded = notFound ? true : R.none(R.isNil, [post, channel, commentsTree]);

  const postDropdownMenuOpen = post ? ui.dropdownMenus.has(getPostDropdownMenuKey(post)) : false;
  ...
  return {
    ...
    errored: anyErrorExcept404([posts, channels]) || anyErrorExcept404or410or500([comments]),
...
export default R.compose(connect(mapStateToProps), withPostModeration, SETTINGS.allow_related_posts_ui ? withPostDetailSidebar("post-page") : withSingleColumn("post-page"), withChannelTracker, withSpinnerLoading)(PostPage);