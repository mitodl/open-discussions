...
import { LINK_TYPE_LINK, LINK_TYPE_TEXT, LINK_TYPE_ARTICLE } from "../lib/channels";
...
import { Channel, CreatePostPayload, PostForm, PostValidation } from "../flow/discussionTypes";
import { LinkType } from "../lib/channels";
import { RestState } from "../flow/restTypes";
...
type CreatePostPageProps = {
  ...
  postForm: PostFormValue | null | undefined;
  channel: Channel;
...
const shouldFormReset = (allowedTypes: Array<LinkType>, prevChannelPostTypes: Array<string> | null | undefined, postForm: PostFormValue) => {
  ...
  if (allowedTypes.length === 1 && allowedTypes[0] !== postForm.value.postType) {
    return true;
  }
  ...
  if (allEmptyOrNil([postForm.value.url, postForm.value.text, postForm.value.article_content])) {
    return true;
  }
...
class CreatePostPage extends React.Component<CreatePostPageProps> {
  ...
  async componentDidMount() {
    ...
    dispatch(actions.forms.formBeginEdit({
      ...CREATE_POST_PAYLOAD,
      value: form
    }));
  ...
  updateForm = (update: Object) => {
    ...
    dispatch(actions.forms.formUpdate({
      ...CREATE_POST_PAYLOAD,
      value: update
    }));
  };
  ...
  componentDidUpdate(prevProps: CreatePostPageProps) {
    ...
    const prevChannelPostTypes = R.path(["channel", "allowed_post_types"], prevProps);
    if (postForm && channel && shouldFormReset(channel.allowed_post_types, prevChannelPostTypes, postForm)) {
      this.updateForm({
        postType: channel.allowed_post_types.length > 1 ? null : channel.allowed_post_types[0],
        url: "",
        text: "",
        thumbnail: null,
        article_content: [],
  ...
  updatePostType = (postType: string | null | undefined) => {
    const {
      dispatch
    } = this.props;
    this.updateForm({
  ...
  getValidationErrors = () => {
    ...
    const validation = R.isNil(channel) ? R.set(R.lensPath(["value", "channel"]), "You need to select a channel", validatePostCreateForm(postForm)) : validatePostCreateForm(postForm);
  ...
  setValidationErrors = (errors: Object) => {
    ...
    dispatch(actions.forms.formValidate({
      ...CREATE_POST_PAYLOAD,
      errors
    }));
  };
  ...
  onSubmit = async (e: Object) => {
    ...
    if (!postForm || !R.isEmpty(validation)) {
      ...
      try {
        ...
        dispatch(setBannerMessage(`Something went wrong creating your post. Please try again or contact us at ${SETTINGS.support_email}`));
      }
    }
  };
  ...
  render() {
    ...
    return <React.Fragment>
        ...
        <Dialog open={clearPostDialogVisible} hideDialog={preventDefaultAndInvoke(this.hideClearPostTypeDialog)} onCancel={preventDefaultAndInvoke(this.hideClearPostTypeDialog)} onAccept={this.clearPostType} title="Clear Post Content?" submitText="Yes" id="clear-post-type-dialog">
          This will clear any input you have made, are you sure you want to
          continue?
        </Dialog>
        <CreatePostForm onSubmit={this.onSubmit} onUpdate={this.onUpdate} updatePostType={this.updatePostType} updateChannelSelection={this.updateChannelSelection} postForm={postForm.value} validation={postForm.errors} channel={channel} history={history} processing={processing} channels={channels.data || new Map()} embedly={embedly} embedlyInFlight={embedlyInFlight} openClearPostTypeDialog={this.openClearPostTypeDialog} setPhotoError={this.setPhotoError} hideCoverImageInput={this.hideCoverImageInput} />
      </React.Fragment>;
  }
}
...
const mapStateToProps = (state, props) => {
  ...
  const embedly = postForm && postForm.value.url ? state.embedly.data.get(postForm.value.url) : undefined;
...
export default R.compose(connect(mapStateToProps), withSingleColumn("create-post-page"))(CreatePostPage);