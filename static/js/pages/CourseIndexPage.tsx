...
import { featuredCoursesRequest, featuredCoursesSelector, upcomingCoursesRequest, upcomingCoursesSelector, newCoursesRequest, newCoursesSelector } from "../lib/queries/courses";
import { newVideosRequest, newVideosSelector } from "../lib/queries/videos";
import { popularContentSelector, popularContentRequest } from "../lib/queries/interactions";
import { favoritesRequest, favoritesListSelector } from "../lib/queries/learning_resources";
...
import { LearningResourceSummary } from "../flow/discussionTypes";
...
const CarouselSection = ({
  ...
  return items.length !== 0 ? <CourseCarousel title={title} courses={items} /> : null;
};
...
export default function CourseIndexPage({
  ...
  return <BannerPageWrapper>
      ...
      <Grid className="main-content one-column">
        <Cell width={12}>
          <CarouselSection title="Your Favorites" isFinished={isFinishedFavorites} items={favorites} />
          <CarouselSection title="Featured Courses" isFinished={isFinishedFeatured} items={featuredCourses} />
          <CarouselSection title="New Courses" isFinished={isFinishedNew} items={newCourses} />
          <CarouselSection title="Popular Learning Resources" isFinished={isFinishedPopular} items={popularResources} />
          <CarouselSection title="Upcoming Courses" isFinished={isFinishedUpcoming} items={upcomingCourses} />
          <CarouselSection title="New Videos" isFinished={isFinishedVideos} items={newVideos} />
        </Cell>
      </Grid>
    </BannerPageWrapper>;
}