...
import { Channel, CommentInTree, Post } from "../flow/discussionTypes";
import { SearchInputs, SearchParams, PostResult, CommentResult, ProfileResult } from "../flow/searchTypes";
...
type Props = {
  ...
  notFound: boolean;
  runSearch: (params: SearchParams) => Promise<any>;
...
export class SearchPage extends React.Component<Props, State> {
  ...
  downvote = async (comment: CommentInTree) => {
    ...
    const updatedComment = await dispatch(actions.comments.patch(comment.id, {
  ...
  renderResults = () => {
    ...
    if (!results.length && (!suggest.length || suggest[0] === this.state.text)) {
      return <div className="empty-list-msg">There are no results to display.</div>;
    }

    return <InfiniteScroll hasMore={from + SETTINGS.search_page_size < total} loadMore={this.loadMore} initialLoad={from === 0} loader={<Loading className="infinite" key="loader" />}>
        {!emptyOrNil(suggest) && suggest[0] !== this.state.text ? <div className="suggestion">
            Did you mean
            {suggest.map((suggestion, i) => <span key={i}>
                <a onClick={preventDefaultAndInvoke(() => this.useSuggestion(suggestion))}>
                  {` ${suggestion}`}
                </a>
                {i < suggest.length - 1 ? " | " : ""}
              </span>)}
          </div> : null}
        {results.length ? results.map((result, i) => <SearchResult key={i} result={result} upvotedPost={result.object_type === "post" ? upvotedPosts.get(result.post_id) : null} votedComment={result.object_type === "comment" ? votedComments.get(result.comment_id) || null : null} commentDownvote={this.downvote} commentUpvote={this.upvote} />) : <div className="empty-list-msg">There are no results to display.</div>}
      </InfiniteScroll>;
  };

  updateText = (event: Event | null | undefined) => {
  ...
  render() {
    ...
    return <Grid className="main-content two-column search-page">
        <Cell mobileWidth={8} width={7}>
          <MetaTags>
            <CanonicalLink match={match} />
          </MetaTags>
          <SearchTextbox onChange={this.updateText} value={text || ""} onClear={this.updateText} onSubmit={preventDefaultAndInvoke(() => this.runSearch())} validation={error} />
          <div className="post-list-title">
            <SearchFilterPicker updatePickerParam={R.curry((type, event) => {
...
export default R.compose(connect(mapStateToProps, mapDispatchToProps), withSearchLoading)(SearchPage);