...
import { podcastsRequest, podcastsSelector, recentPodcastEpisodesRequest, recentEpisodesSelector } from "../lib/queries/podcasts";
...
export default function PodcastFrontpage() {
  const [{
    isFinished: isFinishedPodcasts
  }] = useRequest(podcastsRequest());
  const [{
    isFinished: isFinishedRecent
  }] = useRequest(recentPodcastEpisodesRequest());
  ...
  return <div className="podcasts">
      ...
      <div className="recent-episodes">
        ...
        <div className="subscribe-button-div">
          <PodcastSubscribeButton buttonText="Subscribe to new episodes" rssUrl={`${window.location.origin.toString()}${PODCAST_RSS_URL}`} googleUrl={PODCAST_GOOGLE_URL} />
        </div>
        {isFinishedRecent && isFinishedPodcasts ? <>
            {recentEpisodes.slice(0, 6).map((episode, idx) => <PodcastEpisodeCard episode={episode} podcast={podcasts[episode.podcast]} key={idx} />)}
          </> : <PodcastEpisodeLoading />}
      </div>
      <div className="all-podcasts">
        <h1>Podcasts</h1>
        {isFinishedPodcasts ? <Grid>
            {Object.values(podcasts).map((podcast: any) => <Cell width={4} key={podcast.id}>
