...
import PostDetailSidebar, { PostDetailSidebar as InnerPostDetailSidebar } from "./PostDetailSidebar";
...
describe("PostDetailSidebar", () => {
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(PostDetailSidebar, InnerPostDetailSidebar, {
      relatedPosts: {
        processing: false,
        loaded: false,
        data: []
  ...
  it("shows a list of related posts", async () => {
    ...
    assert.equal(firstRelated.find("Link").at(0).prop("children"), relatedPostData[0].post_title);
  });
});