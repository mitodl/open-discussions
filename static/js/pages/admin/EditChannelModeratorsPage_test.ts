...
import EditChannelModeratorsPage, { EditChannelModeratorsPage as InnerEditChannelModeratorsPage, MODERATORS_KEY } from "./EditChannelModeratorsPage";

import { makeChannel, makeModerator, makeModerators, makeSubscriber } from "../../factories/channels";
import { makeWidgetListResponse } from "../../factories/widgets";
import { actions } from "../../actions";
import { SET_CHANNEL_DATA } from "../../actions/channel";
import { FORM_VALIDATE, FORM_UPDATE } from "../../actions/forms";
import { DIALOG_REMOVE_MEMBER, SHOW_DIALOG, HIDE_DIALOG, SET_DIALOG_DATA, SET_SNACKBAR_MESSAGE } from "../../actions/ui";
...
describe("EditChannelModeratorsPage", () => {
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(EditChannelModeratorsPage, InnerEditChannelModeratorsPage, initialState, initialProps);
    ...
    helper.getPostsForChannelStub.returns(Promise.resolve({
      pagination: {},
      posts: []
    }));
  ...
  it("should set the document title", async () => {
    ...
    assert.equal(inner.find("MetaTags").props().children.props.children, formatTitle("Edit Channel"));
  });
  ...
  it("displays a notice that membership is managed by micromasters", async () => {
    ...
    assert.equal(inner.find(".membership-notice").text(), "Membership is managed via MicroMasters");
  });
  ...
  describe("mounted tests", () => {
    ...
    const mountedRenderPage = async (extraActions = []) => {
      const [wrapper] = await mountedRenderComponent(editChannelModeratorsURL(channel.name), [actions.subscribedChannels.get.requestType, actions.subscribedChannels.get.successType, actions.channels.get.requestType, actions.channels.get.successType, actions.channelModerators.get.requestType, actions.channelModerators.get.successType, actions.profiles.get.requestType, actions.profiles.get.successType, SET_CHANNEL_DATA, actions.forms.FORM_BEGIN_EDIT, ...extraActions]);
      return wrapper.update();
    };

    it("redirects if the user is not a moderator", async () => {
      channel.user_is_moderator = false;
      await mountedRenderPage([actions.channels.get.requestType, actions.channels.get.successType, actions.postsForChannel.get.requestType, actions.postsForChannel.get.successType, actions.widgets.get.requestType, actions.widgets.get.successType, actions.forms.FORM_END_EDIT]);
  ...
  describe("editable", () => {
    ...
    it("tries to add a new moderator but the API request fails", async () => {
      ...
      sinon.assert.calledWith(helper.addChannelModeratorStub, channel.name, email);
    ...
    it("adds a new moderator", async () => {
      ...
      inner.find("EditChannelMembersForm").props().onSubmit({ preventDefault: helper.sandbox.stub() });

      await wait(0);
      sinon.assert.calledWith(helper.addChannelModeratorStub, channel.name, email);
      sinon.assert.calledWith(helper.addChannelSubscriberStub, channel.name, newModerator.moderator_name);

      const actions = store.getActions();
      assert.deepEqual(actions[actions.length - 2], {
        type: SET_SNACKBAR_MESSAGE,
        payload: {
          message: `Successfully added ${String(newModerator.email)} as a moderator`
        }
      });
    });
  });[true, false].forEach(isYou => {
    it(`removes ${isYou ? "yourself from being a moderator" : "a moderator"}`, async () => {
      const {
        inner,
        store
      } = await render({}, { history: helper.browserHistory });
      ...
      await wait(0);
      sinon.assert.calledWith(helper.deleteChannelModeratorStub, channel.name, moderators[0].moderator_name);
      if (isYou) {
        assert.equal(helper.browserHistory.location.pathname, channelURL(channel.name));
      }
      ...
      assert.deepEqual(actions[actions.length - 3], {
        type: SET_SNACKBAR_MESSAGE,
        payload: {
          message: `Successfully removed ${String(moderators[0].email)} as a moderator`
        }
      });
  ...
  it("has a channel header", async () => {
    render = helper.configureHOCRenderer(EditChannelModeratorsPage, "withChannelHeader(WithSingleColumn)", initialState, initialProps);
