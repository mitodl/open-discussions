...
import { AddMemberForm, Channel, Member } from "../../flow/discussionTypes";
...
type Props = {
  ...
  removeMember: (channelName: string, email: string) => Promise<any>;
...
const editChannelMembershipPage = (memberTypeDescription: string, displayName: string, usernameGetter: Function, redirectAfterSelfRemoval: boolean) => {
  class EditChannelMembershipPage extends React.Component<Props> {
    ...
    removeMember = async (channel: Channel, member: Member) => {
      ...
      setSnackbarMessage({
        message: `Successfully removed ${String(member.email)} as a ${memberTypeDescription}`
      });
    ...
    render() {
      ...
      return <React.Fragment>
          ...
          <Card className="members">
            ...
            {!editable ? <div className="membership-notice">
                Membership is managed via MicroMasters
              </div> : renderForm({
            memberTypeDescription
          })}
            <MembersList channel={channel} removeMember={this.removeMember} editable={editable} members={members} usernameGetter={usernameGetter} memberTypeDescription={memberTypeDescription} memberToRemove={memberToRemove} dialogOpen={dialogOpen} setDialogData={setDialogData} setDialogVisibility={setDialogVisibility} />
          </Card>
        </React.Fragment>;
    }
  }
