...
describe("EditChannelBasicPage", () => {
  ...
  const setChannelType = (wrapper, value) => wrapper.find(`input[value='${value}']`).simulate("change", makeEvent("channel_type", value, true));

  const setPostType = (wrapper, value) => wrapper.find(`input[value='${value}']`).simulate("change", makeEvent("allowed_post_types", value, true));

  const submit = wrapper => wrapper.find(".save-changes").simulate("submit");

  const renderPage = async () => {
    const [wrapper] = await renderComponent(editChannelBasicURL(channel.name), [actions.subscribedChannels.get.requestType, actions.subscribedChannels.get.successType, actions.channels.get.requestType, actions.channels.get.successType, SET_CHANNEL_DATA, FORM_BEGIN_EDIT]);
  ...
  it("should set the channel and post types, submit, and navigate back to the channel page", async () => {
    ...
    await listenForActions([actions.channels.patch.requestType, actions.channels.patch.successType, actions.channels.get.requestType, actions.channels.get.successType, actions.widgets.get.requestType, actions.widgets.get.successType, actions.postsForChannel.get.requestType, actions.postsForChannel.get.successType, FORM_END_EDIT], () => {
      submit(wrapper);
    });

    helper.updateChannelStub.calledWith(expected);
    const channelUpdateArg = helper.updateChannelStub.firstCall.args[0];
    assert.deepEqual(Object.keys(channelUpdateArg), ["name", "channel_type", "allowed_post_types"]);
