...
import EditChannelContributorsPage, { EditChannelContributorsPage as InnerEditChannelContributorsPage, CONTRIBUTORS_KEY } from "./EditChannelContributorsPage";

import { makeChannel, makeContributor, makeContributors, makeChannelInvite } from "../../factories/channels";
...
describe("EditChannelContributorsPage", () => {
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(EditChannelContributorsPage, InnerEditChannelContributorsPage, initialState, initialProps);
    ...
    helper.addChannelInvitationStub.returns(Promise.resolve(makeChannelInvite()));
  ...
  it("renders the member list", async () => {
    ...
    assert.equal(props.usernameGetter(contributors[0]), contributors[0].contributor_name);
  ...
  describe("editable", () => {
    ...
    it("tries to add a new contributor but the API request fails", async () => {
      ...
      // let promise resolve
      await wait(0);
      sinon.assert.calledWith(helper.addChannelContributorStub, channel.name, email);
    ...
    it("sends a channel invitation if the channel is private and email doesn't exist", async () => {
      ...
      await wait(0);
      sinon.assert.calledWith(helper.addChannelContributorStub, channel.name, email);
      sinon.assert.calledWith(helper.addChannelInvitationStub, channel.name, email);
    });
    ...
    it("adds a new contributor", async () => {
      ...
      assert.deepEqual(actions[actions.length - 2], {
        ...
        payload: {
          message: `Successfully added ${String(newContributor.email)} as a contributor`
        }
      });
    });
  });
  it("removes a contributor", async () => {
    ...
    await wait(0);
    sinon.assert.calledWith(helper.deleteChannelContributorStub, channel.name, contributors[0].contributor_name);
    ...
    assert.deepEqual(actions[actions.length - 2], {
      type: SET_SNACKBAR_MESSAGE,
      payload: {
        message: `Successfully removed ${String(contributors[0].email)} as a contributor`
      }
    });
    assert.deepEqual(actions[actions.length - 1], {
      type: HIDE_DIALOG,
  ...
  it("has a channel header", async () => {
    render = helper.configureHOCRenderer(EditChannelContributorsPage, "withChannelHeader(WithSingleColumn)", initialState, initialProps);
