...
import EditChannelAppearancePage, { EditChannelAppearancePage as InnerEditChannelAppearancePage } from "./EditChannelAppearancePage";
...
describe("EditChannelAppearancePage", () => {
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(EditChannelAppearancePage, InnerEditChannelAppearancePage, initialState, initialProps);
  ...
  });[[true, true, true, true], [true, true, false, true], [true, false, true, true], [true, false, false, true], [false, true, true, true], [false, true, false, true], [false, false, true, true], [false, false, false, false]].forEach(([page, avatar, banner, expected]) => {
    it(`is ${expected ? "" : "not "}processing when page processing=${String(page)}, avatar processing=${String(avatar)}, and banner processing=${String(banner)}`, async () => {
      const {
        inner
      } = await render({
  ...
  describe("onSubmit", () => {
    [[true, true], [true, false], [false, true], [false, false]].forEach(([hasAvatar, hasBanner]) => {
      it(`submits the form${hasAvatar ? " with an avatar" : ""}${hasBanner ? " with a banner" : ""}`, async () => {
        ...
        await inner.find("EditChannelAppearanceForm").props().onSubmit({ preventDefault: helper.sandbox.stub() });
        ...
        if (hasAvatar) {
          sinon.assert.calledWith(helper.patchChannelAvatarStub, channel.name, avatar.edit, avatar.image.name);
        } else {
          assert.isFalse(helper.patchChannelAvatarStub.called);
        }
        if (hasBanner) {
          sinon.assert.calledWith(helper.patchChannelBannerStub, channel.name, banner.edit, banner.image.name);
        } else {
      ...
      it(`${shouldIf(expectIncludeTitle)} send patch request with title if user ${isIf(isAdmin)} an admin`, async () => {
        ...
        assert.equal(channelUpdateArg.hasOwnProperty("title"), expectIncludeTitle);
      });
    });

    it("doesn't submit if there's a validation error", async () => {
      helper.sandbox.stub(validationFuncs, "validateChannelAppearanceEditForm").returns({ value: "problem" });
      const {
        inner,
        store
      } = await render();
  ...
  it("has a channel header", async () => {
    render = helper.configureHOCRenderer(EditChannelAppearancePage, "withChannelHeader(WithSingleColumn)", initialState, initialProps);
