...
import ChannelModerationPage, { ChannelModerationPage as InnerChannelModerationPage } from "./ChannelModerationPage";
...
import { DIALOG_REMOVE_POST, HIDE_DIALOG, SET_SNACKBAR_MESSAGE } from "../../actions/ui";
import { makeChannelList } from "../../factories/channels";
import { makeChannelPostList } from "../../factories/posts";
import { makeCommentReport, makePostReport, makeReportRecord } from "../../factories/reports";
import { channelModerationURL, channelURL, commentPermalink } from "../../lib/url";
import IntegrationTestHelper from "../../util/integration_test_helper";

describe("ChannelModerationPage", () => {
  let render, channels, channel, postList, postIds, reports, helper, initialState, initialProps;
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(ChannelModerationPage, InnerChannelModerationPage, initialState, initialProps);
  ...
  describe("integration", () => {
    ...
    const renderPage = async () => {
      const [wrapper] = await renderComponent(channelModerationURL(channel.name), [actions.reports.get.requestType, actions.reports.get.successType, actions.channels.get.requestType, actions.channels.get.successType, actions.subscribedChannels.get.requestType, actions.subscribedChannels.get.successType, actions.profiles.get.requestType, actions.profiles.get.successType, SET_CHANNEL_DATA]);
      return wrapper.update();
    };

    it("should fetch reports", async () => {
      const wrapper = await renderPage();
      assert.deepEqual(wrapper.find("ChannelModerationPage").props().reports, reports);
    });
  });

  it("redirects if the user isn't a moderator", async () => {
    // we have to use shallow rendering here because
    const wrapper = shallow( // $FlowFixMe
  ...
  it("has a channel header", async () => {
    render = helper.configureHOCRenderer(ChannelModerationPage, "withChannelHeader(WithSingleColumn)", initialState, initialProps);
  ...
  describe("post reports", () => {
    ...
    it("removes a report for a post", async () => {
      ...
      const props = wrapper.find("OurDialog[id='remove-post-dialog']").first().props();
  ...
  describe("comment report", () => {
    ...
    it("renders comment reports", async () => {
      ...
      reports.forEach((report, index) => {
        ...
        assert.equal(props.commentPermalink("commentId"), commentPermalink(channel.name, report.comment.post_id, null, "commentId"));
