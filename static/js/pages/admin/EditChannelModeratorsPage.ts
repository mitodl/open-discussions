...
const {
  getForm,
  actionCreators
} = configureForm(MODERATORS_KEY, newMemberForm);
...
export const EditChannelModeratorsPage = editChannelMembershipPage("moderator", "EditChannelModeratorsPage", usernameGetter, true);

const mapStateToProps = (state, ownProps) => {
  const channelName = getChannelName(ownProps);
  const channel = state.channels.data.get(channelName);
  const processing = state.channels.processing || state.channelModerators.processing;
...
export default R.compose(connect(mapStateToProps, {
  ...
  ...actionCreators
}, mergeProps), withForm(EditChannelMembersForm), withChannelHeader, withSingleColumn("edit-channel"))(EditChannelModeratorsPage);