...
import { setDialogData, setSnackbarMessage, showDialog, hideDialog, DIALOG_REMOVE_MEMBER } from "../../actions/ui";

import { FormErrors } from "../../flow/formTypes";
...
const {
  getForm,
  actionCreators
} = configureForm(CONTRIBUTORS_KEY, newMemberForm);

const usernameGetter = R.prop("contributor_name");

const onSubmitFailure = (): FormErrors<any> => ({
  email: "Error adding new contributor"
});

export const EditChannelContributorsPage = editChannelMembershipPage("contributor", "EditChannelContributorsPage", usernameGetter, false);

const mapStateToProps = (state, ownProps) => {
  const channelName = getChannelName(ownProps);
  const channel = state.channels.data.get(channelName);
  const processing = state.channels.processing || state.channelContributors.processing;
...
const mergeProps = mergeAndInjectProps(({
  ...
  onSubmit: async form => {
    try {
      ...
      if (isPrivate(channel) && e.email && e.email.includes("email does not exist")) {
        const {
          invite
        } = await inviteMember(channel.name, form.email);
        setSnackbarMessage({
...
export default R.compose(connect(mapStateToProps, {
  ...
  setSnackbarMessage,
  setDialogData: (data: any) => setDialogData({ dialogKey: DIALOG_REMOVE_MEMBER, data: data }),
  setDialogVisibility: (visibility: boolean) => visibility ? showDialog(DIALOG_REMOVE_MEMBER) : hideDialog(DIALOG_REMOVE_MEMBER),
  ...actionCreators
}, mergeProps), withForm(EditChannelMembersForm), withChannelHeader, withSingleColumn("edit-channel"))(EditChannelContributorsPage);