...
import { updatePostTypes, EDIT_CHANNEL_PAYLOAD, getChannelForm } from "../../lib/channels";
...
class EditChannelBasicPage extends React.Component<Props> {
  ...
  onUpdate = (e: Object) => {
    ...
    if (e.target.name === "allowed_post_types") {
      // $FlowFixMe
      updates.allowed_post_types = updatePostTypes(channelForm.value.allowed_post_types, e.target.value, e.target.checked);
    }

    dispatch(actions.forms.formUpdate(R.merge(EDIT_CHANNEL_PAYLOAD, {
      value: updates
    })));
  };
  ...
  onSubmit = (e: Object) => {
    ...
    if (!channelForm || !R.isEmpty(validation)) {
      ...
      const patchValue = R.pickAll(["name", "channel_type", "allowed_post_types"], channelForm.value);
  ...
  render() {
    ...
    return <React.Fragment>
        ...
        {channelForm ? <EditChannelBasicForm onSubmit={this.onSubmit} onUpdate={this.onUpdate} form={channelForm.value} history={history} validation={channelForm.errors} processing={processing} /> : null}
      </React.Fragment>;
  }
}
...
export default R.compose(connect(mapStateToProps, channelFormDispatchToProps), withChannelHeader, withSingleColumn("edit-channel"))(EditChannelBasicPage);