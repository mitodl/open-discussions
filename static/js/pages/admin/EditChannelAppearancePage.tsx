...
import { Channel, ChannelForm } from "../../flow/discussionTypes";
...
export class EditChannelAppearancePage extends React.Component<Props> {
  ...
  onSubmit = async (e: Object) => {
    ...
    const patchValue = R.omit(["avatar", "banner", !SETTINGS.is_admin ? "title" : null], formValue);
    const promises = [dispatch(actions.channels.patch(patchValue))];

    if (formValue.avatar) {
      promises.push(dispatch(actions.channelAvatar.patch(channelName, formValue.avatar.edit, formValue.avatar.image.name)));
    }
    if (formValue.banner) {
      promises.push(dispatch(actions.channelBanner.patch(channelName, formValue.banner.edit, formValue.banner.image.name)));
    }
  ...
  render() {
    ...
    return <React.Fragment>
        ...
        {channelForm ? <EditChannelAppearanceForm channel={channel} onSubmit={this.onSubmit} onUpdate={updateChannelForm} form={channelForm.value} history={history} validation={channelForm.errors} processing={processing} /> : null}
      </React.Fragment>;
  }
}
...
const mapStateToProps = (state, ownProps) => {
  ...
  const processing = state.channels.processing || state.channelAvatar.processing || state.channelBanner.processing;
  return {
    channel,
    channelName,
    processing,
...
export default R.compose(connect(mapStateToProps, channelFormDispatchToProps), withChannelHeader, withSingleColumn("edit-channel"))(EditChannelAppearancePage);