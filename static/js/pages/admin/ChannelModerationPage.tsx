...
import { Channel, PostReportRecord, ReportRecord } from "../../flow/discussionTypes";
...
export class ChannelModerationPage extends React.Component<Props> {
  ...
  renderReports = () => {
    ...
    return reports.length === 0 ? <Card title="Reported Posts & Comments">
        <div className="empty-message">No outstanding reports</div>
      </Card> : // $FlowFixMe: flow being mad pedantic
  ...
  renderReport = (report: ReportRecord) => {
    ...
    if (report.post) {
      return <CompactPostDisplay post={report.post} isModerator={isModerator} removePost={removePost} key={report.post.id} ignorePostReports={ignorePostReports} />;
    } else {
      return <Comment comment={report.comment} commentPermalink={commentPermalink(channelName, report.comment.post_id, null)} key={`${report.comment.id}-${report.comment.post_id}`} moderationUI isModerator={isModerator} isPrivateChannel={isPrivate(channel)} channelName={channelName} shouldGetReports />;
    }
  };
  ...
  render() {
    ...
    return isModerator ? <React.Fragment>
        <MetaTags>
      ...
      </React.Fragment> : <Redirect to={channelURL(channel.name)} />;
  }
}
...
export default R.compose(connect(mapStateToProps), withPostModeration, withChannelHeader, withSingleColumn("edit-channel"))(ChannelModerationPage);