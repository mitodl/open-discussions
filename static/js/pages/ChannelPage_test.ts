...
describe("ChannelPage", () => {
  let helper, render, channels, currentChannel, otherChannel, postList, postIds, ensureTwitterStub;
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(ChannelPage, InnerChannelPage, {
      posts: {
  ...
  describe("integration", () => {
    ...
    const renderPage = async channel => {
      const [wrapper] = await renderComponent(channelURL(channel.name), [actions.profiles.get.requestType, actions.profiles.get.successType, actions.channels.get.requestType, actions.channels.get.successType, actions.postsForChannel.get.requestType, actions.postsForChannel.get.successType, actions.subscribedChannels.get.requestType, actions.subscribedChannels.get.successType, actions.widgets.get.requestType, actions.widgets.get.successType, SET_POST_DATA, SET_CHANNEL_DATA]);
    ...
    it("updates requirements when channel name changes", async () => {
      ...
      await listenForActions([actions.channels.get.requestType, actions.channels.get.successType, actions.postsForChannel.get.requestType, actions.postsForChannel.get.successType, actions.widgets.get.requestType, actions.widgets.get.successType, EVICT_POSTS_FOR_CHANNEL, FORM_END_EDIT // this is because of WidgetListContainer
      ], () => {
  ...
  it("should handle missing data gracefully", async () => {
    const {
      ...
      channelName: "somenamethatshouldnevercollidebecauseitsaridiculouslylongvalue"
    });
  ...
  ;[true, false].forEach(loaded => {
    it(`sets canLoadMore=${String(loaded)} when loaded=${String(loaded)}`, async () => {
      const {
        inner
      } = await render({
        postsForChannel: {
  ...
  it("defines loadMore to fetch from postsForChannel", async () => {
    ...
    sinon.assert.calledWith(helper.getPostsForChannelStub, currentChannel.name, search);
