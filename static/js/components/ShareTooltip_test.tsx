...
describe("ShareTooltip", () => {
  ...
  const renderShareTooltipHelper = (props = {}) => mount(<ShareTooltipHelper url={url} setSnackbarMessage={setSnackbarMessageStub} {...props} />);
  ...
  it("should select text in the input field", () => {
    ...
    assert.ok(setSnackbarMessageStub.calledWith({ message: "Copied to clipboard" }));
  });

  it("should include the share buttons we expect", () => {
    const wrapper = renderShareTooltipHelper();[[FacebookShareButton, FacebookIcon], [LinkedinShareButton, LinkedinIcon], [TwitterShareButton, TwitterIcon]].forEach(([button, icon]) => {
  ...
  it("should have a little flavor text ", () => {
    const wrapper = renderShareTooltipHelper();
    assert.equal(wrapper.find(".tooltip-text").text(), "Share a link to this post:");
  });
  ...
  it("should accept an optional object type prop", () => {
    ...
    assert.equal(wrapper.find(".tooltip-text").text(), "Share a link to this thingamajig:");
  });

  it("should hide buttons, if hideSocialButtons === true", () => {
    const wrapper = renderShareTooltipHelper({ hideSocialButtons: true });[FacebookShareButton, LinkedinShareButton, TwitterShareButton].forEach(button => assert.isNotOk(wrapper.find(button).exists()));
  });
});