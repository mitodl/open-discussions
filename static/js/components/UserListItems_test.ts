...
import { UserListItemSortableCards, PaginatedUserListItems } from "./UserListItems";
import { SortableContainer } from "./SortableList";
import { userListItemsApiURL, userListItemsDetailApiURL } from "../lib/url";
import IntegrationTestHelper from "../util/integration_test_helper";
import { makeUserListItems, makeUserList } from "../factories/learning_resources";
...
describe("UserListItems", () => {
  ...
  describe("UserListItemSortableCards", () => {
    ...
    beforeEach(() => {
      ...
      helper.handleRequestStub.withArgs(userListItemsApiURL.param({ userListId }).toString()).returns({
        status: 200,
        body: {
          count: items.length,
          results: items,
    ...
    it("should let the user reorder their learning path", async () => {
      ...
      assert.equal(url, userListItemsDetailApiURL.param({ userListId, itemId: items[1].id }).toString());
  ...
  describe("PaginatedUserListItems", () => {
    ...
    beforeEach(() => {
      ...
      helper.handleRequestStub.withArgs(userListItemsApiURL.param({ userListId: userList.id }).toString()) // page 1
      .onFirstCall().returns({
        status: 200,
        body: {
          count: pageSize * 2,
