...
import { createUserListMutation, userListMutation } from "../lib/queries/user_lists";
import { LR_TYPE_USERLIST, LR_TYPE_LEARNINGPATH, LR_PUBLIC, LR_PRIVATE } from "../lib/constants";
import { validateCreateUserListForm, validationMessage } from "../lib/validation";
...
export default function UserListFormDialog(props: Props) {
  ...
  return isFinished ? <Formik onSubmit={async params => {
    if (userList) {
    ...
    topics: sortBy("name")(userList.topics.map(topic => topic.id) || [])
  } : {
    title: "",
    short_description: "",
    topics: [],
    privacy_level: SETTINGS.is_list_staff ? null : LR_PRIVATE
  }} validate={validateCreateUserListForm} validateOnBlur={false} validateOnChange={false}>
      {({
      handleSubmit,
      errors
    }) => <Dialog title={userList ? `Edit ${userList.title}` : "Create new list"} open={true} hideDialog={hide} onAccept={handleSubmit} className="create-user-list-dialog" submitText="Save">
          <Form className="user-list-create-form">
            <span className="input-name">List Type</span>
            <div className="type radio">
              <div className="option">
                <Field name="list_type" id="type-list" type="radio" value={LR_TYPE_USERLIST} tabIndex="0" />
              ...
              <div className="option">
                <Field name="list_type" type="radio" id="type-lp" value={LR_TYPE_LEARNINGPATH} tabIndex="0" />
            ...
            {SETTINGS.is_list_staff ? <>
                ...
                <div className="privacy radio">
                  <div className="option">
                    <Field name="privacy_level" id="radio-public" type="radio" value={LR_PUBLIC} tabIndex="0" />
                    <label htmlFor="radio-public">
                      <span className="header">Public</span>
                    </label>
                  </div>
                  <div className="option">
                    <Field name="privacy_level" type="radio" id="radio-private" value={LR_PRIVATE} tabIndex="0" />
            ...
            <Field name="topics" className="basic-multi-select topics-select" options={sortBy("label")(topics.map(topic => ({ value: topic.id, label: topic.name })))} component={Select} placeholder="Select 1 to 3 subjects...." isMulti={true} closeMenuOnSelect={false} openMenuOnClick={false} menuPlacement="top" />
