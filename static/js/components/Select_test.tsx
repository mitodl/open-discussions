...
describe("Select", function () {
  let sandbox, options, isMulti, closeMenuOnSelect, openMenuOnClick, placeholder, menuPlacement, className, field, form, setFieldValueStub;
  ...
  const render = (props = {}) => {
    return mount(<Select options={options} closeMenuOnSelect={closeMenuOnSelect} openMenuOnClick={openMenuOnClick} isMulti={isMulti} placeholder={placeholder} menuPlacement={menuPlacement} className={className} field={field} form={form} {...props} />);
  };
  ...
  ;[[true, null, [null]], [true, 23, [23]], [false, null, null], [false, 24, 24]].forEach(([isMulti, value, expected]) => {
    [true, false].forEach(hasOptions => {
      it(`sets field value to ${String(expected)} when isMulti=${isMulti.toString()} or null/[] when option is null`, () => {
        field = {
          name: "TestName",
        ...
        if (hasOptions) {
          ...
          sinon.assert.calledWith(setFieldValueStub, field.name, isMulti ? [] : null);
        }
      });
    });
  });
