...
describe("NewCoursesWidget", () => {
  ...
  it("should render information for all courses returns", async () => {
    ...
    R.zip(wrapper.find(".course").map(R.identity), courses).forEach(([el, course]) => {
      assert.equal(el.find("Dotdotdot").prop("children"), course.title);
      const availabilityAndPlatform = el.find(".availability-and-platform").text();
      assert.include(availabilityAndPlatform, availabilityLabel(course.runs[0].availability));
