...
describe("PaginatedPodcastEpisodes", () => {
  ...
  beforeEach(() => {
    ...
    helper.handleRequestStub.withArgs(podcastDetailEpisodesApiURL.param({ podcastId: podcast.id }).toString()) // page 1
    .onFirstCall().returns({
      status: 200,
      body: {
        count: 16,
  ...
  it("should disable next button when one full page is present", async () => {
    helper.handleRequestStub.withArgs(podcastDetailEpisodesApiURL.param({ podcastId: podcast.id }).toString()).onFirstCall().returns({
      status: 200,
      body: {
        count: 10,
        results: episodes1,
