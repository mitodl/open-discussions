...
describe("PodcastPlayButton", () => {
  ...
  it("should show black play button if not playing, not initialized", async () => {
    ...
    assert.equal(wrapper.find(podcastPlayButton).prop("className"), "podcast-play-button black-surround");
  });
  ...
  it("should show grey pause button when initialized, playing", async () => {
    ...
    assert.equal(wrapper.find(podcastPlayButton).prop("className"), "podcast-play-button grey-surround");
  });
