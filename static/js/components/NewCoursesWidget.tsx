...
import { newCoursesRequest, newCoursesSelector, upcomingCoursesRequest, upcomingCoursesSelector } from "../lib/queries/courses";
import { availabilityLabel } from "../lib/learning_resources";
import { embedlyThumbnail, COURSE_URL, defaultResourceImageURL } from "../lib/url";
...
export default function NewCoursesWidget() {
  ...
  const courses = loaded && newCourses && upcomingCourses ? flatZip(newCourses.slice(0, 5), upcomingCourses.slice(0, 5)) : [];

  return loaded ? <Card className="new-course-widget widget">
      <div className="title-row">
        <span className="title">Learn at MIT</span>
      </div>
      {courses.map((course, idx) => <div className="course" key={idx}>
          <img src={embedlyThumbnail(SETTINGS.embedlyKey, course.image_src || defaultResourceImageURL(), THUMBNAIL_SIZE, THUMBNAIL_SIZE)} height={THUMBNAIL_SIZE} />
