...
describe("CoverImageInput", () => {
  let renderInput, onUpdateStub, setPhotoErrorStub, imageFile, sandbox, hideCoverImageInputStub;
  ...
  it("should have an onDrop function wired up right on the dropzone", () => {
    ...
    onDrop([imageFile]);
    const event = R.omit(["preventDefault"], makeEvent("cover_image", imageFile));
  ...
  it("should render an image if the prop is provided", () => {
    sandbox.stub(URL, "createObjectURL").returns("image!");[(imageFile, "http://example.com/foo.jpg")].forEach(image => {
  ...
  it("should show a button to clear input if an image is present", () => {
    ...
    btn.props().onClick();
    sinon.assert.calledWith(onUpdateStub, R.omit(["preventDefault"], makeEvent("cover_image", undefined)));
  });
