...
describe("ChannelAvatar", () => {
  ...
  beforeEach(() => {
    ...
    renderPage = helper.configureHOCRenderer(ChannelAvatar, "ChannelAvatar", DEFAULT_STATE);
    channel = makeChannel();
    channel.avatar = null;
  ...
  afterEach(() => {
    ...
    describe(imageSize, () => {
      it("renders an image when there is an avatar", async () => {
        ...
        const {
          inner
        } = await renderPage({}, { channel, imageSize });

        assert.equal(inner.find("img").props().alt, `Channel avatar for ${channel.name}`);
        assert.equal(inner.find("img").props().src, channel[`avatar_${imageSize}`]);
      ...
      ;[true, false].forEach(editable => {
        it(`${editable ? "renders" : "doesn't render"} an ImageUploader`, async () => {
          const {
            inner
          } = await renderPage({}, { channel, editable, imageSize });
          assert.equal(inner.find("Connect(withForm(ImageUploader))").length, editable ? 1 : 0);
        });
      });
      ...
      describe(`shows the right icon when the channel`, () => {
        [true, false].forEach(hasAvatar => {
          it(`${hasAvatar ? "has" : "doesn't have"} a avatar`, async () => {
            ...
            const {
              inner
            } = await renderPage({}, { channel, editable: true, imageSize });
          ...
          it(`${hasFormImageUrl ? "provides" : "doesn't provide"} a formImageUrl`, async () => {
            const {
              inner
            } = await renderPage({}, {
              channel,
        ...
        it(`passes in ${field}`, async () => {
          ...
          const {
            inner
          } = await renderPage({}, { channel, [field]: value, editable: true, imageSize });
          const imageUploader = inner.find("Connect(withForm(ImageUploader))");
      ...
      it("has a link to upload avatar which shows the dialog", async () => {
        ...
        assert.isTrue(inner.find("Connect(withForm(ImageUploader))").props().showButton);
      });
    });
  });
});