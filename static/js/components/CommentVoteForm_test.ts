...
describe("CommentVoteForm", () => {
  ...
  beforeEach(async () => {
    ...
    helper.updateCommentStub.callsFake(() => new Promise((resolve, reject) => {
  ...
  const assertButtons = (wrapper, isUpvote, isClear, isVoting) => {
    const clickedButtonSelector = isUpvote ? ".upvote-button" : ".downvote-button";
    ...
    if (isClear) {
      assert.notInclude(wrapper.find(clickedButtonSelector).props().className, clickedVoteClass);
      assert.equal(wrapper.find(clickedButtonSelector).find("img").props().src, `${imgPrefix}.png`);
    } else {
      assert.include(wrapper.find(clickedButtonSelector).props().className, clickedVoteClass);
      assert.equal(wrapper.find(clickedButtonSelector).find("img").props().src, `${imgPrefix}_on.png`);
    }

    assert.notInclude(wrapper.find(otherButtonSelector).props().className, otherVoteClass);
  };
  ...
  ;[true, false].forEach(isUpvote => {
    describe(`clicks the ${isUpvote ? "upvote" : "downvote"} button`, () => {
      it("gracefully handles a failed request", async () => {
        ...
        wrapper.find(isUpvote ? ".upvote-button" : ".downvote-button").simulate("click");
        await rejectRequest();
        await wait(1);
        assert.deepEqual(store.getState().ui.banner.message, `Something went wrong ${isUpvote ? "upvoting" : "downvoting"} this comment. Contact us at ${SETTINGS.support_email}`);
      });
      ...
      it("when the previous state was clear", async () => {
        ...
        assert.equal(wrapper.find(".score").text(), isUpvote ? `${comment.score + 1}` : `${comment.score - 1}`);
      ...
      ;[true, false].forEach(wasUpvote => {
        it(`when the previous state was ${wasUpvote ? "upvote" : "downvote"}`, async () => {
          if (wasUpvote) {
            comment.upvoted = true;
          } else {
            comment.downvoted = true;
          ...
          // should pre-emptively adjust the score so user's action looks instant
          assert.equal(wrapper.find(".score").text(), // $FlowFixMe: it's a test, leave me alone
