...
const editorClassName = (readOnly?: boolean): string => `ck-editor ${readOnly ? "read-only" : ""}`.trim();
...
export default class ArticleEditor extends React.Component<Props> {
  ...
  initializeEditor = async (node: any) => {
    ...
    try {
      ...
      if (!readOnly) {
        editor.model.document.on("change:data", // editor.getData() is kind of expensive so we debounce
        _.debounce(() => {
          if (onChange) {
  ...
  render() {
    ...
    return <div className={editorClassName(readOnly)} ref={this.initializeEditor} />;
  }
}