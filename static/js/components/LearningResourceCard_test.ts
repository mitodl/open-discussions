...
import { LearningResourceCard, LearningResourceRow } from "./LearningResourceCard";
import Card from "./Card";

import IntegrationTestHelper from "../util/integration_test_helper";

import { bestRun } from "../lib/learning_resources";
import { makeCourse, makeLearningResource, makeUserList } from "../factories/learning_resources";
import { CAROUSEL_IMG_WIDTH, CAROUSEL_IMG_HEIGHT, LR_TYPE_ALL, LR_TYPE_COURSE, OPEN_CONTENT, PROFESSIONAL, CERTIFICATE, DISPLAY_DATE_FORMAT } from "../lib/constants";
import { COURSE_SEARCH_URL, userListApiURL, embedlyThumbnail, toQueryString } from "../lib/url";
...
describe("LearningResourceCard", () => {
  ...
  beforeEach(() => {
    ...
    helper.handleRequestStub.withArgs(userListApiURL.toString()).returns(queryListResponse([userList]));
  ...
  it("should set an onClick handler with the setShowResourceDrawer function", async () => {
    ...
    const {
      objectId,
      objectType
    } = store.getState().ui.LRDrawerHistory[0];
  ...
  it("should render the image", async () => {
    ...
    assert.equal(coverImage.prop("src"), embedlyThumbnail(SETTINGS.embedlyKey, course.image_src, CAROUSEL_IMG_HEIGHT, CAROUSEL_IMG_WIDTH));
  ...
  it("should render topics as links", async () => {
    ...
    course.topics.forEach(({
      ...
      assert.equal(link.prop("href"), `${COURSE_SEARCH_URL}${toQueryString({
        t: name
      })}`);
    });
  ...
  it("should render a single topic", async () => {
    ...
    const {
      label
    } = wrapper.find("Subtitle").at(1).props();
  ...
  ;[true, false].forEach(isFavorite => {
    LR_TYPE_ALL.forEach(objectType => {
      it(`should render ${isFavorite ? "filled-in" : "empty"} bookmark icon when ${objectType} is ${isFavorite ? "a" : "not a"} favorite`, async () => {
        const object = makeLearningResource(objectType);
        object.is_favorite = isFavorite;
        const {
          wrapper
        } = await render({ object });
        assert.include(wrapper.find(".favorite i").prop("className"), isFavorite ? "bookmark" : "bookmark_border");
      });
    });
  }) //
  ;[true, false].forEach(inList => {
    it(`should render ${inList ? "filled-in" : "empty"} star when course is ${inList ? "" : "not"} in the user's lists`, async () => {
      ...
      assert.include(wrapper.find(".favorite i").prop("className"), inList ? "bookmark" : "bookmark_border");
    });
  });
  ...
  LR_TYPE_ALL.forEach(objectType => {
    it(`should call showListDialog with a ${objectType}`, async () => {
      ...
      assert.deepEqual(store.getState().ui.dialogs.get(DIALOG_ADD_TO_LIST), object);
    });
  });
  ...
  it("should render a LearningResourceRow", async () => {
    ...
    assert.deepEqual(wrapper.find("LearningResourceDisplay").prop("object"), course);
    assert.isNotOk(wrapper.find(Card).exists());
  }) //
  ;[true, false].forEach(reordering => {
    it(`should set a classname and show UI when reordering=${String(reordering)}`, async () => {
      const {
        wrapper
      } = await render({ reordering });
      assert.equal(wrapper.find(Card).prop("className"), reordering ? "learning-resource-card  reordering" : "learning-resource-card");
  ...
  it(`should render a start date if there is a certificate`, async () => {
    ...
    const startDate = wrapper.find(".start-date").text().replace("calendar_today", "");
    assert.equal(startDate, moment(bestRun(object.runs).best_start_date).format(DISPLAY_DATE_FORMAT));
  });
  ...
  ;[[], [CERTIFICATE]].forEach(certification => {
    [[], [OPEN_CONTENT], [PROFESSIONAL], [OPEN_CONTENT, PROFESSIONAL]].forEach(audience => {
      it(`should render the corret icons for certificate=[${certification}] and audience=[${audience}]`, async () => {
        ...
        const expectedIcons = audience.concat(certification).map(attribute => iconMap[attribute]);
        const displayedIcons = wrapper.find(".audience-certificates").find("img").map(image => image.prop("src"));
