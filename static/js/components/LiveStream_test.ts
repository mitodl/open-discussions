...
describe("LiveStream", () => {
  ...
  ;[true, false].forEach(livestreamUIEnabled => {
    it(`${shouldIf(livestreamUIEnabled)} call the get method on load if feature flag is ${String(livestreamUIEnabled)}`, async () => {
  ...
  it("should render an iframe if there is a live event", async () => {
    const urlStub = helper.sandbox.stub(embedLib, "livestreamEventURL").returns("http://foo.bar.baz");
