...
describe("ChannelBanner", () => {
  ...
  beforeEach(() => {
    ...
    renderPage = helper.configureHOCRenderer(ChannelBanner, "ChannelBanner", DEFAULT_STATE);
    channel = makeChannel();
    channel.banner = null;
  });
  ...
  ;[true, false].forEach(editable => {
    it(`${editable ? "renders" : "doesn't render"} an ImageUploader and link`, async () => {
      const {
        inner
      } = await renderPage({}, { channel, editable });
  ...
  describe(`shows the right icon when the channel`, () => {
    [true, false].forEach(hasBanner => {
      it(`${hasBanner ? "has" : "doesn't have"} a banner`, async () => {
        ...
        const {
          inner
        } = await renderPage({}, { channel, editable: true });
  ...
  ;["name", "onUpdate"].forEach(field => {
    it(`passes in ${field}`, async () => {
      ...
      const {
        inner
      } = await renderPage({}, { channel, [field]: value, editable: true });
      const imageUploader = inner.find("Connect(withForm(ImageUploader))");
  ...
  it("has a link to upload banner which shows the dialog", async () => {
    const showDialogStub = helper.sandbox.stub(uiActions, "showDialog").returns({ type: "action" });
    const {
      inner
    } = await renderPage({}, { channel, editable: true });
