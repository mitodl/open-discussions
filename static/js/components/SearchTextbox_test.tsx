...
describe("SearchTextbox", () => {
  ...
  const render = (props = {}) => shallow(<SearchTextbox onChange={onChangeStub} onSubmit={onSubmitStub} onClear={onClearStub} value={""} validation={null} {...props} />);

  const renderMount = (props = {}) => mount(<SearchTextbox onChange={onChangeStub} onSubmit={onSubmitStub} onClear={onClearStub} value={""} validation={null} {...props} />);
  ...
  it("triggers onSubmit when the spyglass is clicked", () => {
    ...
    it(`if there ${hasText ? "is text" : "isn't text"} then there should ${hasText ? "" : "not "}be a clear button`, () => {
