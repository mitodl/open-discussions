...
describe("Banner", () => {
  ...
  ;[[false, false], [true, true]].forEach(([visible, expActive]) => {
    it(`${shouldIf(expActive)} set an active class on the banner when visible=${String(visible)}`, () => {
      banner.visible = visible;
      const wrapper = renderBanner();
      assert.equal(_.includes(wrapper.find(".banner").prop("className"), "banner--active"), expActive);
    });
  });
});