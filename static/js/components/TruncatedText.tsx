...
const RenderedNewlines = React.memo(({
  text
}) => text.split(/\n/g).map((item, key) => <React.Fragment key={key}>
      {key === 0 ? null : <br />}
      {item}
    </React.Fragment>));
...
export default function TruncatedText(props: Props) {
  ...
  useLayoutEffect(() => {
    ...
    if (dotRef.current) {
      ...
      if (container.scrollHeight === 0 && container.offsetHeight === 0 && text.trim() !== "") {
  ...
  return <React.Fragment>
      {expanded ? <div>
        ...
        </div> : <Dotdotdot clamp={lines} ref={dotRef} className={className}>
          {
        /* Dotdotdot seems to trip on long, unbroken strings. As a fail-safe, we're limiting
          ...
          <RenderedNewlines text={text.substring(0, estCharsPerLine * (lines + 2))} />
        </Dotdotdot>}
      {showExpansionControls && hasOverflow ? <span className="tt-expansion-control" onClick={() => setExpanded(!expanded)}>
