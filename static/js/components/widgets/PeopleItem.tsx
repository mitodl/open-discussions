...
export const PeopleItem: StatelessFunctionalComponent<Props> = ({
  ...
  profile
    ...
    {editing ? <div className="edit-buttons">
        <i className="material-icons delete widget-button" // $FlowFixMe: if editing is true deleteProfile should always be defined
    onClick={() => deleteProfile(profile)}>
          delete
        </i>
        <DragHandle />
      </div> : null}
    {addProfile ? <button className="add-profile" onClick={() => addProfile(profile)}>
        Add
      </button> : null}
  </div>;

const SortablePeopleItem: StatelessFunctionalComponent<Props> = SortableElement(PeopleItem);
