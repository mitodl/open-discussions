...
describe("WidgetInstance", () => {
  let widgetInstance, helper, deleteInstanceStub, startEditInstanceStub, toggleExpandedStub;
  ...
  const WrappedInstance = SortableContainer(props => <WidgetInstance {...props} />);

  const render = (props = {}) => mount(<Router store={helper.store} history={helper.browserHistory}>
        <WrappedInstance widgetInstance={widgetInstance} deleteInstance={deleteInstanceStub} index={3} editing={false} startEditInstance={startEditInstanceStub} expanded={true} toggleExpanded={toggleExpandedStub} {...props} />
      </Router>, {
  ...
  it("renders the widget title", () => {
  ...
  });[["URL", "UrlWidget"], ["RSS Feed", "RssWidget"], ["Markdown", "MarkdownWidget"]].forEach(([widgetType, widgetClassName]) => {
    it(`renders a widget instance for ${widgetClassName} given widget_type=${widgetType}`, () => {
      ...
      assert.equal(wrapper.find(widgetClassName).prop("widgetInstance"), instance);
    });
  });[true, false].forEach(expanded => {
    it(`${shouldIf(expanded)} render the widget instance if it is ${expanded ? "" : "not "}expanded`, () => {
  ...
  });[[true, true, "keyboard_arrow_up"], [true, false, "keyboard_arrow_down"], [false, true, null], [false, false, null]].forEach(([editing, expanded, expected]) => {
    it(`${shouldIf(!!expected)} render the proper arrow for expanded=${String(expanded)} and editing=${String(editing)}`, () => {
