...
describe("PeopleItem", () => {
  ...
  const WrappedPeopleItem = SortableContainer(props => <PeopleItem {...props} />);

  const render = (props = {}) => mount(<Router store={helper.store} history={helper.browserHistory}>
        <WrappedPeopleItem profile={profile} deleteProfile={deleteProfileStub} index={3} editing={false} addProfile={addProfileStub} {...props} />
      </Router>, {
  ...
  it("renders a PeopleItem", () => {
    ...
    assert.equal(wrapper.find(".description .headline").text(), profile.headline);
  });[true, false].forEach(hasEditButtons => {
    it(`${shouldIf(hasEditButtons)} have a delete button and a drag handle`, () => {
