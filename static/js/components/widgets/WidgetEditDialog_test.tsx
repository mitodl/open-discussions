...
import WidgetEditDialog, { WIDGET_CREATE, WIDGET_EDIT, WIDGET_TYPE_SELECT } from "./WidgetEditDialog";
import * as validationFuncs from "../../lib/validation";

import { makeFieldSpec, makeWidgetInstance, makeWidgetListResponse, makeWidgetSpec, validFieldSpecTypes } from "../../factories/widgets";
import { WIDGET_TYPE_MARKDOWN, WIDGET_TYPE_RSS, WIDGET_TYPE_URL } from "../../lib/constants";
import { isIf, shouldIf } from "../../lib/test_utils";
import IntegrationTestHelper from "../../util/integration_test_helper";

describe("WidgetEditDialog", () => {
  let helper, setDialogDataStub, setDialogVisibilityStub, updateFormStub, dialogData, validationStub, specs;
  ...
  beforeEach(() => {
    ...
    validationStub = helper.sandbox.stub(validationFuncs, "validateWidgetDialog").returns({});
  });
  ...
  const render = (props = {}) => shallow(<WidgetEditDialog dialogData={dialogData} dialogOpen={true} setDialogData={setDialogDataStub} setDialogVisibility={setDialogVisibilityStub} specs={specs} updateForm={updateFormStub} {...props} />);
  ...
  it("doesn't render the dialog if there's no data", () => {
  ...
  });[[WIDGET_TYPE_SELECT, "Select widget", "Next"], [WIDGET_EDIT, "Edit widget", "Update widget"]].forEach(([state, expectedTitle, expectedSubmitText]) => {
    it(`sets the title and submit text based on isEditing=${String()} and state=${state}`, () => {
      ...
      assert.equal(wrapper.find("OurDialog").prop("submitText"), expectedSubmitText);
    });
  });
  ...
  it("uses the widget instance title for the dialog for creating a new widget", () => {
    ...
    const spec = specs.find(spec => spec.widget_type === dialogData.instance.widget_type);
    // $FlowFixMe
  ...
  describe("widget type selection", () => {
    ...
    it("renders validation for widget type", () => {
      ...
      const wrapper = render();
      assert.equal(wrapper.find(".validation-message").text(), // $FlowFixMe
      dialogData.validation.widget_type);
    });[[true, WIDGET_TYPE_SELECT, true], [false, WIDGET_TYPE_SELECT, false], [false, WIDGET_CREATE, true], [false, WIDGET_CREATE, false], [false, WIDGET_EDIT, true], [false, WIDGET_EDIT, false]].forEach(([expected, dialogState, dialogOpen]) => {
      it(`${shouldIf(expected)} shift focus from radio buttons when state=${dialogState} and dialog ${isIf(dialogOpen)} open`, async () => {
  ...
  describe("configuration", () => {
    ...
    it("renders the widget title field", () => {
    ...
    });[["widget title", WIDGET_TYPE_MARKDOWN, { title: "Missing field" }, "Missing field"], ["embed url", WIDGET_TYPE_URL, { configuration: { url: "URL is not valid" } }, "URL is not valid"], ["rss url", WIDGET_TYPE_RSS, { configuration: { url: "URL is not valid" } }, "URL is not valid"]].forEach(([description, widgetType, validation, expectedMessage]) => {
      it(`renders validation for ${description}`, () => {
        ...
        const wrapper = render();
        assert.equal(wrapper.find(".validation-message").text(), expectedMessage);
      });
    });
    ...
    describe("fields", () => {
      ...
      beforeEach(() => {
        spec = {
          ...makeWidgetSpec(),
          form_spec: validFieldSpecTypes.map(specType => makeFieldSpec(specType, specType))
        };
        specs = [spec];
        const configuration = {};
        const json = {};
      ...
      it("renders fields for a widget configuration", () => {
        ...
        assert.equal(wrapper.find(".configuration-field").length, validFieldSpecTypes.length);
        wrapper.find(".configuration-field").forEach((field, i) => {
          ...
          assert.equal(wrapper.instance().getValue, fieldWrapper.prop("getValue"));
          assert.equal(wrapper.instance().updateValues, fieldWrapper.prop("updateValues"));
        });
      });
      ...
      describe("lenses", () => {
        ...
        beforeEach(() => {
          ...
          configurationLens = R.lensPath(["configuration", fieldSpec.field_name]);
        ...
        it("gets a value", () => {
          ...
          assert.equal(dialogData.instance.configuration[fieldSpec.field_name], value);
        });
        ...
        it("sets multiple values", () => {
          ...
          wrapper.instance().updateValues([configurationLens, jsonLens], [configValue, jsonValue]);
          sinon.assert.calledWith(setDialogDataStub, {
            ...dialogData,
            instance: {
              ...dialogData.instance,
