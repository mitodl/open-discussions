...
describe("PeopleList", () => {
  ...
  const render = (props = {}) => mount(<Router store={helper.store} history={helper.browserHistory}>
        <PeopleList profiles={profiles} deleteProfile={deleteProfileStub} addProfile={addProfileStub} editing={false} {...props} />
      </Router>) //
  ;[true, false].forEach(editing => {
    ...
    ;[true, false].forEach(showAllMembersLink => {
      it(`${shouldIf(!editing && showAllMembersLink)} show a link to the members page if editing=${String(editing)} and showAllMembersLink = ${String(showAllMembersLink)}`, () => {
        const channelNameStub = helper.sandbox.stub(urlLib, "getChannelNameFromPathname").returns("HEY");
        const channelMembersURLStub = helper.sandbox.stub(urlLib, "channelMembersURL").returns("Boop");
