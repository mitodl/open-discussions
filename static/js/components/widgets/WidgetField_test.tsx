...
import { WIDGET_FIELD_TYPE_MARKDOWN, WIDGET_FIELD_TYPE_NUMBER, WIDGET_FIELD_TYPE_PEOPLE, WIDGET_FIELD_TYPE_TEXT, WIDGET_FIELD_TYPE_TEXTAREA, WIDGET_FIELD_TYPE_URL, WIDGET_FIELD_TYPE_CHECKBOX } from "../../lib/constants";
...
describe("WidgetField", () => {
  ...
  beforeEach(() => {
    ...
    const spec = response.available_widgets.filter(_spec => _spec.widget_type === instance.widget_type);
  ...
  const render = (props = {}) => shallow(<WidgetField fieldSpec={fieldSpec} getValue={getValue} updateValues={updateValues} {...props} />) //
  ;[WIDGET_FIELD_TYPE_TEXT, WIDGET_FIELD_TYPE_TEXTAREA, WIDGET_FIELD_TYPE_NUMBER, WIDGET_FIELD_TYPE_URL].forEach(fieldType => {
  ...
  ;[WIDGET_FIELD_TYPE_TEXT, WIDGET_FIELD_TYPE_TEXTAREA, WIDGET_FIELD_TYPE_URL].forEach(fieldType => {
  ...
  describe("people", () => {
    [true, false].forEach(hasJson => {
      it(`renders ${hasJson ? "with" : "without"} JSON data`, () => {
        ...
        assert.deepEqual(props.profiles, hasJson ? instance.json[fieldSpec.field_name] : []);
      });
    });
    ...
    it("updates profiles", () => {
      ...
      assert.deepEqual(instance.configuration[fieldSpec.field_name], profiles.map(profile => profile.username));
    });
  });
