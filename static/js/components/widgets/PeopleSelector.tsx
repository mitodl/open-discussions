...
export class PeopleSelector extends React.Component<Props, State> {
  ...
  renderInputComponent = (inputProps: Object) => <SearchTextbox {...inputProps} />;
  ...
  deleteProfile = (profile: Profile) => {
    ...
    const newProfiles = profiles.filter(_profile => _profile.username !== profile.username);
  ...
  renderPersonSuggestion = (profile: Profile) => <div className="person person-suggestion" onClick={() => {
  ...
  render() {
    ...
    return <React.Fragment>
        <Autosuggest suggestions={suggestions} getSuggestionValue={profile => profile.name} onSuggestionsFetchRequested={this.onSuggestionsFetchRequested} alwaysRenderSuggestions={true} renderInputComponent={this.renderInputComponent} renderSuggestion={this.renderPersonSuggestion} inputProps={{
        value: text,
        onChange: this.onChangeText,
        onClear: this.clear,
        autoComplete: "off"
      }} />
        <PeopleList profiles={profiles} editing={true} addProfile={null} deleteProfile={this.deleteProfile} onSortEnd={this.onSortEnd} useDragHandle={true} />
      </React.Fragment>;
  }
}
...
const mapStateToProps = (state, ownProps) => {
  ...
  const newSuggestions = suggestions.filter(profile => !usernames.has(profile.username));
...
const mapDispatchToProps = (dispatch: Dispatch<any>) => ({
  ...
  fetchSuggestions: async (text: string): Promise<Array<Profile>> => dispatch(actions.search.post({
    type: SEARCH_FILTER_PROFILE,
    text
  }))
});

export default R.compose(connect(mapStateToProps, mapDispatchToProps))(PeopleSelector);