...
import WidgetEditDialog, { WIDGET_EDIT, WIDGET_TYPE_SELECT } from "../../components/widgets/WidgetEditDialog";

import { actions } from "../../actions";
import { DIALOG_EDIT_WIDGET, hideDialog, setDialogData, showDialog } from "../../actions/ui";
import { WIDGET_FORM_KEY, newWidgetInstance, getWidgetKey } from "../../lib/widgets";

import { Dispatch } from "redux";
import { WidgetInstance as WidgetInstanceType, WidgetListResponse, WidgetDialogData, WidgetForm, WidgetSpec } from "../../flow/widgetTypes";
import { FormValue } from "../../flow/formTypes";

type Props = {
  clearForm: () => void;
  dialogData: WidgetDialogData | null | undefined;
  dialogOpen: boolean;
...
export class WidgetListContainer extends React.Component<Props> {
  ...
  deleteInstance = (widgetInstance: WidgetInstanceType) => {
    ...
    const widgetInstances = this.getWidgetInstances().filter(_widgetInstance => getWidgetKey(_widgetInstance) !== key);
  ...
  updateForm = (data: WidgetDialogData) => {
    ...
    const replacementInstances = data.state === WIDGET_EDIT ? instances.map(instance => getWidgetKey(instance) === key ? instanceFromDialog : instance) : instances.concat([instanceFromDialog]);
  ...
  render() {
    ...
    return <React.Fragment>
        <WidgetList expanded={this.getExpanded()} editing={!!form} widgetInstances={this.getWidgetInstances()} onSortEnd={this.onSortEnd} clearForm={clearForm} deleteInstance={this.deleteInstance} setExpanded={updateExpanded} startAddInstance={this.startAddInstance} startEditInstance={this.startEditInstance} useDragHandle={true} />
        <WidgetEditDialog updateForm={this.updateForm} dialogData={dialogData} dialogOpen={dialogOpen} setDialogData={setDialogData} setDialogVisibility={setDialogVisibility} specs={specs} validation={validation} />
      </React.Fragment>;
  }
}
...
const mapDispatchToProps = (dispatch: Dispatch<any>, ownProps) => bindActionCreators({
  clearForm: () => actions.forms.formEndEdit({ formKey: WIDGET_FORM_KEY }),
  setDialogData: (data: WidgetDialogData) => setDialogData({ dialogKey: DIALOG_EDIT_WIDGET, data: data }),
  setDialogVisibility: (visibility: boolean) => visibility ? showDialog(DIALOG_EDIT_WIDGET) : hideDialog(DIALOG_EDIT_WIDGET),
  ...
  updateWidgetInstances: (instances: Array<WidgetInstanceType>) => actions.forms.formUpdate({
    formKey: WIDGET_FORM_KEY,
    value: { instances }
  })
}, dispatch);

export default R.compose(connect(mapStateToProps, mapDispatchToProps))(WidgetListContainer);