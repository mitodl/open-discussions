...
describe("UrlWidget", () => {
  ...
  it("renders a UrlWidget", () => {
    ...
    assert.equal(wrapper.find("EmbedlyCard").prop("url"), widgetInstance.configuration.url);
  });
  ...
  it("renders using the TwitterEmbed, if that's what it is given", () => {
    ...
    const twitterEmbed = shallow(<UrlWidget widgetInstance={widgetInstance} />).find("TwitterEmbed");
  ...
  it("twitter embed calls twitter platform after loading", () => {
    ...
    const wrapper = mount(<TwitterEmbed embed="<div class='asdf'>fooooo</div>" />);
