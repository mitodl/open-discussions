...
describe("WidgetList", () => {
  let listResponse, expanded, helper, clearFormStub, submitFormStub, deleteInstanceStub, startEditInstanceStub, startAddInstanceStub, setExpandedStub;
  ...
  beforeEach(() => {
    ...
    helper.getEmbedlyStub.returns(Promise.resolve({
      response: makeTweet()
    }));
  });
  ...
  const render = (props = {}) => mount(<Router store={helper.store} history={helper.browserHistory}>
        <WidgetList widgetInstances={listResponse.widgets} clearForm={clearFormStub} submitForm={submitFormStub} editing={false} expanded={expanded} deleteInstance={deleteInstanceStub} startEditInstance={startEditInstanceStub} startAddInstance={startAddInstanceStub} setExpanded={setExpandedStub} {...props} />
      </Router>);

  it("renders a list of WidgetInstances", () => {
    const wrapper = render({ editing: true });
    assert.equal(wrapper.find(WidgetInstance).length, listResponse.widgets.length);
  ...
  ;[[true, true, true], [true, false, false], [false, true, true], [false, false, true]].forEach(([editing, isExpanded, expected]) => {
    it(`has expanded=${String(expected)} when editing=${String(editing)} and expanded[key]=${String(isExpanded)}`, () => {
      expanded = {};
      ...
      listResponse.widgets.forEach((widgetInstance, i) => {
        assert.equal(wrapper.find(WidgetInstance).at(i).prop("expanded"), expected);
      });
    });
  });
  ...
  ;[true, false].forEach(editing => {
    it(`${shouldIf(editing)} show the manage widgets links when editing`, () => {
  ...
  describe("editing", () => {
    ...
    ;[true, false].forEach(allExpanded => {
      it(`has a link to ${allExpanded ? "expand" : "collapse"} widgets`, () => {
        ...
        const link = render({ editing: true, expanded }).find(".toggle-collapse-all");
