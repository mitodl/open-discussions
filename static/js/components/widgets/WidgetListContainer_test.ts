...
import WidgetListContainer, { WidgetListContainer as InnerWidgetListContainer } from "./WidgetListContainer";
...
import { makeWidgetInstance, makeWidgetListResponse } from "../../factories/widgets";
...
import { DIALOG_EDIT_WIDGET, HIDE_DIALOG, SET_DIALOG_DATA, SHOW_DIALOG } from "../../actions/ui";
import { shouldIf } from "../../lib/test_utils";

import { WidgetDialogData } from "../../flow/widgetTypes";

describe("WidgetListContainer", () => {
  let helper, listResponse, render, initialState, initialProps, updatedWidgetList, updatedExpanded;
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(WidgetListContainer, InnerWidgetListContainer, initialState, initialProps);
  });
  ...
  it("clears the form on unmount", async () => {
    ...
    assert.isTrue(store.getActions().filter(action => action.type === FORM_END_EDIT).length >= 1);
  });
  ...
  describe("WidgetList", () => {
    it("returns loader if no widget instance is loaded yet", async () => {
      const {
        inner
      } = await render({ widgets: { loaded: false } });
      assert.isTrue(inner.find("Loading").exists());
      sinon.assert.calledWith(helper.getWidgetListStub, listResponse.id);
    }) //
    ;[["is present", { value: { instances: true } }, true], ["is missing the instances", { value: { instances: null } }, false], ["is missing the value", { value: null }, false], ["doesn't exist", null, false]].forEach(([description, form, expected]) => {
      it(`${shouldIf(expected)} use the widget instances in a form when the form ${description}`, async () => {
        ...
        assert.deepEqual(props.widgetInstances, expected ? updatedWidgetList : listResponse.widgets);
    ...
    it("reorders widgets", async () => {
      ...
      const oldIndex = 1;
      const newIndex = 0;
      wrapper.find("sortableList").at(0).prop("onSortEnd")({ oldIndex, newIndex });
    ...
    it("opens a dialog to add a new widget", async () => {
      ...
      const newWidgetInstanceStub = helper.sandbox.stub(widgetFuncs, "newWidgetInstance").returns(aNewWidgetInstance);
      const {
        wrapper,
        store
      } = await render();
    ...
    describe("expand/collapse", () => {
      [["is present", { value: { expanded: true } }, true], ["is missing expanded", { value: { expanded: null } }, false], ["is missing the value", { value: null }, false], ["doesn't exist", null, false]].forEach(([description, form, expected]) => {
        it(`${shouldIf(expected)} use expanded from the form when the form ${description}`, async () => {
  ...
  describe("WidgetEditDialog", () => {
    describe("updateForm", () => {
      it("updates the widget list with an updated widget instance", async () => {
        ...
        const list = updatedWidgetList.map(item => item.id === updateId ? replaceInstance : item);
    ...
    ;[true, false].forEach(newVisibility => {
      it(`sets dialog visibility=${String(newVisibility)}`, async () => {
        ...
        wrapper.find("WidgetEditDialog").prop("setDialogVisibility")(newVisibility);
        const actions = store.getActions();
        assert.deepEqual(actions[actions.length - 1], {
    ...
    ;[true, false].forEach(isOpen => {
      it(`passes props when the dialog is ${isOpen ? "" : "not "}open`, async () => {
