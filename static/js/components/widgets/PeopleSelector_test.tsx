...
import PeopleSelector, { PeopleSelector as InnerPeopleSelector } from "./PeopleSelector";
...
describe("PeopleSelector", () => {
  let helper, profiles, suggestionsResponse, suggestions, render, initialState, initialProps, updateProfilesStub;

  beforeEach(() => {
    helper = new IntegrationTestHelper();
    suggestionsResponse = makeSearchResponse(10, 15, "profile");
    suggestions = suggestionsResponse.hits.hits.map(item => item._source).map((profile, i) => ({
      ...profile,
      author_id: `user ${i}`
    }));
    ...
    render = helper.configureHOCRenderer(PeopleSelector, InnerPeopleSelector, initialState, initialProps);
  });
  ...
  it("clears the search on mount", async () => {
    ...
    assert.deepEqual(store.getActions(), [{ type: actions.search.clear.toString() }]);
  });
  ...
  describe("Autosuggest", () => {
    it("has suggestion props", async () => {
      ...
      assert.deepEqual(props.suggestions, suggestions.map(searchResultToProfile));
    ...
    it("renders the input component", async () => {
      ...
      assert.deepEqual(shallow(component).find("input").prop(propKey), propValue);
    });
    ...
    it("renders a suggestion and adds a profile", async () => {
      ...
      const component = inner.find("Autosuggest").prop("renderSuggestion")(profile);

      const props = shallow(<div>{component}</div>).find(".person-suggestion").props();
      props.onClick(profile);
      sinon.assert.calledWith(updateProfilesStub, [...profiles, profile]);

      const actionsList = store.getActions();
      assert.equal(actionsList[actionsList.length - 1].type, actions.search.clear.toString());
  ...
  describe("PeopleList", () => {
    ...
    it("deletes", async () => {
      ...
      const updatedProfiles = profiles.filter(_profile => _profile.username !== profile.username);
  ...
  ;[true, false].forEach(hasDuplicate => {
    it(`${shouldIf(hasDuplicate)} filters out duplicate suggestions from state`, async () => {
      let expected = suggestions.map(searchResultToProfile);

      if (hasDuplicate) {
        profiles[0].username = suggestions[0].author_id;
        expected = expected.filter(profile => profile.username !== profiles[0].username);
      }
      const {
        inner
      } = await render();
