...
import { WidgetInstance as WidgetInstanceType } from "../../flow/widgetTypes";
...
const SortableWidgetList: StatelessFunctionalComponent<Props> = SortableContainer(({
  ...
  return <div className="widget-list">
        {editing ? <div className="manage-widgets">
            <span className="add-widget" onClick={() => startAddInstance()}>
              + Add widget
            </span>
            <span className="toggle-collapse-all" onClick={() => setExpanded(widgetKeys, anyCollapsed)}>
      ...
      return <WidgetInstance startEditInstance={startEditInstance} widgetInstance={widgetInstance} key={key} index={i} editing={editing} deleteInstance={deleteInstance} expanded={!editing || instanceIsExpanded} toggleExpanded={() => setExpanded([key], !instanceIsExpanded)} />;
    })}
      </div>;
});
