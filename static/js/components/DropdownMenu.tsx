...
export class _DropdownMenu extends React.Component<Props> {
  ...
  setClickHandlersOnChildren = () => {
    ...
    return React.Children.map(children, child => child ? React.cloneElement(child, { onClick: closeMenu }) : child);
  };
  ...
  render() {
    ...
    return <ul className={spaceSeparated(["dropdown-menu", className])}>
