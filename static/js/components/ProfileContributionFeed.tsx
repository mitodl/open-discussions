...
import { UserContributionState } from "../reducers/user_contributions";
...
class ProfileContributionFeed extends React.Component<Props> {
  ...
  componentDidUpdate() {
    ...
    if (canLoadMore && contributions && contributions[selectedTab].loaded === false) {
      this.loadData();
    }
  }
  ...
  getUpdatedContributionsList = (objectKey: string, updatedObjects: Map<string, Object>) => {
    const {
      contributions
    } = this.props;
    return updatedObjects ? contributions[objectKey].data.map(contribution => {
  ...
  renderContributionList = () => {
    ...
    if (selectedTab === POSTS_OBJECT_TYPE) {
      const posts = this.getUpdatedContributionsList(POSTS_OBJECT_TYPE, upvotedPosts);
      if (R.isEmpty(posts)) {
        return <div className="empty-list-msg">There are no posts to display.</div>;
      }
      return <div className="post-list">
          {posts.map((post, i) => <CompactPostDisplay key={i} post={post} isModerator={false} menuOpen={false} useSearchPageUI />)}
        </div>;
    } else {
      const comments = this.getUpdatedContributionsList(COMMENTS_OBJECT_TYPE, votedComments);
      if (R.isEmpty(comments)) {
        return <div className="empty-list-msg">
            There are no comments to display.
          </div>;
      }
      return <div className="comment-list">
          {comments.map((comment, i) => <Comment key={i} comment={comment} isModerator={false} isPrivateChannel={false} commentPermalink={commentPermalink(comment.channel_name, comment.post_id, comment.post_slug)} useSearchPageUI />)}
        </div>;
    }
  };
  ...
  render() {
    ...
    const contributionsLoading = !contributions || contributions[selectedTab].loaded === false;

    return <div>
        <IntraPageNav>
          {tabs.map(tabName => <NavLink to={`/profile/${userName}/${tabName}`} activeClassName="active" isActive={() => {
          return selectedTab === tabName;
        }} key={tabName}>
              {_.capitalize(tabName)}
            </NavLink>)}
        </IntraPageNav>
        {contributionsLoading ? <Loading className="infinite" /> : <InfiniteScroll hasMore={!!contributions[selectedTab].pagination.after} loadMore={this.loadMore} initialLoad={false} loader={<Loading className="infinite" key="loader" />}>
...
export default connect<Props, OwnProps, _, _, _, _>(mapStateToProps)(ProfileContributionFeed);