...
export default class CoverImageInput extends React.Component<Props> {
  ...
  setCoverImage = (image: File | null | undefined) => {
    const {
      onUpdate
    } = this.props;
  ...
  render() {
    ...
    return <div className="article-banner-image-wrapper" ref={this.node}>
        <div className="article-banner-image">
          {image ? <div className="cover-img-container">
              <img src={typeof image === "string" || image instanceof String ? image : URL.createObjectURL(image)} />
              <button className="high-contrast-outlined" type="button" onClick={this.clearCoverImage}>
                remove image
              </button>
            </div> : <Dropzone onDrop={this.handleImageDrop} className="photo-active-item photo-dropzone dashed-border" accept="image/*" onDropRejected={() => {
          if (setPhotoError) {
            setPhotoError("Please select a valid photo");
              ...
              <div className="cover-image-message">
                ...
                <a className="navy small-text" onClick={this.hideCoverImageInput} href="#">
                  remove cover image
                </a>
              </div>
            </Dropzone>}
