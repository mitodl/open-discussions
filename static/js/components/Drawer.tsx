...
import NavigationItem, { NavigationExpansion } from "../components/NavigationItem";
import HamburgerAndLogo from "../components/HamburgerAndLogo";
import Footer from "../components/Footer";

import { setShowDrawerMobile, setShowDrawerHover, setShowDrawerDesktop } from "../actions/ui";
import { getSubscribedChannels, isAudioPlayerLoadedSelector } from "../lib/redux_selectors";
import { getViewportWidth, isMobileWidth, DRAWER_BREAKPOINT, userIsAnonymous } from "../lib/util";
...
export class ResponsiveDrawer extends React.Component<DrawerProps> {
  ...
  onResize() {
    ...
    if (newWidth > this.width && newWidth > DRAWER_BREAKPOINT && this.width < DRAWER_BREAKPOINT && showDrawerMobile) {
  ...
  render() {
    ...
    if (userIsAnonymous()) {
      ...
      showComposeLink = currentChannel ? userCanPost(currentChannel) : R.any(userCanPost, [...channels.values()]);
    }
    ...
    return <div className={`${this.wrappingClass}${audioPlayerPadding}`}>
        <Theme>
          <Drawer modal={isMobile} open={isMobile ? showDrawerMobile : true} onClose={this.onDrawerClose} onPointerEnter={this.onMouseEnter} onPointerLeave={this.onMouseExit} className={this.wrappingClass}>
            <ScrollArea horizontal={false}>
              <DrawerContent>
                <NavigationExpansion.Provider value={expanded}>
                  <div>
                    {isMobile ? <div className="drawer-mobile-header">
                        <HamburgerAndLogo onHamburgerClick={this.onDrawerClose} />
                      </div> : null}
                    <Navigation subscribedChannels={subscribedChannels} pathname={pathname} showComposeLink={showComposeLink} composeHref={composeHref} />
                  </div>
