...
describe("Grid components", () => {
  ...
  ;[1, 12].forEach(width => {
    it(`should create a cell with width ${width}`, () => {
      const wrapper = mount(<Cell width={width}>
          <div className="child" />
        </Cell>);
  ...
  it("should allow passing a className to Grid", () => {
    const wrapper = mount(<Grid width={2} className="foobar">
        <div />
      </Grid>);
    assert.ok(wrapper.find(".foobar"));
  }) //
  ;[true, false].forEach(isMobileWidth => {
    it(`${shouldIf(isMobileWidth)} use mobileWidth if present and mobile==${String(isMobileWidth)}`, () => {
      isMobileWidthStub.returns(isMobileWidth);
      const wrapper = mount(<Cell width={4} mobileWidth={7}>
          <div />
        </Cell>);
      assert.ok(wrapper.find(`.mdc-layout-grid__cell--span-${isMobileWidth ? 7 : 4}`).exists());
    });
  });

  it("should allow passing a className to Cell", () => {
    const wrapper = mount(<Cell width={2} className="foobar">
        <div />
      </Cell>);
    assert.ok(wrapper.find(".foobar"));
  });
