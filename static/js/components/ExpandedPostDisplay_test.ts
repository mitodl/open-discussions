...
import { postPermalink, postDetailURL, profileURL, embedlyResizeImage } from "../lib/url";
...
describe("ExpandedPostDisplay", () => {
  let helper, render, post, channel, beginEditingStub, approvePostStub, removePostStub, showPostDeleteDialogStub, showPostReportDialogStub, toggleFollowPostStub, postProps;
  ...
  it("should render a post correctly", async () => {
    ...
    assert.equal(authoredBy.find(".right").text(), moment(post.created).fromNow());
  });
  ...
  ;[["My headline", true], [null, false]].forEach(([headlineText, expElementExists]) => {
    it(`${shouldIf(expElementExists)} display headline span when text=${String(headlineText)}`, async () => {
  ...
  it("should hide text content if passed showPermalinkUI", async () => {
    const {
      wrapper
    } = await render({ showPermalinkUI: true });
    assert.isFalse(wrapper.find(ReactMarkdown).exists());
  }) //
  ;[true, false].forEach(isLinkPost => {
    it(`${shouldIf(isLinkPost)} show a embedly compoent when link post:${isLinkPost.toString()}`, async () => {
  ...
  it("should link to the detail view, if a text post", async () => {
    ...
    const {
      to,
      children
    } = wrapper.find(Link).at(0).props();
    assert.equal(children, post.title);
    assert.equal(to, postDetailURL(post.channel_name, post.id, post.slug));
  }) //
  ;[["text", true, true], ["article", true, true], ["url", true, false], ["text", false, false], ["article", false, false], ["url", false, false]].forEach(([postType, userAuthor, shouldShowLink]) => {
    it(`${shouldIf(shouldShowLink)} show an edit link if post is ${postType} and user ${userAuthor ? "is" : "is not"} author`, async () => {
  ...
  ;[true, false].forEach(userAuthor => {
    it(`${shouldIf(userAuthor)} show a delete button if user ${userAuthor ? "is" : "is not"} author`, async () => {
      post = makePost();
      if (userAuthor) {
        SETTINGS.username = post.author_id;
      }
  ...
  it("should hide the follow link for anonymous users", async () => {
    ...
    assert.isNotOk(wrapper.find(".comment-action-button.subscribe-post").exists());
  });

  it("should hide post action buttons when editing", async () => {
    post = makePost(false);
    helper.store.dispatch(actions.forms.formBeginEdit({
  ...
  ;[[true, false, true], [true, false, false], [true, true, true], [true, true, false], [false, false, true], [false, false, false], [false, true, true], [false, true, false]].forEach(([isModerator, removed, isPostAuthor]) => {
    it(`${shouldIf(isModerator && !removed && !isPostAuthor)} display approve and remove links if isModerator=${String(isModerator)}, removed=${String(removed)}, isPostAuthor=${String(isPostAuthor)}`, async () => {
      ...
      const {
        wrapper
      } = await render({ isModerator });
      assert.equal(wrapper.find(".approve-post").exists(), isModerator && removed && !isPostAuthor);
      assert.equal(wrapper.find(".remove-post").exists(), isModerator && !removed && !isPostAuthor);
    });
  });
  ...
  it("should render a sharepopup", async () => {
    const {
      wrapper
    } = await render({ postShareMenuOpen: true });
  ...
  it("should use ArticleEditor and embedlyResizeImage to display if an article post", async () => {
    ...
    assert.equal(src, embedlyResizeImage(SETTINGS.embedlyKey, "/img/image.jpg", 300));
  });
});