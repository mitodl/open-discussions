...
describe("ProfileContributionFeed", function () {
  const contributionFetchActions = [actions.userContributions.get.requestType, actions.userContributions.get.successType];
  ...
  beforeEach(() => {
    ...
    helper.getUserPostsStub.returns(Promise.resolve({ [POSTS_OBJECT_TYPE]: postsResult, pagination: {} }));
  ...
  const renderFeed = async (props = {}) => {
    ...
    const renderedWrapper = await listenForActions(contributionFetchActions, () => {
      wrapper = mount(<Router history={helper.browserHistory} store={helper.store}>
            <ProfileContributionFeed userName={profile.username} selectedTab={POSTS_OBJECT_TYPE} {...props} />
          </Router>);
    }).then(() => {
  ...
  ;[[POSTS_OBJECT_TYPE, "getUserPostsStub", "CompactPostDisplay"], [COMMENTS_OBJECT_TYPE, "getUserCommentsStub", "Comment"]].forEach(([tabName, expStubCalled, expFeedComponent]) => {
  ...
  ;[[POSTS_OBJECT_TYPE, "no posts to display"], [COMMENTS_OBJECT_TYPE, "no comments to display"]].forEach(([tabName, expMessageFragment]) => {
    it(`should show a message when feed is empty in the ${tabName} tab`, async () => {
      helper.getUserPostsStub.returns(Promise.resolve({ [POSTS_OBJECT_TYPE]: [], pagination: {} }));
      helper.getUserCommentsStub.returns(Promise.resolve({
