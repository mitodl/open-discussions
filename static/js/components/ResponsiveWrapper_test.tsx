...
describe("ResponsiveWrapper", () => {
  const render = (props = { onlyOn: [] }) => mount(<ResponsiveWrapper {...props}>
  ...
  ;[PHONE, DESKTOP, TABLET].forEach(deviceCategory => {
    [[[PHONE], deviceCategory === PHONE], [[DESKTOP], deviceCategory === DESKTOP], [[TABLET], deviceCategory === TABLET]].forEach(([onlyOn, shouldShowChildren]) => {
      it(`${shouldIf(shouldShowChildren)} show children when ${deviceCategory} and onlyOn is ${JSON.stringify(onlyOn)}`, () => {
        useDeviceCategoryStub.returns(deviceCategory);
        const wrapper = render();
        assert.isNotOk(wrapper.find(".child-child-child").exists());
      });
