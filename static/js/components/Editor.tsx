...
export const menuBarManifest: Array<Array<MenuBarItem>> = [[{
  ...
  title: "bullet list",
  icon: <i className="material-icons format_list_bulleted">
          format_list_bulleted
        </i>
}, {
  command: wrapInList(schema.nodes.ordered_list),
  title: "ordered list",
  icon: <i className="material-icons format_list_numbered">
...
export const menuButtonClass = (state: Object | null | undefined, command: Function, active: Function | null | undefined) => {
...
export class Editor extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    const {
      getForm,
      actionCreators
    } = configureForm(this.uuid, newLinkForm);
    this.formHelpers = {
      getForm,
      ...actionCreators
    };
  ...
  renderMenuButton = ({
    ...
    icon,
    active
  }: MenuBarItem) => <div className={menuButtonClass(this.view ? this.view.state : null, command, active)} onClick={this.menuClickHandlerCreator(command)} key={title}>
      {icon}
    </div>;
  ...
  renderMenuBar = () => {
    ...
    const linkIconName = this.view && markIsActive(schema.marks.link, this.view.state) ? "link_off" : "link";

    return <div className="menu-bar">
        {menuBarManifest.map((buttonGroup, idx) => <div className="button-group" key={idx}>
            {buttonGroup.map(this.renderMenuButton)}
          </div>)}
        <div className="button-group">
          <div className={`menu-button ${selectionIsEmpty ? "disabled" : ""}`} onClick={selectionIsEmpty ? null : this.handleLinkClick}>
  ...
  selectionIsEmpty = () => {
    ...
    return ( // we want to check whether the selection is "empty"
    ...
    this.view.state.selection.empty || this.getEditorSelectionText().trim() === "");
  };

  getEditorSelectionText = () => {
    return this.view ? this.view.state.selection.content().content.content.map(node => node.textContent).join("") : "";
  };

  handleLinkClick = () => {
    if (markIsActive(schema.marks.link, this.view.state)) {
      this.dispatchTransaction(this.view.state.tr.replaceSelectionWith(schema.text(this.getEditorSelectionText()), false));
      this.view.focus();
    } else {
      this.openLinkMenu();
    }
  ...
  updateLinkMenuForm = (e: React.SyntheticEvent<HTMLInputElement>) => {
    const {
      dispatch
    } = this.props;
    const {
  ...
  addLinkToEditor = (e: Object) => {
    ...
    this.dispatchTransaction(this.view.state.tr.replaceSelectionWith(schema.text(text, [schema.marks.link.create({
      href: url
    })]), false));
  ...
  renderLinkMenu = () => {
    ...
    return <AddLinkMenu onChange={this.updateLinkMenuForm} onSubmit={this.addLinkToEditor} closeMenu={this.closeLinkMenu} text={text} url={url} />;
  };
  ...
  render() {
    ...
    return <div className="editor-wrapper">
        <div className="editor" id="editor" onClick={this.setEditorFocus} ref={this.node} />
        {this.renderMenuBar()}
