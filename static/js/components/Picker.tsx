...
const Picker = (labels, className) => {
  class Picker extends React.Component<Props, State> {
    ...
    render() {
      ...
      return <MenuSurfaceAnchor className={`picker ${className}`}>
          <Menu open={menuOpen} onClose={this.toggleMenuOpen}>
            {labels.map(([type, label]) => <MenuItem key={label} onClick={updatePickerParam(type)}>
                {label}
              </MenuItem>)}
          </Menu>
...
export const CommentSortPicker = Picker(VALID_COMMENT_SORT_LABELS, "comment-sort-picker");
CommentSortPicker.displayName = "CommentSortPicker";

export const SearchFilterPicker = Picker(VALID_SEARCH_FILTER_LABELS, "search-filter-picker");
SearchFilterPicker.displayName = "SearchFilterPicker";

export const ChannelMembersSortPicker = Picker(VALID_MEMBERS_SORT_LABELS, "channel-member-sort-picker");
