...
const runSearch = (searcher, text) => // $FlowFixMe
searcher.search(text).map(({
  item
}) => item);
...
function FilterableSearchFacet(props: Props) {
  ...
  return results && results.buckets && results.buckets.length === 0 ? null : <div className="facets filterable-facet">
      <div className="filter-section-title" onClick={() => setShowFacetList(!showFacetList)}>
        {title}
        <i className={`material-icons ${titleLineIcon}`}>{titleLineIcon}</i>
      </div>
      {showFacetList ? <>
          <div className="input-wrapper">
            <input className="facet-filter" type="text" onChange={handleFilterInput} value={filterText} placeholder={`Search ${title || ""}`} />
            {filterText === "" ? <i className="material-icons search-icon">search</i> : <i className="material-icons clear-icon" onClick={() => setFilterText("")} onKeyPress={e => {
          if (e.key === "Enter") {
            setFilterText("");
          }
        }} tabIndex="0">
          ...
          <div className="facet-list">
            {facets.map((facet, i) => <SearchFacetItem key={i} facet={facet} isChecked={R.contains(facet.key, currentlySelected || [])} onUpdate={onUpdate} labelFunction={labelFunction} name={name} />)}
          </div>
        </> : null}
    </div>;
}
