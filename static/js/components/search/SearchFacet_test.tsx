...
describe("SearchFacet", () => {
  ...
  const renderSearchFacet = props => mount(<SearchFacet name={name} title={title} // $FlowFixMe: test results are fine
  results={results} currentlySelected={[]} onUpdate={onUpdateStub} {...props} />);
  ...
  ;[[2, false], [20, true]].forEach(([numBuckets, shouldShowExpansionUI]) => {
    it(`${shouldIf(shouldShowExpansionUI)} show hide/show button when ${String(numBuckets)} buckets`, () => {
      // $FlowFixMe: who cares it's a test
      results.buckets = R.times(() => ({ key: "Physics", doc_count: 32 }), numBuckets);
