...
describe("CourseFilterDrawer", () => {
  let clearAllFiltersStub, toggleFacetStub, facetOptionsStub, onUpdateFacetsStub, sandbox, useDeviceCategoryStub;

  const render = (props = {}) => mount(<CourseFilterDrawer activeFacets={{}} clearAllFilters={clearAllFiltersStub} toggleFacet={toggleFacetStub} facetOptions={facetOptionsStub} onUpdateFacets={onUpdateFacetsStub} {...props} />);
  ...
  beforeEach(() => {
    ...
    useDeviceCategoryStub = sandbox.stub(utilHooks, "useDeviceCategory").returns(DESKTOP);
  });
  ...
  describe("FilterDisplay", () => {
    ...
    it("should create SearchFilters", () => {
      ...
      facetDisplayMap.map(([name,, labelFn], idx) => {
        ...
        sinon.assert.calledWith(toggleFacetStub, name, activeFacets[name][0], false);
      });
    });
    ...
    it("should render SearchFacets", () => {
      ...
      facetDisplayMap.map(([name, title, labelFn, useFilterableFacet]) => {
        ...
        assert.equal(searchFacet.name(), useFilterableFacet ? "FilterableSearchFacet" : "SearchFacet");
