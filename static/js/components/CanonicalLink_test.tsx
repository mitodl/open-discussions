...
describe("CanonicalLink", () => {
  [[partialUrl, null, fullUrl, "relative URL"], [null, makeMatch(partialUrl), fullUrl, "react-router Match object"], [partialUrl, makeMatch("different/url/stub"), fullUrl, "relative URL and Match object"]].forEach(([relativeUrlProp, matchProp, expectedLinkHref, desc]) => {
    it(`renders a <link> tag with the right href when given a ${desc}`, () => {
      SETTINGS.site_url = baseURL;
      const wrapper = shallow(<CanonicalLink relativeUrl={relativeUrlProp} match={matchProp} />);
