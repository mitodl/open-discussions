...
import ChannelFollowControls, { CHANNEL_FOLLOW_DROPDOWN, ChannelFollowControls as InnerChannelFollowControls } from "./ChannelFollowControls";
...
describe("ChannelFollowControls", () => {
  ...
  beforeEach(() => {
    ...
    getOwnProfileStub = helper.sandbox.stub(reduxSelectors, "getOwnProfile").returns(profile);
    ...
    render = helper.configureHOCRenderer(ChannelFollowControls, InnerChannelFollowControls, {
      ui: {
        dropdownMenus
      }
    }, {
  ...
  it("shows a 'follow' button when the user is not subscribed", async () => {
    ...
    assert.equal(store.getActions()[0].type, actions.channelSubscribers.post.requestType);
    sinon.assert.calledWithExactly(helper.addChannelSubscriberStub, props.channel.name, profile.username);
  ...
  describe("when subscribed", () => {
    ...
    ;[true, false].forEach(isOpen => {
      it(`${shouldIf(isOpen)} include a visible dropdown menu when indicated by state`, async () => {
        if (isOpen) {
    ...
    describe("has a dropdown menu", () => {
      ...
      it("with an 'unfollow' button that removes the user's subscription", async () => {
        ...
        assert.equal(actionsObserved[0].type, actions.channelSubscribers.delete.requestType);
        sinon.assert.calledWithExactly(helper.deleteChannelSubscriberStub, props.channel.name, profile.username);
        sinon.assert.calledOnce(helper.getChannelsStub);
      });
      ...
      });[["channel_type", CHANNEL_TYPE_PUBLIC], ["user_is_contributor", false], ["user_is_moderator", true]].forEach(([channelProp, propValue]) => {
        it(`without a 'leave' button if ${channelProp}=${String(propValue)}`, async () => {
