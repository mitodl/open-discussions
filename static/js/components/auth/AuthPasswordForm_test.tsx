...
describe("AuthPasswordForm component", () => {
  ...
  const mountForm = (props = {}) => mount(<Router history={browserHistory}>
        <AuthPasswordForm onSubmit={onSubmitStub} onUpdate={onUpdateStub} form={form} validation={{}} processing={false} formError={""} {...props} />
      </Router>);
  ...
  it("should show validation", () => {
    ...
    assert.equal(wrapper.find(".validation-message").text(), "Password is required");
