...
describe("AuthDetailsForm component", () => {
  ...
  const mountForm = (props = {}) => mount(<AuthDetailsForm onSubmit={onSubmitStub} onUpdate={onUpdateStub} form={form} validation={{}} processing={false} formError={""} {...props} />);
  ...
  it("should show validation", () => {
    ...
    assert.equal(wrapper.find(".validation-message").at(0).text(), "Name is required");
    assert.equal(wrapper.find(".validation-message").at(1).text(), "Password is required");
  ...
  it("should show a form level error", () => {
    ...
    assert.equal(wrapper.find(".validation-message").at(0).text(), "backend error");
  }) //
