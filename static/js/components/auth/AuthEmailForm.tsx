...
class AuthEmailForm extends React.Component<Props, State> {
  ...
  scaleRecaptcha = () => {
    if (this.recaptcha) {
      ...
      const recaptchaScale = clientWidth < RECAPTCHA_NATURAL_WIDTH ? clientWidth / RECAPTCHA_NATURAL_WIDTH : 1.0;
  ...
  render() {
    ...
    return <form onSubmit={onSubmit} className="form">
        <div className="emailfield row">
          <input type="email" name="email" value={form.email} onChange={onUpdate} placeholder="Email" />
          {validationMessage(validation.email)}
        </div>
        {SETTINGS.recaptchaKey && onRecaptcha ? <div className="recaptcha row">
            <StyledReCAPTCHA ref={this.setRecaptcha} sitekey={SETTINGS.recaptchaKey} onChange={onRecaptcha} scale={recaptchaScale} />
            {validationMessage(validation.recaptcha)}
          </div> : null}
        <div className="actions row right-aligned">
          <button type="submit" className={`submit-login ${processing ? "disabled" : ""}`} disabled={processing}>
            {submitLabel || "Next"}
          </button>
        </div>
      </form>;
