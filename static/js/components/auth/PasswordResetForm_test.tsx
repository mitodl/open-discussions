...
describe("PasswordResetForm component", () => {
  ...
  const mountForm = (props = {}) => mount(<PasswordResetForm onSubmit={onSubmitStub} onUpdate={onUpdateStub} form={form} validation={{}} processing={false} emailApiError={undefined} {...props} />);
  ...
  it("should show an error message if validation fails", () => {
    ...
    assert.equal(wrapper.find(".validation-message").text(), "Email is required");
  });
