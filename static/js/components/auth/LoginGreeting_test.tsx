...
describe("LoginGreeting", () => {
  [["Testuser", "x.jpg", "a@b.com", "exists"], [undefined, undefined, "a@b.com", "does not exist"]].forEach(([name, profileImageUrl, email, descriptor]) => {
    it(`should render the page with correct messages when user data ${descriptor}`, async () => {
      ...
      const expectedGreeting = expectedProfileInfo ? `Hi ${String(name)}` : "Welcome Back!";

      const wrapper = mount(<LoginGreeting name={name} profileImageUrl={profileImageUrl} email={email} onBackButtonClick={sinon.stub()} />);
      ...
      if (expectedProfileInfo) {
        assert.equal(profileInfoSection.find("img").prop("src"), profileImageUrl);
  ...
  it("passes down a back button click handler", async () => {
    ...
    const wrapper = mount(<LoginGreeting name="Test User" profileImageUrl="abc.jpg" email="abc@example.com" onBackButtonClick={onBackButtonClick} />);
