...
import { ImageForm } from "../flow/discussionTypes";
...
export class ImageUploader extends React.Component<ImageProps> {
  ...
  render() {
    ...
    return <span>
        ...
        {showButton ? <button className="open-photo-dialog" onClick={e => {
        e.preventDefault();
        this.setDialogVisibility(true);
      }}>
            {isAdd ? <i name="camera_alt" className="material-icons add">
                add
              </i> : <i name="camera_alt" className="material-icons edit">
                edit
              </i>}
          </button> : null}
      </span>;
...
const mapStateToProps = (state, ownProps) => {
  ...
  const {
    getForm
  } = configureForm(formKey, newImageForm);
  const {
    ui
  } = state;
...
export default R.compose(connect(mapStateToProps, mapDispatchToProps, mergeProps), withForm(ImageUploaderForm))(ImageUploader);