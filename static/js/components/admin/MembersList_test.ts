...
import { makeChannel, makeContributors, makeModerators } from "../../factories/channels";
...
describe("MembersList", () => {
  ...
  ;[["contributor", R.prop("contributor_name")], ["moderator", R.prop("moderator_name")]].forEach(([memberTypeDescription, usernameGetter]) => {
    describe(memberTypeDescription, () => {
      let members, renderForm;

      beforeEach(() => {
        members = memberTypeDescription === "contributor" ? makeContributors() : makeModerators(null, true);
      ...
      ;[true, false].forEach(editable => {
        it(`should ${editable ? "" : "not "}render remove links when the channel is ${editable ? "" : "not "}editable`, () => {
      ...
      ;[true, false].forEach(isYou => {
        it(`removes ${isYou ? "you" : "a member"} from the member list when the dialog box accept button is clicked`, () => {
