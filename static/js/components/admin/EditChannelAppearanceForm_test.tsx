...
describe("EditChannelAppearanceForm", () => {
  const renderForm = (form: ChannelForm, {
    onSubmit,
    onUpdate
  } = { onSubmit: () => {}, onUpdate: () => {} }) => shallow(<EditChannelAppearanceForm channel={channel} form={form} onSubmit={onSubmit} onUpdate={onUpdate} validation={{ title: "", description: "", public_description: "" }} history={history} processing={false} />);
  ...
  it("should render a form with values", () => {
    ...
    assert.equal(wrapper.find("input[name='public_description']").props().value, form.public_description);
    ...
    describe("onUpdate", () => {
      ...
      ;["avatar", "banner"].forEach(field => {
        [true, false].forEach(hasEdit => {
          it(`should modify the ${field} when the blob ${hasEdit ? "exists" : "doesn't exist"}`, () => {
