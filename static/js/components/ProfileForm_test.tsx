...
describe("ProfileForm", () => {
  let helper, profile, onUpdateStub, onSubmitStub, onUpdateSocialSiteStub, onSubmitSocialSiteStub, onUpdatePersonalSiteStub, onSubmitPersonalSiteStub, onDeleteSiteStub, onUpdateLocationStub, onClearLocationStub;
  ...
  }, validation = { reason: "", name: "" }) => mount(<Provider store={helper.store}>
        <ProfileForm profile={profile} form={form} validation={validation} socialSiteFormValues={{}} socialSiteFormErrors={{}} personalSiteFormValues={{}} personalSiteFormErrors={{}} onUpdate={onUpdateStub} onSubmit={onSubmitStub} onUpdateSocialSite={onUpdateSocialSiteStub} onSubmitSocialSite={onSubmitSocialSiteStub} onUpdatePersonalSite={onUpdatePersonalSiteStub} onSubmitPersonalSite={onSubmitPersonalSiteStub} onDeleteSite={onDeleteSiteStub} onUpdateLocation={onUpdateLocationStub} onClearLocation={onClearLocationStub} history={{}} processing={false} {...props} />
      </Provider>);

  it("displays the name, headline and bio", () => {
    const wrapper = renderForm();
    assert.equal(wrapper.find(".profile-name").find(".name").props().value, profile.name);
    assert.equal(wrapper.find(".profile-name").find(".name").props().placeholder, "Full name");
    assert.equal(wrapper.find(".bio").find("textarea").props().value, profile.bio);
    assert.equal(wrapper.find(".bio").find("textarea").props().placeholder, "Description");
    assert.equal(wrapper.find(".bio").find("label").text(), "Add a short description about yourself, max 1000 characters");
    assert.equal(wrapper.find(".headline").find("input").props().value, profile.headline);
    assert.equal(wrapper.find(".headline").find("input").props().placeholder, "Headline");
    assert.equal(wrapper.find(".headline").find("label").text(), "For example: 'Post Doc, Photonics MIT', max 60 characters");
    assert.equal(wrapper.find(".location").find("input").props().defaultValue, profile.placename);
  }) //
  ;[true, false].forEach(sameUser => {
    it(`ProfileImage should ${sameUser ? "" : "not"} include an edit profile button`, async () => {
  ...
  describe("websites section", () => {
    ...
    beforeEach(() => {
      ...
      it(`updates when text is entered into ${inputName} input`, () => {
        const updateStub = inputName === "socialSite" ? onUpdateSocialSiteStub : onUpdatePersonalSiteStub;
        const wrapper = renderForm();
        const event = {
          target: {
            name: "name",
        ...
        wrapper.find(`input[name="${inputName}"]`).at(0).simulate("change", event);
    ...
    it("renders an input when the user has not yet provided a personal site", () => {
      ...
      assert.isTrue(wrapper.find('.personal-site-link input[type="text"]').exists());
    });
    ...
    });[[".social-site-links", "socialSiteFormValues", "social"], [".personal-site-link", "personalSiteFormValues", "personal"]].forEach(([sectionSel, formValueProp, siteTypeDesc]) => {
      it(`submits input text when the submit button is clicked in the ${siteTypeDesc} site section`, () => {
        const submitStub = siteTypeDesc === "social" ? onSubmitSocialSiteStub : onSubmitPersonalSiteStub;
        const wrapper = renderForm({
          [formValueProp]: {
            url: "example.com"
          }
