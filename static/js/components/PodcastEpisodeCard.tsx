...
export default function PodcastEpisodeCard(props: Props) {
  ...
  return <Card className="podcast-episode-card low-padding" persistentShadow={persistentShadow}>
      <div id="podcast-episode-card-inner-container" tabIndex="0" onKeyPress={e => {
      if (e.key === "Enter") {
        openEpisodeDrawer(e);
      }
        ...
        <div className="right-col">
          <img src={embedlyThumbnail(SETTINGS.embedlyKey, episode.image_src || defaultResourceImageURL(), PODCAST_IMG_HEIGHT, PODCAST_IMG_WIDTH)} height={PODCAST_IMG_HEIGHT} alt={`cover image for ${episode.title}`} className="episode-cover-image" />
        </div>
      </div>
    </Card>;
}