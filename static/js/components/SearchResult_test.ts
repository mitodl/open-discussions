...
import { makeCommentResult, makePostResult, makeProfileResult, makeLearningResourceResult } from "../factories/search";
import { searchResultToComment, searchResultToLearningResource, searchResultToPost, searchResultToProfile } from "../lib/search";
...
describe("SearchResult", () => {
  ...
  it("renders a profile card", async () => {
    ...
    assert.equal(wrapper.find(".name").at(0).prop("to"), profileURL(profile.username));
  ...
  it("renders an upvoted post", async () => {
    ...
    const {
      wrapper
    } = await render({ result, upvotedPost });
    const postDisplay = wrapper.find("Connect(CompactPostDisplay)");
  ...
  it("renders a comment that has been voted on", async () => {
    ...
    const {
      wrapper
    } = await render({ result, votedComment });
    const commentTree = wrapper.find("CommentTree");
