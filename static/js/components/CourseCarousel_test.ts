...
describe("CourseCarousel", () => {
  ...
  beforeEach(() => {
    ...
    render = helper.configureHOCRenderer(CourseCarousel, CourseCarousel, {}, {
  ...
  it("should display the provided title", async () => {
    ...
    assert.equal(wrapper.find(".title-and-controls .title").text(), "Greatest Courses Ever");
  });
  ...
  it("should render a card for each course in the carousel", async () => {
    ...
    R.zip(carousel.find("LearningResourceDisplay").map(R.identity), courses).forEach(([el, course]) => {
