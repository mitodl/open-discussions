...
import { LR_TYPE_USERLIST, LR_TYPE_LEARNINGPATH, LR_PUBLIC, LR_PRIVATE } from "../lib/constants";
...
describe("UserListFormDialog", () => {
  ...
  beforeEach(() => {
    ...
    helper.handleRequestStub.withArgs(topicApiURL).returns(queryListResponse(topics));
    userList = makeUserList();
    helper.handleRequestStub.withArgs(userListApiURL.toString()).returns(userList);
  ...
  ;[true, false].forEach(isListStaff => {
    it("should render a form with all the right inputs", async () => {
      ...
      assert.equal(wrapper.find(`input[value="${LR_PUBLIC}"]`).exists(), isListStaff);
      assert.equal(wrapper.find(`input[value="${LR_PRIVATE}"]`).exists(), isListStaff);
    });
  }) //
  ;[[0, "Subject is required"], [4, `Select no more than ${TOPICS_LENGTH_MAXIMUM} subjects`]].forEach(([selectCount, topicError]) => {
    it(`should call validator, show the results with ${selectCount} subjects selected`, async () => {
      ...
      topics.slice(0, selectCount).forEach(topic => {
        wrapper.find("Select").at(1).instance().selectOption({ label: topic.name, value: topic.id });
        wrapper.update();
      });
      ...
      await wait(50);
      wrapper.update();
      assert.deepEqual(["You need to select a list type", "You need to select a privacy level", "Title is required", "Description is required", topicError], wrapper.find(".validation-message").map(el => el.text()));
    });
  });
  ...
  it("should let a user with correct permissions fill out the form and create a public list", async () => {
    ...
    changeFormikInput(wrapper.find(`input[value="${LR_TYPE_USERLIST}"]`), "list_type", LR_TYPE_USERLIST);
    changeFormikInput(wrapper.find(`input[value="${LR_PUBLIC}"]`), "privacy_level", LR_PUBLIC);
    changeFormikInput(wrapper.find('input[name="title"]'), "title", "Title");
    wrapper.find("Select").at(1).instance().selectOption({ label: topics[0].name, value: topics[0].id });
    changeFormikInput(wrapper.find("textarea"), "short_description", "My Great Description");
    wrapper.update();
    wrapper.find("form").simulate("submit");
    await wait(50);
    ...
    const [url, method, {
      body
    }] = helper.handleRequestStub.args[1];
  ...
  it("should pre-populate fields when you pass a list", async () => {
    ...
    assert.isTrue(wrapper.find(`input[value="${userList.list_type}"]`).prop("checked"));
    assert.isTrue(wrapper.find(`input[value="${userList.privacy_level}"]`).prop("checked"));
    assert.equal(wrapper.find('input[name="title"]').prop("value"), userList.title);
    assert.equal(wrapper.find("textarea").prop("value"), userList.short_description);
  });
  ...
  it("should let you edit and then patch a list", async () => {
    ...
    changeFormikInput(wrapper.find('input[name="title"]'), "title", "My Brand New Title");
    wrapper.find("Select").at(1).instance().selectOption({ label: topics[0].name, value: topics[0].id });
    wrapper.find("Select").at(1).instance().selectOption({ label: topics[1].name, value: topics[1].id });
    wrapper.update();
    wrapper.find("form").simulate("submit");
    await wait(50);
    assert.ok(hideStub.called);
    ...
    assert.equal(url, userListDetailApiURL.param({ userListId: userList.id }).toString());
