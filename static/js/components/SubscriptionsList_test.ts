...
describe("SubscriptionsList", function () {
  ...
  it("should show each channel", () => {
    ...
    assert.equal(wrapper.find(".my-channels .channel-link").length, myChannels.length);
    assert.equal(wrapper.find(".channels .channel-link").length, notMyChannels.length);
    ...
    wrapper.find(".my-channels .channel-link").forEach((link, index) => {
      ...
      const {
        badge,
        whenExpanded
      } = link.find(NavigationItem).props();
  ...
  it("should highlight the current channel", () => {
    ...
    assert.equal(currentLocation.find(".channel-link").at(0).props().to, channelURL(channels[0].name));
  });
});