...
describe("CommentTree", () => {
  ...
  ;[true, false].forEach(useSearchPageUI => {
    it(`${shouldIf(!useSearchPageUI)} show the Share button`, async () => {
      const {
        wrapper
      } = await render({ useSearchPageUI });
      assert.equal(wrapper.find(".share-button-wrapper").exists(), !useSearchPageUI);
    });
  });
  ...
  it('should not include a "reply" button if useSearchPageUI is true', async () => {
    const {
      wrapper
    } = await render({ useSearchPageUI: true });
  ...
  it('should not include a "reply" button for deleted comments', async () => {
    ...
    assert.isNotOk(wrapper.find(".top-level-comment .comment-actions").at(0).find(".comment-action-button.reply-button").exists());
  }) //
  ;[true, false].forEach(useSearchPageUI => {
    [true, false].forEach(userIsAnonymous => {
      it(`${shouldIf(!userIsAnonymous && !useSearchPageUI)} include a showMenu icon when anon=${String(userIsAnonymous)} and search=${String(useSearchPageUI)}`, async () => {
        anonStub.returns(userIsAnonymous);
        const {
          wrapper
        } = await render({ useSearchPageUI });
        assert.equal(wrapper.find(".more_vert").exists(), !userIsAnonymous && !useSearchPageUI);
      });
    });
  });
  ...
  it('should include a "report" button', async () => {
    ...
    openMenu(wrapper);
    wrapper.find(".comment-action-button.report-button").at(0).simulate("click", null);
  ...
  ;[[true, "Unfollow"], [false, "Follow"]].forEach(([subscribed, buttonText]) => {
    it(`should include a ${buttonText} button when subscribed === ${String(subscribed)}`, async () => {
      ...
      assert.deepEqual(helper.updateCommentStub.args[0], [comment.id, { subscribed: !subscribed }]);
    });
  });
  ...
  it("should show the author name", async () => {
    ...
    const authorName = wrapper.find(".comment").at(0).find(".author-name").at(0).text();
  ...
  ;[true, false].forEach(isPrivateChannel => {
    it(`should render a ShareTooltip on a ${isPrivateChannel ? "private" : "public"} channel`, async () => {
      const {
        wrapper
      } = await render({ isPrivateChannel });
  ...
  it("should render a profile image", async () => {
    ...
    const {
      src
    } = wrapper.find(".profile-image").at(0).props();
  ...
  describe("moderation UI", () => {
    ...
    it("should include an ignoreCommentReports button if report and moderator", async () => {
      ...
      assert.deepEqual(helper.updateCommentStub.args[0], [comment.id, { ignore_reports: true }]);
    });
    ...
    it("should let a moderator approve a comment", async () => {
      ...
      assert.deepEqual(helper.updateCommentStub.args[0], [comment.id, { removed: false }]);
    });
    ...
    it("should let a moderator remove a comment", async () => {
      ...
      assert.deepEqual(helper.updateCommentStub.args[0], [comment.id, { removed: true }]);
    });
  });
});