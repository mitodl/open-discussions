...
import { learningResourceSelector, getResourceRequest, similarResourcesRequest, getSimilarResources } from "../lib/queries/learning_resources";
import { interactionMutation } from "../lib/queries/interactions";
import { isAudioPlayerLoadedSelector } from "../lib/redux_selectors";

const getLRHistory = createSelector(state => state.ui, ui => {
  const mostRecentHistoryEntry = ui.LRDrawerHistory[ui.LRDrawerHistory.length - 1];

  const objectId = mostRecentHistoryEntry ? mostRecentHistoryEntry.objectId : undefined;
  const objectType = mostRecentHistoryEntry ? mostRecentHistoryEntry.objectType : undefined;
  const runId = mostRecentHistoryEntry ? mostRecentHistoryEntry.runId : undefined;
...
const getEmbedlyForObject = createSelector(getLRHistory, learningResourceSelector, getEmbedlys, (LRHistory, lrSelector, embedlys) => {
  const {
    objectId,
    objectType
  } = LRHistory;
  ...
  if (!object || object.object_type !== LR_TYPE_VIDEO || !object.url || !embedlys) {
    return null;
  }
...
const getSimilarResourcesForObject = createSelector(getLRHistory, learningResourceSelector, getSimilarResources, (LRHistory, lrSelector, similarResources) => {
  const {
    objectId,
    objectType
  } = LRHistory;
...
export default function LearningResourceDrawer(props: Props) {
  ...
  useRequest(objectType === LR_TYPE_VIDEO && object ? embedlyRequest(object.url) : null);
  ...
  const pushHistory = useCallback(object => dispatch(pushLRHistory(object)), [dispatch]);
  const popHistory = useCallback(() => dispatch(popLRHistory()), [dispatch]);

  return <Theme>
      <Drawer open={numHistoryEntries > 0} onClose={closeDrawer} dir="rtl" className={`align-right lr-drawer${audioPlayerPadding}`} modal>
        <DrawerContent dir="ltr" ref={drawerContent}>
          <div className="drawer-nav">
            <div tabIndex="0" className="drawer-close" onKeyPress={e => {
            if (e.key === "Enter") {
              closeDrawer();
            }
          }} onClick={closeDrawer}>
            ...
            {numHistoryEntries > 1 ? <div tabIndex="0" className="back" onKeyPress={e => {
            if (e.key === "Enter") {
              popHistory();
            }
          }} onClick={popHistory}>
          ...
          {object ? <ExpandedLearningResourceDisplay object={object} runId={runId} setShowResourceDrawer={pushHistory} embedly={embedly} similarItems={similarResources} hideSimilarLearningResources={!!hideSimilarLearningResources} /> : null}
