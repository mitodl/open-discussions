...
import { audioPlayerStateSelector, currentlyPlayingAudioSelector } from "../lib/redux_selectors";
...
export default function AudioPlayer() {
  ...
  const audioLoaded = !_.isEqual(currentlyPlayingAudio, INITIAL_AUDIO_STATE.currentlyPlaying);
  ...
  const seekBarClick = useCallback((e: Object) => {
    ...
    if (current && current.getBoundingClientRect && current.offsetWidth) {
      ...
      Amplitude.setSongPlayedPercentage(parseFloat(x) / parseFloat(offsetWidth) * 100);
    }
  }, [seekBar]);

  const stopAudioPlayer = useStopAudioPlayer();

  return <div className={`audio-player-container-outer${!audioLoaded ? " hidden" : ""}`}>
      <div className="audio-player-container-inner">
        <div className="audio-player-text">
          <div className="audio-player-titles">
            <div className="audio-player-title" data-amplitude-song-info="album" />
            <div className="audio-player-description" data-amplitude-song-info="name" />
          </div>
        </div>
        <div className="audio-player-controls">
          <div className="audio-player-playback-controls">
            <div className="audio-player-button-container">
              <i className="audio-player-close material-icons close" onClick={stopAudioPlayer}>
                close
              </i>
              ...
              <div className="audio-player-button amplitude-play-pause" id="play-pause" ref={playPauseButton}>
                <span className="material-icons">
                  {audioPlayerState === AUDIO_PLAYER_PLAYING ? "pause_circle_outline" : "play_circle_outline"}
                </span>
              </div>
          ...
          <div className="audio-player-progress-container">
            ...
            <progress className="amplitude-song-played-progress audio-player-progress" id="audio-player-progress" ref={seekBar} onClick={seekBarClick} />
