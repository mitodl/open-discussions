...
import { CommentInTree } from "../flow/discussionTypes";
...
export default function CommentVoteForm(props: Props) {
  ...
  const showErrorBanner = useCallback(gerund => {
    dispatch(setBannerMessage(`Something went wrong ${gerund} this comment. Contact us at ${SETTINGS.support_email}`));
  }, [dispatch]);

  const upvote = useCallback(async e => {
    e.preventDefault();
    setUpvoting(true);
    try {
  ...
  const downvote = useCallback(async e => {
    e.preventDefault();
    setDownvoting(true);
    try {
      await dispatch(actions.comments.patch(comment.id, {
  ...
  return <div className="votes-form">
      <div className="score">
        {SETTINGS.username === comment.author_id ? comment.score : score}
      </div>
      <LoginTooltip>
        <button className={`vote upvote-button ${upvoted ? "upvoted" : ""}`} onClick={userIsAnonymous() ? null : upvote} disabled={disabled}>
          <img className="vote-arrow" src={upvoted ? "/static/images/upvote_arrow_on.png" : "/static/images/upvote_arrow.png"} width="13" />
        </button>
      </LoginTooltip>
      <span className="pipe">|</span>
      <LoginTooltip>
        <button className={`vote downvote-button ${downvoted ? "downvoted" : ""}`} onClick={userIsAnonymous() ? null : downvote} disabled={disabled}>
          <img className="vote-arrow" src={downvoted ? "/static/images/downvote_arrow_on.png" : "/static/images/downvote_arrow.png"} width="13" />
        </button>
      </LoginTooltip>
    </div>;
}