...
import { searchResultToComment, searchResultToPost, searchResultToProfile } from "../lib/search";
...
import { LearningResourceResult, ProfileResult, Result } from "../flow/searchTypes";
...
const PostSearchResult = ({
  post
}: PostProps) => <CompactPostDisplay post={post} isModerator={false} menuOpen={false} useSearchPageUI />;
...
const CommentSearchResult = ({
  ...
  return <CommentTree comments={[comment]} remove={() => undefined} approve={() => undefined} upvote={upvote} downvote={downvote} isModerator={false} isPrivateChannel={false} commentPermalink={commentPermalink(channel, comment.post_id, slug)} curriedDropdownMenufunc={dropdownMenuFuncs(() => null)} dropdownMenus={new Set()} useSearchPageUI />;
};
...
const ProfileSearchResult = ({
  ...
  return <Card className="profile-search-result">
...
const LearningResourceSearchResult = ({
  ...
  return <LearningResourceCard object={searchResultToFavoriteLR(result)} searchResultLayout={searchResultLayout} />;
};
...
type Props = {
  ...
  result: Result;
  upvotedPost?: Post | null | undefined;
  votedComment?: CommentInTree | null | undefined;
  setShowResourceDrawer?: (arg0: {
    objectId: string;
    objectType: string;
...
export default class SearchResult extends React.Component<Props> {
  ...
  render() {
    ...
    if (result.object_type === "post") {
      ...
      return <CommentSearchResult comment={comment} upvote={commentUpvote} downvote={commentDownvote} // $FlowFixMe: This will always be a Comment result
      channel={result.channel_name} // $FlowFixMe: This will always be a Comment result
      ...
      return <LearningResourceSearchResult result={result} setShowResourceDrawer={setShowResourceDrawer} searchResultLayout={searchResultLayout} />;
    }
    return null;
  }
}