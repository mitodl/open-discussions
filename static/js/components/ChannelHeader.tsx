  ...
  children
    ...
    <Grid className="main-content two-column channel-header">
      <Cell className="avatar-headline-row" width={12}>
        <div className="left">
          <ChannelAvatar editable={false} channel={channel} imageSize={CHANNEL_AVATAR_MEDIUM} />
          <div className="title-and-headline">
            <div className="title">
              <Link to={channelURL(channel.name)}>{channel.title}</Link>
            </div>
            {channel.public_description ? <div className="headline">{channel.public_description}</div> : null}
          </div>
        </div>
        <div className="right channel-controls">
          <ChannelFollowControls channel={channel} history={history} />
          {isModerator ? <ChannelSettingsLink channel={channel} history={history} /> : null}
        </div>
      </Cell>
    </Grid>
    {children}
