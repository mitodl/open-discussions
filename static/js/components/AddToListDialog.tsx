...
import { LR_TYPE_COURSE, LR_TYPE_PROGRAM, LR_TYPE_VIDEO, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE } from "../lib/constants";
...
import { favoriteUserListMutation, userListsRequest, myUserListsSelector } from "../lib/queries/user_lists";
import { createUserListItemMutation, deleteUserListItemMutation } from "../lib/queries/user_list_items";
import { favoriteVideoMutation } from "../lib/queries/videos";
import { favoritePodcastMutation, favoritePodcastEpisodeMutation } from "../lib/queries/podcasts";
import { learningResourceSelector, getResourceRequest } from "../lib/queries/learning_resources";

const uiDialogSelector = createSelector(state => state.ui, ui => ui.dialogs.get(DIALOG_ADD_TO_LIST) || {});
...
export default function AddToListDialog() {
  ...
  }] = useRequest(userListsRequest());

  const listItemsById = listsById(resource && isFinishedResource ? resource.lists : []);
  ...
  const [, toggleListItem] = useMutation((resource, list, item, remove) => {
    return remove ? deleteUserListItemMutation(list.id, item) : createUserListItemMutation(list.id, resource);
  });
  ...
  const renderAddToListForm = () => <div className="user-listitem-form">
      <div className="flex-row">
        <div>
          <Checkbox checked={resource ? resource.is_favorite : false} onChange={() => toggleFavorite(resource)}>
            Favorites
          </Checkbox>
        </div>
        <div>
      ...
      {resource && !isUserList(resource.object_type) ? <React.Fragment>
          {userLists.map((userList, i) => {
        const listItem = listItemsById[userList.id];
        return <div className="flex-row" key={i}>
                <div>
                  <Checkbox checked={!!listItem} onChange={(e: any) => {
              toggleListItem(resource, userList, listItem, !e.target.checked);
              updateResource(e.target.checked, userList.id);
                ...
                <div>
                  <div className="grey-surround privacy">
                    <i className={`material-icons ${privacyIcon(userList.privacy_level)}`}>
          ...
          <button className="create-new-list blue-btn" onClick={() => setShowUserListFormDialog(true)} onKeyPress={e => {
        if (e.key === "Enter") {
  ...
  return resource && isFinishedList && isFinishedResource ? <React.Fragment>
      <Dialog id="list-add-dialog" open={!emptyOrNil(resource)} hideDialog={showUserListFormDialog ? null : hide} title="Add to List" onAccept={hide} noButtons submitText="OK" className="user-listitem-dialog">
        {renderAddToListForm()}
      </Dialog>
      {showUserListFormDialog ? <UserListFormDialog hide={() => setShowUserListFormDialog(false)} /> : null}
    </React.Fragment> : null;
}