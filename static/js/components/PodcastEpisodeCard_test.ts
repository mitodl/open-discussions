...
import PodcastEpisodeCard, { PODCAST_IMG_WIDTH, PODCAST_IMG_HEIGHT, EPISODE_DATE_FORMAT } from "./PodcastEpisodeCard";
...
describe("PodcastEpisodeCard", () => {
  ...
  it("should render basic stuff", async () => {
    ...
    assert.equal(wrapper.find(".date").text(), moment(episode.last_modified).format(EPISODE_DATE_FORMAT));
    assert.equal(wrapper.find("img").prop("src"), embedlyThumbnail(SETTINGS.embedlyKey, episode.image_src, PODCAST_IMG_HEIGHT, PODCAST_IMG_WIDTH));
  });
  ...
  it("should put a click handler on card to open drawer", async () => {
    ...
    wrapper.find("#podcast-episode-card-inner-container").at(0).simulate("click");
  ...
  it("should pass down persistentShadow to Card if passed", async () => {
    const {
      wrapper
    } = await render({ persistentShadow: true });
