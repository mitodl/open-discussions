...
import { makeCourse, makeLearningResource } from "../factories/learning_resources";
import { makeYoutubeVideo } from "../factories/embedly";
import { LR_TYPE_COURSE, LR_TYPE_PROGRAM, LR_TYPE_VIDEO, LR_TYPE_USERLIST, LR_TYPE_LEARNINGPATH, LR_TYPE_ALL, LR_PUBLIC, LR_PRIVATE, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE, readableLearningResources } from "../lib/constants";
import { mockHTMLElHeight } from "../lib/test_utils";
import { bestRun, getInstructorName, isUserList } from "../lib/learning_resources";
...
describe("ExpandedLearningResourceDisplay", () => {
  let course, embedly, helper, setShowResourceDrawerStub, render, similarResources;
  ...
  beforeEach(() => {
    ...
    similarResources = [makeLearningResourceResult(LR_TYPE_COURSE), makeLearningResourceResult(LR_TYPE_PROGRAM), makeLearningResourceResult(LR_TYPE_COURSE)];
    mockHTMLElHeight(50, 100);
    helper.stubComponent(UserListItemsMod, "PaginatedUserListItems", "PaginatedUserListItems");
    helper.stubComponent(PaginatedPodcastEpisodesMod, "PaginatedPodcastEpisodes");
    render = helper.configureHOCRenderer(ExpandedLearningResourceDisplay, ExpandedLearningResourceDisplay, {}, {
      object: course,
  ...
  it(`should render a course image`, async () => {
    ...
    assert.ok(wrapper.find(".image-div").find("img").prop("src").includes("https://i.embed.ly/1/display/crop"));
    assert.ok(wrapper.find(".image-div").find("img").prop("src") // $FlowFixMe: this won't be null
  ...
  it(`should render a default course image if none exists`, async () => {
    ...
    assert.ok(wrapper.find(".image-div").find("img").prop("src").includes(encodeURIComponent(defaultResourceImageURL())));
  }) //
  ...
  it("should render the podcast title for podcast episode", async () => {
    ...
    assert.equal(wrapper.find(".podcast-subtitle").at(0).text(), object.podcast_title);
  });
  ...
  ;[[LR_TYPE_PODCAST_EPISODE, "View Episode Details"], [LR_TYPE_PODCAST, "View Podcast Details"]].forEach(([lrType, expectedText]) => {
  ...
  it("should set the href property of the view episode details button to the podcast episode's episode_link property", async () => {
    ...
    assert.equal(wrapper.find(".podcast-detail-row .podcast-detail-link").prop("href"), object.episode_link);
  });
  ...
  it("should set href for podcast to the podcasts URL", async () => {
    ...
    assert.equal(wrapper.find(".podcast-detail-row .podcast-detail-link").prop("href"), object.url);
  }) //
  ...
  LR_TYPE_ALL.forEach(objectType => {
    ...
    it(`should render a title for ${objectType}`, async () => {
      ...
      assert.equal(wrapper.find(objectType === LR_TYPE_PODCAST || objectType === LR_TYPE_PODCAST_EPISODE ? ".podcast-main-title" : ".title").at(0).text(), object.title);
    });
    ...
    it(`should display all topics for the ${objectType}`, async () => {
      ...
      assert.deepEqual(topicDivs.map(topicDiv => topicDiv.text()).sort(), object.topics.map(topic => topic.name).sort());
    });

    it(`should include a display of similar resources for ${objectType}`, async () => {
      const object = makeLearningResource(objectType);
      const listIdx = [LR_TYPE_LEARNINGPATH, LR_TYPE_USERLIST, LR_TYPE_PROGRAM, LR_TYPE_PODCAST].includes(objectType) ? 1 : 0;
      const {
        wrapper
      } = await render({}, {
        object,
      ...
      const similarResourcesDiv = wrapper.find(".expanded-learning-resource-list").at(listIdx);
      assert.ok(similarResourcesDiv.exists());

      if (isUserList(object.object_type) || object.object_type === LR_TYPE_PROGRAM) {
        assert.equal(similarResourcesDiv.find("LearningResourceRow").length, 0);
        similarResourcesDiv.find("i").simulate("click");
      }
      assert.equal(wrapper.find(".expanded-learning-resource-list").at(listIdx).find("LearningResourceRow").length, 3);
    });
    ...
    it(`should hide similar resources for ${objectType} if prop passed`, async () => {
      ...
      const {
        wrapper
      } = await render({}, { object, hideSimilarLearningResources: true });
      const listIdx = [LR_TYPE_LEARNINGPATH, LR_TYPE_USERLIST, LR_TYPE_PROGRAM, LR_TYPE_PODCAST].includes(objectType) ? 1 : 0;
      assert.isNotOk(wrapper.find(".expanded-learning-resource-list").at(listIdx).exists());
    });
    ...
    it(`should have a share link for a ${objectType}`, async () => {
      ...
      if (objectType === LR_TYPE_PODCAST || objectType === LR_TYPE_PODCAST_EPISODE) {
        assert.isNotOk(wrapper.find("ShareTooltip").exists());
      } else {
        const {
          objectType: _objectType,
          url
        } = wrapper.find("ShareTooltip").props();
  ...
  ;[["mitx", "September 01, 2019", "Start Date:"], ["ocw", "Fall 2019", "As Taught In:"]].forEach(([platform, expectedValue, expectedLabel]) => {
  ...
  ;[[1, false], [2, true]].forEach(([runs, showDropdown]) => {
    it(`${shouldIf(showDropdown)} display a course run dropdown for a course with ${runs} run(s)`, async () => {
  ...
  ;[["2019-09-01T00:00:00Z", "2019-08-01T00:00:00Z", "September 01, 2019"], [null, "2019-08-01T00:00:00Z", "August 01, 2019"], [null, null, "Ongoing"]].forEach(([startDate, bestDate, expected]) => {
    it(`mitx run date should be ${expected} for start date ${String(startDate)}, best date ${String(bestDate)}`, async () => {
      course.platform = "mitx";
      const courseRun = course.runs[0];
      ...
      const dateValue = wrapper.find(".select-semester-div").find("option").at(0).text();
  ...
  ;[LR_TYPE_PROGRAM, LR_TYPE_COURSE].forEach(objectType => {
    it(`should display all instructors for the ${objectType}`, async () => {
      ...
      const instructorText = wrapper.find(".school").at(1).closest(".info-row").find(".value").text();
      // $FlowFixMe: course run won't be null here
  ...
  ;[["en-us", "English"], ["fr", "French"], ["zh-CN", "Chinese"], [null, "English"], ["", "English"]].forEach(([langCode, langName]) => {
    it(`should display the correct language name for ${String(langCode)}`, async () => {
      // $FlowFixMe: course run won't be null here
      bestRun(course.runs).language = langCode;
      // $FlowFixMe: course run won't be null here
      const {
        wrapper
      } = await render({}, { runId: bestRun(course.runs).id });
      assert.equal(wrapper.find(".language").closest(".info-row").find(".value").text(), langName);
    });
  }) //
  ;[LR_TYPE_COURSE, LR_TYPE_PROGRAM].forEach(objectType => {
  ...
  ;[LR_TYPE_COURSE, LR_TYPE_PROGRAM].forEach(objectType => {
    it(`should display the cost for ${objectType}`, async () => {
      ...
      assert.equal(wrapper.find(".lr-metadata").find(".info-row").at(0).find(".value").text(), "$25.50");
    });
  }) //
  ;[LR_TYPE_USERLIST, LR_TYPE_LEARNINGPATH].forEach(objectType => {
    it(`should display the authors name for ${objectType}`, async () => {
      ...
      assert.equal(wrapper.find(".local_offer").closest(".info-row").find(".value").text(), object.author_name);
    });
    ...
    it(`should display the privacy for ${objectType}`, async () => {
      ...
      assert.equal(wrapper.find(".lock").closest(".info-row").find(".value").text(), capitalize(object.privacy_level));
    });
    ...
    it(`should display the length for ${objectType}`, async () => {
      ...
      assert.equal(wrapper.find(".view_list").closest(".info-row").find(".value").text(), object.item_count);
    });
    ...
    ;[true, false].forEach(isPrivate => {
      it(`${shouldIf(isPrivate)} hide sharing button for ${objectType} when private === ${String(isPrivate)}`, async () => {
        const object = makeLearningResource(objectType);
  ...
  it("should include a display of list items for program", async () => {
    ...
    listItemsDiv.find("i").simulate("click");
    assert.equal(wrapper.find(".expanded-learning-resource-list").at(0).find("LearningResourceRow").length, object.items.length);
    R.zip(object.items.map(item => item.content_data), wrapper.find(".expanded-learning-resource-list").at(0).find("LearningResourceRow").map(el => el.prop("object"))).forEach(([obj1, obj2]) => assert.deepEqual(obj1, obj2));
  });
  ...
  it(`should not include a display of list items for course`, async () => {
    ...
    assert.isOk(wrapper.find(".expanded-learning-resource-list").text().startsWith("Similar Learning Resources"));
  });
  ...
  it(`should still display without errors in case of a bad course with no runs`, async () => {
    ...
    assert.equal(wrapper.find(".language").closest(".info-row").find(".value").text(), "English");
  }) //
  ;[[LR_TYPE_COURSE, false], [LR_TYPE_PROGRAM, false], [LR_TYPE_USERLIST, false], [LR_TYPE_VIDEO, true]].forEach(([objectType, hasEmbedly]) => {
    it(`should ${hasEmbedly ? "" : "not "}display an embedly component for object_type=${objectType}`, async () => {
      const object = makeLearningResource(objectType);
      const {
        wrapper
      } = await render({}, { object });
