...
describe("UserListCard tests", () => {
  ...
  it("should print the list type (learning path || list)", async () => {
    const {
      wrapper
    } = await renderUserListCard();
    assert.equal(wrapper.find(".platform").text(), readableLearningResources[userList.list_type]);
  });
  ...
  ;[[0, "Items"], [1, "Item"], [2, "Items"], [10, "Items"]].forEach(([itemCount, expectedText]) => {
    it(`should have a properly-formatted count of ${String(itemCount)} items`, async () => {
  ...
  it("should hide the dropdown menu if you pass the flag", async () => {
    const {
      wrapper
    } = await renderUserListCard({ hideUserListOptions: true });
  ...
  it("should have a button to open a delete menu, which should confirm", async () => {
    ...
    assert.equal(title, `Delete this ${readableLearningResources[userList.list_type]}?`);
    hideDialog();
    wrapper.update();
  ...
  it("should issue a DELETE request if the user confirms", async () => {
    ...
    const {
      onAccept
    } = getDeleteDialog(wrapper).props();
    await onAccept();

    sinon.assert.calledWith(helper.handleRequestStub, userListDetailApiURL.param({ userListId: userList.id }).toString(), "DELETE");
  });
