...
import { ContextRouter, Location } from "react-router";
...
const renderNavLinks = (location: Location, linkSpecMap: {
  [to: string]: LinkSpec;
}) => Object.keys(linkSpecMap).map((to, i) => {
  const linkSpec = linkSpecMap[to];
  const isPathHandled = location.pathname in linkSpecMap;
  const extraClass = !isPathHandled && linkSpec.isDefault ? activeClassName : "";
  return <NavLink key={i} to={to} className={`tab ${extraClass}`} activeClassName={activeClassName}>
        {linkSpec.text}
      </NavLink>;
});
