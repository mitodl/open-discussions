...
describe("CommentForm", () => {
  ...
  const renderCommentForm = (props = {}) => mount(<Provider store={helper.store}>
  ...
  it("should pass the initial text to formik if passed the editing flag", () => {
    ...
    assert.equal(comment.text, wrapper.find("Formik").prop("initialValues").commentText);
  });
  ...
  it("should create a comment with the right arguments onSubmit", async () => {
    ...
    await wrapper.find("Formik").prop("onSubmit")({ commentText: "hey!" }, onSubmitActions);
    sinon.assert.calledWith(onSubmitActions.resetForm, { commentText: "" });
    sinon.assert.calledWith(onSubmitActions.setSubmitting, false);
    sinon.assert.calledWith(helper.createCommentStub, post.id, "hey!", comment.id);
  });
  ...
  it("should call closeReply on submit, if passed", async () => {
    ...
    await wrapper.find("Formik").prop("onSubmit")({ commentText: "asdfasdfasdf" }, onSubmitActions);
