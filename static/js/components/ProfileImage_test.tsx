...
import ProfileImage, { PROFILE_IMAGE_MICRO, PROFILE_IMAGE_SMALL, PROFILE_IMAGE_MEDIUM } from "./ProfileImage";
...
describe("ProfileImage", () => {
  ...
  const renderProfileImage = (props = {}) => {
    ...
    return mount(<Provider store={helper.store}>
        <ProfileImage profile={thatProfile} imageSize={PROFILE_IMAGE_MEDIUM} {...props} />
      </Provider>, {
      attachTo: div
    });
  };
  ...
  describe("upload button", () => {
    ...
    describe("save button", () => {
      ...
      it("should show a validation message if the image upload fails", async () => {
        ...
        await wait(0);
        image.update();
        assert.equal(image.find(".validation-message").text(), "Error uploading image");
        assert.isTrue(helper.patchProfileImageStub.called);
      });
      ...
      it("should show a validation message if the drag and drop fails", async () => {
        ...
        await wait(0);
        image.update();
        assert.equal(image.find(".validation-message").text(), "Please select a valid photo");
      });
      ...
      it("clears the form when the cancel button is clicked", async () => {
        ...
        await listenForActions([actions.forms.FORM_BEGIN_EDIT, HIDE_DIALOG, HIDE_DIALOG], () => {
  ...
  it("should set imageSize as a className", () => {
    [PROFILE_IMAGE_MICRO, PROFILE_IMAGE_SMALL, PROFILE_IMAGE_MEDIUM].forEach(imageSize => {
      const image = renderProfileImage({
        imageSize
      });
      assert.include(image.find(".profile-image").props().className, imageSize);
    });
  });

  it("should pass the processing value", async () => {
    const render = helper.configureHOCRenderer(ProfileImage, "ProfileImage", {
      profileImage: {
        processing: true
      }
    }, { editable: true });
    ...
    assert.equal(inner.find("Connect(withForm(ImageUploader))").props().processing, true);
  }) //
  ;[[PROFILE_IMAGE_MICRO, true], [PROFILE_IMAGE_SMALL, true], [PROFILE_IMAGE_MEDIUM, false]].forEach(([imageSize, exp]) => {
    it(`image url ${shouldIf(exp)} be small if imageSize is ${imageSize}`, () => {
      const image = renderProfileImage({ imageSize });
      assert.equal(image.find(".profile-image").props().src, thatProfile.profile_image_small);
    });
  });
});