...
describe("EditPostForm", () => {
  ...
  const renderEditPostForm = (props = {}) => mount(<Provider store={helper.store}>
        <EditPostForm post={post} editing {...props} />
      </Provider>);

  const setEditorText = (wrapper, text) => wrapper.find("Editor").props().onChange(text);
  ...
  it("should submit the form and only submit the text field", async () => {
    ...
    const state = await helper.listenForActions([requestType, successType, forms.FORM_END_EDIT], () => {
      wrapper.find("form").simulate("submit");
    });

    assert.deepEqual(state.forms, {});
    assert.deepEqual(helper.editPostStub.args[0], [post.id, { text: "edited text", id: post.id }]);
  });
});