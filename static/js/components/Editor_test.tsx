...
import ConnectedEditor, { Editor, newLinkForm, menuBarManifest, menuButtonClass } from "./Editor";
...
describe("Editor component", () => {
  ...
  const renderEditor = (props = {}) => mount(<Editor dispatch={dispatch} onChange={onChange} forms={{}} {...props} />);

  const renderConnectedEditor = (props = {}) => {
    const wrapper = mount(<Provider store={helper.store}>
  ...
  const executeTestTransaction = wrapper => toggleMark(schema.marks.strong)(wrapper.instance().view.state, wrapper.instance().view.dispatch, wrapper.instance().view);
  ...
  ;[true, false].forEach(autoFocus => {
    it(`${shouldIf(autoFocus)} call .focus on view if autoFocus = ${String(autoFocus)}`, () => {
  ...
  it("should set the focus helper on the whole editor", () => {
    const wrapper = renderEditor();
    assert.equal(wrapper.instance().setEditorFocus, wrapper.find(".editor").props().onClick);
  });
  ...
  it("should use the initialValue prop, if supplied", () => {
    ...
    const output = defaultMarkdownSerializer.serialize(wrapper.instance().view.state.doc);
    assert.equal(output, initialValue);
  });

  describe("dispatchTransaction", () => {
    it("should bind our custom handler", () => {
      const dispatchStub = helper.sandbox.stub(Editor.prototype, "dispatchTransaction");
      const wrapper = renderEditor();
    ...
    it("should call onChange with serialized md", () => {
      const mdStub = helper.sandbox.stub(pmMarkdown.defaultMarkdownSerializer, "serialize").returns("# MARKDOWN!");
  ...
  describe("menuClickHandlerCreator", () => {
    it("menuClickHandlerCreator should return a function", () => {
      assert.isFunction(renderEditor().instance().menuClickHandlerCreator(helper.sandbox.stub()));
    });
    ...
    it("should call passed function with correct args, preventDefault, focus", () => {
      ...
      const wrapper = renderEditor();
      const clickHandler = wrapper.instance().menuClickHandlerCreator(commandStub);
      clickHandler({ preventDefault });
      sinon.assert.calledWith(commandStub, wrapper.instance().view.state, wrapper.instance().view.dispatch, wrapper.instance().view);
  ...
  describe("menu bar", () => {
    describe("menuButtonClass", () => {
      it("should return menu-button if no state", () => {
        assert.equal(menuButtonClass(null, new Function(), new Function()), "menu-button");
      }) //
      ;[[true, true, "menu-button active"], [true, false, "menu-button active"], [false, true, "menu-button"], [false, false, "menu-button disabled"]].forEach(([active, command, exp]) => {
        it(`should return ${exp} if active == ${String(active)} and command == ${String(command)}`, () => {
      ...
      it("should defer to command if active is null", () => {
        [[true, "menu-button"], [false, "menu-button disabled"]].forEach(([command, exp]) => {
          const commandStub = helper.sandbox.stub().returns(command);
    ...
    it("should have the buttons in the manifest", () => {
      const wrapper = renderEditor();
      assert.equal(wrapper.find(".button-group").length, menuBarManifest.length + 1);
      ...
      const manifestIconNames = [...R.flatten(menuBarManifest).map(item => item.icon.props.children), "link"];
      assert.deepEqual(buttonTexts, manifestIconNames);
    }) //
    ;[true, false].forEach(markIsActive => {
      it(`should handle link button click if link active == ${String(markIsActive)}`, () => {
        const wrapper = renderEditor();
        const isActiveStub = helper.sandbox.stub(pmLib, "markIsActive").returns(markIsActive);
        const openLinkMenuStub = helper.sandbox.stub(wrapper.instance(), "openLinkMenu");

        if (markIsActive) {
          helper.sandbox.stub(wrapper.instance(), "getEditorSelectionText").returns("link text");
        }
    ...
    ;[true, false].forEach(empty => {
      it(`should disable the link button if the selection is ${empty ? "" : "not "}empty`, () => {
        const wrapper = renderEditor();
        const selectionIsEmptyStub = helper.sandbox.stub(wrapper.instance(), "selectionIsEmpty").returns(empty);
    ...
    ;[true, false].forEach(enabled => {
      R.flatten(menuBarManifest).forEach((item, index) => {
        it(`${shouldIf(!enabled)} disable ${item.title} button if command ${enabled ? "can" : "can't"} be applied`, () => {
          const itemActiveStub = helper.sandbox.stub(item, "command").returns(enabled);
          const wrapper = renderEditor();
          updateWrapper(wrapper);
          const menuItem = wrapper.find(".menu-button").at(index);

          assert.equal(menuItem.props().className, enabled ? "menu-button" : "menu-button disabled");
          sinon.assert.called(itemActiveStub);
        });
      });
    }) //
    ;[[true, "link_off"], [false, "link"]].forEach(([linkActive, linkNameExp]) => {
      it(`should use right link icon when linkActive === ${String(linkActive)}`, () => {
        const wrapper = renderEditor();
        const markIsActiveStub = helper.sandbox.stub(pmLib, "markIsActive").returns(linkActive);
        wrapper.instance().forceUpdate();
  ...
  describe("<AddLinkMenu />", () => {
    ...
    it("should not render <AddLinkMenu /> if the form doesn't exist", () => {
      ...
      inner.instance().openLinkMenu();
      helper.store.dispatch(actions.forms.formEndEdit({ formKey: inner.instance().uuid }));
      wrapper.update();
    ...
    it("should pass the right props to <AddLinkMenu />", () => {
      ...
      const {
        ...
        onSubmit,
        closeMenu
      } = wrapper.find(AddLinkMenu).props();
    ...
    it("should update the form", () => {
      const wrapper = renderEditor();
      const formUpdateStub = helper.sandbox.stub(wrapper.instance().formHelpers, "formUpdate");
    ...
    it("should add link to editor", () => {
      ...
      const getFormStub = helper.sandbox.stub(wrapper.instance().formHelpers, "getForm").returns({
        value: { url, text }
      });
      const closeLinkMenuStub = helper.sandbox.stub(wrapper.instance(), "closeLinkMenu");
      const preventDefault = helper.sandbox.stub();
