...
import { courseDetailApiURL, topicApiURL, userListApiURL, userListDetailApiURL } from "../lib/url";
...
describe("AddToListDialog", () => {
  ...
  beforeEach(() => {
    ...
    helper.handleRequestStub.withArgs(userListApiURL.toString()).returns(queryListResponse(userLists));
    helper.handleRequestStub.withArgs(topicApiURL).returns(queryListResponse([]));
    helper.handleRequestStub.withArgs(courseDetailApiURL.param({ courseId: course.id }).toString()).returns({
      status: 200,
      body: course
    });
  });
  ...
  const render = async (object = course) => {
    const {
      wrapper,
      store
    } = await renderDialog({}, [setDialogData({ dialogKey: DIALOG_ADD_TO_LIST, data: object })]);
  ...
  it("if resource is a list, there should only be a Favorites checkbox", async () => {
    ...
    helper.handleRequestStub.withArgs(userListDetailApiURL.param({ userListId: object.id }).toString()).returns({
      status: 200,
      body: object
    });
    const {
  ...
  ;[true, false].forEach(inList => {
    it(`${shouldIf(inList)} precheck a userlist if the resource is ${inList ? "" : "not"} in it`, async () => {
      if (inList) {
        course.lists.push({
  ...
  it(`should uncheck the correct userlist`, async () => {
    ...
    const {
      wrapper
    } = await render(course);[1, 2, 3].forEach(idx => {
