...
describe("PasswordChangeForm component", () => {
  ...
  const getPasswordInput = wrapper => wrapper.find("input[name='current_password']");
  const getNewPasswordInput = wrapper => wrapper.find("input[name='new_password']");

  const mountForm = (props = {}) => mount(<Router history={browserHistory}>
        <PasswordChangeForm onSubmit={onSubmitStub} onUpdate={onUpdateStub} form={form} validation={{}} processing={false} invalidPwError={undefined} {...props} />
      </Router>);
