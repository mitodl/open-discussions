...
describe("CommentRemovalForm", () => {
  let sandbox, approveStub, removeStub, comment, resolveApprove, resolveRemove, renderForm;

  beforeEach(() => {
    sandbox = sinon.createSandbox();
    // use promise which will never resolve
    approveStub = sandbox.stub().returns(new Promise(resolve => {
      resolveApprove = resolve;
    }));
    removeStub = sandbox.stub().returns(new Promise(resolve => {
  ...
  const assertButton = (wrapper, isRemove) => {
    const availableActionSelector = isRemove ? ".remove-button" : ".approve-button";
  ...
  ;[true, false].forEach(isRemove => {
    it(`clicks the ${isRemove ? "remove" : "approve"} button`, async () => {
      ...
      const button = wrapper.find(isRemove ? ".remove-button" : ".approve-button");
