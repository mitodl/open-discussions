...
export default class SearchTextbox extends React.Component<Props> {
  ...
  render() {
    const {
      ...
      noFocusOnLoad,
      // have to pull this out here to avoid a console error
      ...extraProps
    } = this.props;
    return <div className="search-textbox">
        ...
        {value ? <i className="material-icons clear-icon" onClick={onClear}>
            clear
          </i> : null}
        <input ref={this.input} type="text" name="query" className="underlined" value={value} onChange={onChange} onKeyDown={event => event.key === "Enter" ? onSubmit(event) : null} {...extraProps} />
