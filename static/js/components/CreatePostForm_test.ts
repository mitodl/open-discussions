...
describe("CreatePostForm", () => {
  let sandbox, isLinkTypeAllowedStub, renderPostForm, openClearPostTypeDialogStub, updatePostTypeStub, setPhotoErrorStub, onUpdateStub, hideCoverImageInputStub;
  ...
  it("should show post_type validation message", () => {
    ...
    assert.equal(wrapper.find(".post-type-row .validation-message").text(), "HEY");
  });
  ...
  ;[["share-a-link", [LINK_TYPE_TEXT, LINK_TYPE_ARTICLE]], ["write-something", [LINK_TYPE_LINK, LINK_TYPE_ARTICLE]], ["write-an-article", [LINK_TYPE_TEXT, LINK_TYPE_TEXT]]].forEach(([selector, postTypes]) => {
  ...
  it("should show article validation message", () => {
    ...
    assert.equal(wrapper.find(".article .validation-message").text(), "WHAT?! NO! OF COURSE NOT!");
  });
  ...
  it("should pass down a coverImage, if there is one in the form", () => {
    ...
    const {
      image
    } = wrapper.find("CoverImageInput").props();
  ...
  ;[[[LINK_TYPE_TEXT, LINK_TYPE_LINK, LINK_TYPE_ARTICLE], LINK_TYPE_LINK, true], [[LINK_TYPE_TEXT, LINK_TYPE_LINK, LINK_TYPE_ARTICLE], LINK_TYPE_TEXT, true], [[LINK_TYPE_LINK, LINK_TYPE_ARTICLE], LINK_TYPE_LINK, true], [[LINK_TYPE_TEXT, LINK_TYPE_ARTICLE], LINK_TYPE_TEXT, true], [[LINK_TYPE_TEXT], LINK_TYPE_TEXT, false], [[LINK_TYPE_LINK], LINK_TYPE_LINK, false], [[LINK_TYPE_ARTICLE], LINK_TYPE_ARTICLE, false]].forEach(([allowedTypes, selectedType, showClosebutton]) => {
    it(`${shouldIf(showClosebutton)} show clear button when channel ${allowedTypes.toString()} and form has ${selectedType}`, () => {
  ...
  ;[true, false].forEach(formIsEmpty => {
    it(`${formIsEmpty ? "clears form" : "opens confirm dialog"} when clear button clicked if form is ${formIsEmpty ? "empty" : "not empty"}`, () => {
  ...
  it("should show channel validation message", () => {
    ...
    assert.equal(wrapper.find(".channel-select .validation-message").text(), "HEY");
  });
  ...
  describe("embedly preview link", () => {
    ...
    it("should show embedly when a non-error response has arrived", () => {
      ...
      error.type = "error";[[article, true], [error, false]].forEach(([embedly, shouldRenderComponent]) => {
        const wrapper = renderPostForm({
          postForm,
          embedly
        });
    ...
    it("should show EmbedlyLoader when no embedly and embedlyInFlight", () => {
      const article = makeArticle();[[article, true, false], [article, false, false], [undefined, true, true], [undefined, false, false]].forEach(([embedly, embedlyInFlight, shouldRenderComponent]) => {
        const wrapper = renderPostForm({
          postForm,
          embedly,
          embedlyInFlight
        });
        assert.equal(wrapper.find(EmbedlyLoader).exists(), shouldRenderComponent);
      });
    });
  }) //
  ;[[true, false], [false, true], [true, true]].forEach(([showLink, showText]) => {
    it(`shows ${showLink && showText ? "both buttons" : showText ? "the text button" : "the link button"}`, () => {
      isLinkTypeAllowedStub.callsFake((_, linkType) => linkType === LINK_TYPE_TEXT ? showText : showLink);
      const form = {
        postType: null,
        title: "",
        text: "",
  ...
  ;[true, false].forEach(uiEnabled => {
    it(`${shouldIf(uiEnabled)} show the article button when flag is ${String(uiEnabled)}`, () => {
