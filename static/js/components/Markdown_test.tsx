...
describe("Markdown", () => {
  ...
  it("should not render images", () => {
    const wrapper = renderMD("![](https://upload.wikimedia.org/wikipedia/commons/4/4c/Chihuahua1_bvdb.jpg)");
    assert.isNotOk(wrapper.find("img").exists());
  });

  it("should automatically turn URLs into links", () => {
    const wrapper = renderMD("ah I love linking to things https://en.wikipedia.org/wiki/Pie");
    const link = wrapper.find("a");
    assert.ok(link.exists());
    assert.equal(link.props().href, "https://en.wikipedia.org/wiki/Pie");
    assert.deepEqual(link.props().children, ["https://en.wikipedia.org/wiki/Pie"]);
  });
