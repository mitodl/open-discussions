...
describe("UserMenu", () => {
  ...
  it("should include the profile image with onclick handler", () => {
    ...
    const {
      onClick
    } = wrapper.find(".user-menu-clickarea").props();
    onClick();
  ...
  ;[[true, true, false], [true, false, true], [false, true, false], [false, false, false]].forEach(([featureFlagEnabled, complete, shouldShowDot]) => {
    it(`${shouldIf(shouldShowDot)} include a red dot since the feature flag is ${featureFlagEnabled ? "enabled" : "disabled"} and the profile is
  ...
  describe("menu items", () => {
    ...
    ;[true, false].forEach(uiEnabled => {
      it(`dropdown menu ${shouldIf(uiEnabled)} include a profile link if profile UI ${uiEnabled ? "" : "not"} enabled`, async () => {
        SETTINGS.profile_ui_enabled = uiEnabled;
        SETTINGS.username = profile.username;
        const wrapper = shallow(<LoggedInMenu {...menuProps} />);
        assert.equal(wrapper.find("Link").at(1).exists(), uiEnabled);
        if (uiEnabled) {
          assert.equal(wrapper.find("Link").at(1).props().to, profileURL(profile.username));
        }
      });
    });
  });
