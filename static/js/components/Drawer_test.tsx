...
describe("Drawer", () => {
  ...
  const renderDrawer = (props = {}) => shallow(<ResponsiveDrawer subscribedChannels={channels} location={{ pathname: "a path", search: "", hash: "" }} dispatch={dispatchStub} channels={new Map(channels.map(channel => [channel.name, channel]))} showDrawerDesktop={false} showDrawerMobile={false} showDrawerHover={false} audioPlayerLoaded={false} {...props} />);

  it("should set a CSS class when not mobile and open", () => {
    isMobileWidthStub.returns(false);[true, false].forEach(showDrawerDesktop => {
      const wrapper = renderDrawer({ showDrawerDesktop });

      assert.equal(wrapper.props().className, showDrawerDesktop ? "persistent-drawer open" : "persistent-drawer closed");
    });
  });

  it("should include a menu button when mobile", () => {
    const wrapper = renderDrawer();
    const component = wrapper.find(".drawer-mobile-header").find("HamburgerAndLogo");
    assert.ok(component.exists());
    assert.equal(component.props().onHamburgerClick, wrapper.instance().onDrawerClose);
  });
  ...
  ;[[true, true, true, true], [true, false, true, true], [true, true, false, true], [true, false, false, true], [false, true, true, true], [false, false, true, true], [false, true, false, true], [false, false, false, false]].forEach(([isMobile, showDrawerDesktop, showDrawerHover, expandedExp]) => {
    it(`should have expanded==${String(expandedExp)} when isMobile==${String(isMobile)}, showDrawerDesktop=${String(showDrawerDesktop)}, showDrawerHover=${String(showDrawerHover)}`, () => {
      isMobileWidthStub.returns(isMobile);
      const wrapper = renderDrawer({
        showDrawerDesktop,
        showDrawerHover
      });
      assert.equal(wrapper.find(NavigationExpansion.Provider).prop("value"), expandedExp);
    });
  });
  ...
  describe("props passed to Navigation component", () => {
    ...
    it("should always include subscribed channels and path", () => {
      const {
        subscribedChannels,
        pathname
      } = renderDrawer().find(Navigation).props();
    ...
    describe("on a channel page", () => {
      ...
      beforeEach(() => {
        ...
        it(`${shouldIf(hasPermission)} include showComposeLink=true if user post permission=${String(hasPermission)}`, () => {
  ...
  describe("mapStateToProps", () => {
    ...
    beforeEach(() => {
      ...
      getSubscribedChannelsStub = sandbox.stub(selectors, "getSubscribedChannels");
      isAudioPlayerLoadedStub = sandbox.stub(selectors, "isAudioPlayerLoadedSelector");
    });
