...
const imageHeight = css`
  @media (max-width: ${PHONE_WIDTH}px) {
    height: ${props => props.compactOnMobile ? shortMobileBannerHeight : bannerHeight};
  }
...
const imageWrapperHeight = css`
  @media (max-width: ${PHONE_WIDTH}px) {
    min-height: ${props => props.compactOnMobile ? shortMobileBannerHeight : bannerHeight};
  }
  @media (min-width: ${PHONE_WIDTH + 1}px) {
...
export const BannerImage = ({
  ...
  tall,
  compactOnMobile
}: ImgProps) => src ? <StyledImage src={src} tall={tall} alt={alt || ""} compactOnMobile={compactOnMobile} /> : <PlaceholderDiv />;
