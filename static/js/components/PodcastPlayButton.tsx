...
import { PodcastEpisode } from "../flow/podcastTypes";
...
export default function PodcastPlayButton(props: Props) {
  ...
  const episodeCurrentlyPlaying = episodeAudioInitialized && audioPlayerState === AUDIO_PLAYER_PLAYING;
  ...
  const initializeAudioPlayer = useCallback(e => {
    e.stopPropagation();
    e.preventDefault();
    initAudioPlayer();
  }, [initAudioPlayer]);
  ...
  return <div className={`podcast-play-button ${className}`}>
