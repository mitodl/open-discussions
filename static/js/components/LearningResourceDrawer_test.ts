...
import { makeCourse, makeProgram, makeVideo, makeUserList, makeLearningResource } from "../factories/learning_resources";
import { makeYoutubeVideo } from "../factories/embedly";
import { LR_TYPE_PROGRAM, LR_TYPE_COURSE, LR_TYPE_LEARNINGPATH } from "../lib/constants";
import IntegrationTestHelper from "../util/integration_test_helper";
import { pushLRHistory } from "../actions/ui";

import { courseDetailApiURL, programDetailApiURL, userListDetailApiURL, videoDetailApiURL, embedlyApiURL, similarResourcesURL, interactionsApiURL } from "../lib/url";
...
describe("LearningResourceDrawer", () => {
  ...
  beforeEach(() => {
    ...
    course = makeCourse();
    similarItems = [makeSearchResult(LR_TYPE_COURSE), makeSearchResult(LR_TYPE_PROGRAM), makeSearchResult(LR_TYPE_COURSE)];
    helper.stubComponent(LRCardMod, "LearningResourceRow", "LearningResourceRow");
  ...
  const mockObjectAPI = (object, apiUrl) => helper.handleRequestStub.withArgs(`${apiUrl}/${object.id}/`).returns({
    status: 200,
    body: object
  });
  ...
  const renderWithObject = async (object, apiUrl, extraProps = {}) => {
    ...
    const {
      wrapper,
      store
    } = await render(extraProps, [pushLRHistory({
      objectId: object.id,
  ...
  it("should have a button to hide the course drawer", async () => {
    const {
      wrapper,
      store
    } = await renderWithObject(course, courseDetailApiURL.param({ courseId: course.id }).toString());
  ...
  it("should have a back button if there is more than one object in the drawer history", async () => {
    ...
    mockObjectAPI(course, courseDetailApiURL.param({ courseId: course.id }).toString());
    mockObjectAPI(program, programDetailApiURL.param({ programId: program.id }).toString());
    const {
      wrapper,
      store
    } = await render({}, [pushLRHistory({
      objectId: course.id,
  ...
  it("should pass a callback to add to the history stack to ExpandedLearningResourceDisplay", async () => {
    ...
    mockObjectAPI(learningPath, userListDetailApiURL.param({ userListId: learningPath.id }).toString());
    const {
      wrapper,
      store
    } = await renderWithObject(course, courseDetailApiURL.param({ courseId: course.id }).toString());
  ...
  ;[[makeProgram(), programDetailApiURL, "programId"], [makeCourse(), courseDetailApiURL, "courseId"], [makeUserList(), userListDetailApiURL, "userListId"]].forEach(([object, apiUrl, key]) => {
    it(`should render ExpandedLearningResourceDisplay with object of type ${object.object_type}`, async () => {
      ...
      const {
        wrapper
      } = await renderWithObject(object, url);
  ...
  it("should include an ExpandedLearningResourceDisplay for a video", async () => {
    ...
    helper.handleRequestStub.withArgs(`${embedlyApiURL}/${encodeURIComponent(encodeURIComponent(video.url))}/`).returns({
      status: 200,
      body: embedly
    });
    const {
      wrapper
    } = await renderWithObject(video, videoDetailApiURL.param({ videoId: video.id }).toString());
    const expandedDisplay = wrapper.find(ExpandedLearningResourceDisplay);
  ...
  it("should pass hideSimilarLearningResources prop to ExpandedLearningResourceDisplay", async () => {
    ...
    assert.ok(wrapper.find(ExpandedLearningResourceDisplay).prop("hideSimilarLearningResources"));
  });
});