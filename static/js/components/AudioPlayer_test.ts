...
import { INITIAL_AUDIO_STATE, setAudioPlayerState, setCurrentlyPlayingAudio } from "../reducers/audio";
...
describe("AudioPlayer", () => {
  ...
  const exampleAudio = {
    ...
    url: "https://chtbl.com/track/F9DD6B/cdn.simplecast.com/audio/2c64ac/2c64ace6-baf4-4e86-b527-445e611e6a31/aa0ca88f-3c4c-4d33-9897-36e45c43e012/film-is-for-everyone-with-prof-david-thorburn_tc.mp3"
  };
  const seekElements = ["audio-player-back-ten", "audio-player-forward-thirty", "audio-player-progress"];

  beforeEach(() => {
    helper = new IntegrationTestHelper();
    render = helper.configureReduxQueryRenderer(AudioPlayer);
    sandbox = sinon.createSandbox();
    setSongPlayedPercentageSpy = sandbox.spy(Amplitude, "setSongPlayedPercentage");
  });
  ...
  it("should be hidden if we try and render it without setting audio", async () => {
    ...
    assert.isTrue(wrapper.find(".audio-player-container-outer").hasClass("hidden"));
  });
  ...
  it("should render the component and be visible if audio is passed in", async () => {
    ...
    assert.isFalse(wrapper.find(".audio-player-container-outer").hasClass("hidden"));
  });
  ...
  it("should render a pause icon when the player state is playing", async () => {
    const {
      wrapper
    } = await render({}, [setCurrentlyPlayingAudio(exampleAudio), setAudioPlayerState("playing")]);
    assert.equal(wrapper.find(`.amplitude-play-pause span`).text(), "pause_circle_outline");
  });

  it("should render a play icon when the player state is paused", async () => {
    const {
      wrapper
    } = await render({}, [setCurrentlyPlayingAudio(exampleAudio), setAudioPlayerState("paused")]);
    assert.equal(wrapper.find(`.amplitude-play-pause span`).text(), "play_circle_outline");
  });
