...
import ChannelSettingsLink, { CHANNEL_SETTINGS_MENU_DROPDOWN } from "./ChannelSettingsLink";
...
describe("ChannelSettingsLink", () => {
  ...
  beforeEach(() => {
    ...
    render = props => mount(<Router history={helper.browserHistory}>
          <ChannelSettingsLink history={helper.browserHistory} store={store} channel={channel} {...props} />
        </Router>);
  });
  ...
  describe("dropdown menu", () => {
    [true, false].forEach(isOpen => {
      it(`renders a ${isOpen ? "visible" : "hidden"} menu`, async () => {
        ...
        assert.equal(wrapper.find("OnClickOutside(_DropdownMenu)").exists(), isOpen);
      });
    }) //
    ;[true, false].forEach(isOpen => {
      it(`toggles the menu state to ${isOpen ? "closed" : "open"}`, async () => {
        ...
        assert.equal(store.getActions()[0].type, isOpen ? "HIDE_DROPDOWN" : "SHOW_DROPDOWN");
      });
    });
    ...
    it("has two links in the menu normally", async () => {
      ...
      assert.equal(wrapper.find("li").at(0).find("Link").prop("to"), editChannelBasicURL(channel.name));
