...
const renderEmbedly = embedlyResponse => mount(<Embedly embedly={embedlyResponse} />);
...
describe("Embedly", () => {
  ...
  it("should render generic article-type content sensibly", () => {
    ...
    assert.equal(wrapper.find(".link-summary .description").text(), article.description);
  ...
  ;[true, false].forEach(hasIframe => {
    it(`should render a 'rich' type link ${hasIframe ? "with" : "without"} an iframe`, () => {
      const hasIframeStub = sandbox.stub(embedFuncs, "hasIframe").returns(hasIframe);
      const article = makeArticle();
      article.type = "rich";
