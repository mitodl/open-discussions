...
describe("Navigation", () => {
  let sandbox, userIsAnonymousStub, defaultProps: Object, userCanPostStub, renderComponent;
  ...
  ;[[null, true], ["/not/home", false]].forEach(([pathnameProp, expHighlighted]) => {
    it(`${shouldIf(expHighlighted)} highlight the home link if path=${String(pathnameProp)}`, () => {
      const wrapper = renderComponent(pathnameProp ? { pathname: pathnameProp } : {});
      assert.equal(wrapper.find(".location.current-location").find(".home-link").exists(), expHighlighted);
    });
  });
  ...
  it("should show a SubscriptionsList", () => {
    ...
    assert.equal(wrapper.find(SubscriptionsList).props().subscribedChannels, channels);
  });
  ...
  it("should pass the current channel down to the SubscriptionsList", () => {
    ...
    assert.equal(wrapper.find(SubscriptionsList).props().currentChannel, "foobar");
  });
});