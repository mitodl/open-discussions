...
describe("LoginTooltip", () => {
  ...
  it("should include login and signup buttons", () => {
    ...
    assert.equal(links.at(0).prop("to")({ pathname }), `/login/?next=${encodeURIComponent(pathname)}`);
    assert.equal(links.at(1).prop("to")({ pathname }), `/signup/?next=${encodeURIComponent(pathname)}`);
  });
});