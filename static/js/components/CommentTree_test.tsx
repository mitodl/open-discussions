...
describe("CommentTree", () => {
  ...
  const renderCommentTree = (props = {}) => mount(<Router store={helper.store} history={helper.browserHistory}>
        <CommentTree comments={comments} processing={false} loadMoreComments={loadMoreCommentsStub} commentPermalink={permalinkFunc} isPrivateChannel={true} post={post} {...props} />
      </Router>);
  ...
  ;[true, false].forEach(isPrivateChannel => {
    it(`should  pass isPrivateChannel = ${String(isPrivateChannel)} down`, () => {
      const comment = renderCommentTree({ isPrivateChannel }).find("Comment").at(0);
  ...
  it("should render replies to a comment as children of that comment", () => {
    ...
    const wrapper = renderCommentTree();
    const firstComment = wrapper.find(".top-level-comment").at(0).find("Comment").at(0);
  ...
  describe("more_comments", () => {
    it("should render a moreComments object at root level", async () => {
      ...
      const moreCommentsDiv = wrapper.find(".top-level-comment > .more-comments");
