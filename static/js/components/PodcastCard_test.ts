...
describe("PodcastCard", () => {
  ...
  it("should render basic stuff", async () => {
    ...
    assert.equal(wrapper.find("Dotdotdot").at(0).props().children, podcast.title);
    assert.equal(wrapper.find("Dotdotdot").at(1).props().children, podcast.offered_by);
    assert.equal(wrapper.find(".podcast-episode-count").text(), `${podcast.episode_count} Episodes`);
    assert.equal(wrapper.find("img").prop("src"), embedlyThumbnail(SETTINGS.embedlyKey, podcast.image_src, PODCAST_IMG_HEIGHT, CAROUSEL_IMG_WIDTH));
  });
