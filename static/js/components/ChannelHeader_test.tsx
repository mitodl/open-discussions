...
describe("ChannelHeader", () => {
  ...
  const render = (props = {}) => shallow(<ChannelHeader channel={channel} isModerator={false} history={history} {...props} />);

  it("renders a channel header", () => {
    const wrapper = render();
    assert.deepEqual(wrapper.find("Connect(ChannelAvatar)").prop("channel"), channel);
  ...
  it("should render children", () => {
    const wrapper = shallow(<ChannelHeader channel={channel} isModerator={false} history={history}>
        <div className="im-a-child" />
      </ChannelHeader>);
    assert.ok(wrapper.find(".im-a-child").exists());
  }) //
  ;[true, false].forEach(hasHeadline => {
    it(`${hasHeadline ? "shows" : "doesn't show"} the headline for the channel`, () => {
  ...
  ;[true, false].forEach(isModerator => {
    it(`${isModerator ? "shows" : "doesn't show"} the moderator edit button`, () => {
