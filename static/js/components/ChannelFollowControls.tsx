...
export class ChannelFollowControls extends React.Component<Props> {
  ...
  render() {
    ...
    return channel.user_is_subscriber ? <React.Fragment>
        <a className="follow-button dropdown-button" onClick={showDropdown}>
          <span>Following</span>
          {isDropdownOpen ? <DropUpArrow /> : <DropDownArrow />}
        </a>
        {isDropdownOpen ? <DropdownMenu closeMenu={hideDropdown} className="channel-follow-dropdown">
            <li>
              <a onClick={this.handleUnfollowClick}>Unfollow channel</a>
            </li>
            {channel.channel_type !== CHANNEL_TYPE_PUBLIC && channel.user_is_contributor && !channel.user_is_moderator ? <li>
                  <a onClick={this.handleLeaveChannelClick}>Leave channel</a>
                </li> : null}
          </DropdownMenu> : null}
      </React.Fragment> : <button className="follow-button" onClick={this.handleFollowClick}>
        Follow
      </button>;
  }
}
...
const mapDispatchToProps = (dispatch: Dispatch<any>, {
  ...
  followChannel: (username: string) => dispatch(actions.channelSubscribers.post(channel.name, username)),
  unfollowChannel: (username: string) => dispatch(actions.channelSubscribers.delete(channel.name, username)),
  leaveChannel: (username: string) => leaveChannel(dispatch, channel.name, username),
  loadChannel: () => dispatch(actions.channels.get(channel.name)),
  loadChannels: () => dispatch(actions.subscribedChannels.get())
});

export default R.compose(connect(mapStateToProps, mapDispatchToProps))(ChannelFollowControls);