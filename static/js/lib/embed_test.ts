...
import { handleTwitterWidgets, hasIframe, loadEmbedlyPlatform, onCardRendered, renderEmbedlyCard } from "./embed";
...
describe("embed utils", () => {
  ...
  ;[[{ response: { provider_name: "Facebook" } }, false], [{ response: { provider_name: "Twitter" } }, true]].forEach(([response, shouldCall]) => {
    it(`should ${shouldCall ? "" : "not "}call the load func when appropriate`, () => {
  ...
  it("hasIframe should check for an iframe!", () => {
    [['<iframe className="iframe-wow"></iframe>', true], ['<div className="iframe">iframe here? never!</div', false]].forEach(([htmlString, exp]) => {
  ...
  describe("initialization", () => {
    ...
    it("loads embedly", () => {
      ...
      assert.equal(firstScript.getAttribute("src"), "http://cdn.embedly.com/widgets/platform.js");

      const callback = embedlyStub.firstCall.args[2];
      assert.equal(callback, onCardRendered);
    }) //
    ;[true, false].forEach(hidesTitle => {
      it(`${hidesTitle ? "adds" : "doesn't add"} a style element to hide the title`, () => {
        ...
        if (hidesTitle) {
          assert.equal(iframe.contentDocument.querySelector("style").innerText, ".hdr { display: none; }");
        } else {
          assert.equal(iframe.contentDocument.querySelectorAll("style").length, 0);
        }
      });
    });
  });
