...
import { EmbedlyResponse } from "../reducers/embedly";
...
export const loadEmbedlyPlatform = () => {
  ...
  if (!document.getElementById(id)) {
    ...
    // $FlowFixMe
    el.async = 1;
    el.src = `${"https:" === document.location.protocol ? "https" : "http"}://cdn.embedly.com/widgets/platform.js`;
...
export const renderEmbedlyCard = (url: string): string => ReactDOMServer.renderToStaticMarkup(<a data-card-chrome="0" data-card-controls="0" data-card-key={SETTINGS.embedlyKey} href={url} className="embedly-card" />);

export const livestreamEventURL = (accountId: number, eventId: number) => `https://livestream.com/accounts/${accountId}/events/${eventId}/player`;