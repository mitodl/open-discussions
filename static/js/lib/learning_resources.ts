...
import { COURSE_ARCHIVED, COURSE_AVAILABLE_NOW, COURSE_CURRENT, COURSE_PRIOR, DATE_FORMAT, LR_TYPE_USERLIST, LR_TYPE_LEARNINGPATH, LR_TYPE_PROGRAM, LR_TYPE_COURSE, platforms } from "./constants";

import { capitalize, emptyOrNil, formatPrice } from "./util";

import { LearningResourceRun, CourseInstructor, CoursePrice } from "../flow/discussionTypes";

export const AVAILABLE_NOW = "availableNow";

export const availabilityLabel = (availability: string | null | undefined) => {
  switch (availability) {
    case COURSE_CURRENT:
...
const runStartDate = (objectRun: LearningResourceRun): moment$Moment => moment(objectRun.best_start_date, DATE_FORMAT);

const runEndDate = (objectRun: LearningResourceRun): moment$Moment => moment(objectRun.best_end_date, DATE_FORMAT);

const compareRuns = (firstRun: LearningResourceRun, secondRun: LearningResourceRun) => runStartDate(firstRun).diff(runStartDate(secondRun), "hours");
...
export const bestRun = (runs: Array<LearningResourceRun>) => {
  ...
  const currentRuns = runs.filter(run => runStartDate(run).isSameOrBefore() && runEndDate(run).isAfter());
  if (!emptyOrNil(currentRuns)) {
    return currentRuns[0];
  }

  // The next future run
  const futureRuns = runs.filter(run => runStartDate(run).isAfter()).sort(compareRuns);
  if (!emptyOrNil(futureRuns)) {
    return futureRuns[0];
  }

  // The most recent run that "ended"
  const mostRecentRuns = runs.filter(run => runStartDate(run).isSameOrBefore()).sort(compareRuns).reverse();
...
export const minPrice = (prices: Array<CoursePrice>, includeDollarSign: boolean = false) => {
  if (emptyOrNil(prices)) {
    return null;
  }
...
export const getStartDate = (object: Object, objectRun: LearningResourceRun) => {
...
export const privacyIcon = (privacyLevel: string) => privacyLevel === "public" ? "lock_open" : "lock";

export const isCoursewareResource = R.contains(R.__, [LR_TYPE_COURSE, LR_TYPE_PROGRAM]);

export const hasCourseList = R.contains(R.__, [LR_TYPE_PROGRAM]);

export const hasListItemsList = R.contains(R.__, [LR_TYPE_USERLIST, LR_TYPE_LEARNINGPATH]);
...
export const isUserList = (objectType: string) => R.contains(objectType, [LR_TYPE_LEARNINGPATH, LR_TYPE_USERLIST]);