...
import { GenericComment } from "../flow/discussionTypes";

/**
 * Returns a Ramda lens to the comment or null if none is found
 */
export const findComment = (commentTree: Array<GenericComment>, commentId: string, parentIndexes: Array<number> = []): Object | null => {
...
export const flattenCommentTree = (comments: any) => comments.reduce((acc, comment) => acc.concat(stripReplies(comment), flattenCommentTree(comment.replies)), []);