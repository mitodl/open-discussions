...
import { LOGIN_URL, LOGIN_PASSWORD_URL, REGISTER_URL, REGISTER_CONFIRM_URL, REGISTER_DETAILS_URL, INACTIVE_USER_URL, FRONTPAGE_URL, SETTINGS_URL, AUTH_REQUIRED_URL, LOGIN_PROVIDER_URL } from "./url";
import { STATE_LOGIN_EMAIL, STATE_LOGIN_PASSWORD, STATE_LOGIN_PROVIDER, STATE_REGISTER_EMAIL, STATE_REGISTER_CONFIRM_SENT, STATE_REGISTER_CONFIRM, STATE_REGISTER_DETAILS, STATE_SUCCESS, STATE_INACTIVE } from "../reducers/auth";
...
export const processAuthResponse = (history: Object, response: AuthResponse) => {
  const {
    state
  } = response;
...
export const isAnonAccessiblePath = (pathname: string): boolean => pathname === AUTH_REQUIRED_URL || pathname.startsWith(SETTINGS_URL);

export const needsAuthedSite = (): boolean => !SETTINGS.authenticated_site.session_url;
