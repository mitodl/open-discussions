...
import { AUTH_REQUIRED_URL, channelURL, editChannelAppearanceURL, editChannelBasicURL, editChannelContributorsURL, editChannelModeratorsURL, FRONTPAGE_URL, newPostURL, postDetailURL, editProfileURL, profileURL, getChannelNameFromPathname, commentPermalink, toQueryString, urlHostname, channelModerationURL, postPermalink, embedlyThumbnail, blankThumbnailUrl, defaultResourceImageURL, embedlyResizeImage, absolutizeURL, getNextParam, channelSearchURL, SITE_SEARCH_URL, COURSE_URL, learningResourcePermalink } from "./url";
import { makePost } from "../factories/posts";
import { makeLearningResource } from "../factories/learning_resources";
import { LR_TYPE_ALL, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE } from "../lib/constants";
...
describe("url helper functions", () => {
  ...
  describe("channelModerationURL", () => {
    it("should return a good URL", () => {
      assert.equal(channelModerationURL("foochannel"), "/manage/c/edit/foochannel/moderation/");
    });
  });

  describe("postDetailURLWithSlug", () => {
    it(`should return a good post URL including slug`, () => {
      assert.equal(postDetailURL("foobar", "23434j3j3", "post-slug"), "/c/foobar/23434j3j3/post-slug");
    });
  });
  ...
  describe("editChannelAppearanceURL", () => {
    it("should return a url to edit channel appearance", () => {
      assert.equal(editChannelAppearanceURL("channel_name"), "/manage/c/edit/channel_name/appearance/");
    });
  });

  describe("editChannelBasicURL", () => {
    it("should return a url to edit basic channel settings", () => {
      assert.equal(editChannelBasicURL("channel_name"), "/manage/c/edit/channel_name/basic/");
    });
  });

  describe("editChannelContributorsURL", () => {
    it("should return a url to edit channel appearance", () => {
      assert.equal(editChannelContributorsURL("channel_name"), "/manage/c/edit/channel_name/members/contributors/");
    });
  });

  describe("editChannelModeratorsURL", () => {
    it("should return a url to edit channel appearance", () => {
      assert.equal(editChannelModeratorsURL("channel_name"), "/manage/c/edit/channel_name/members/moderators/");
    });
  });
  ...
  describe("getChannelNameFromPathname", () => {
    it("should return a channel", () => {
      [["/c/foobar/", "foobar"], ["/c/foobar", "foobar"], ["/c/foobar/baz/", "foobar"], ["/c/foobar_baz/boz", "foobar_baz"], ["/c/foobarbaz9/boz", "foobarbaz9"], ["/c/Foobarbaz9/boz", "Foobarbaz9"], ["/c/fOObar_Baz9/boz", "fOObar_Baz9"], ["/moderation/c/foobar", "foobar"], ["/moderation/c/bippity/boppity", "bippity"], ["/manage/c/edit/FooBar", "FooBar"], ["/manage/c/edit/BippitY/boppity", "BippitY"]].forEach(([url, expectation]) => {
  ...
  describe("commentPermalink with post slug", () => {
    it("should return a comment permalink with post slug", () => {
      assert.equal("/c/channel_name/post_id/post-slug/comment/comment_id/", commentPermalink("channel_name", "post_id", "post-slug", "comment_id"));
    });
  });

  describe("commentPermalink without post slug", () => {
    it("should return a comment permalink without post slug", () => {
      assert.equal("/c/channel_name/post_id/comment/comment_id/", commentPermalink("channel_name", "post_id", null, "comment_id"));
    });
  });
  ...
  describe("urlHostname", () => {
    it("should pull out the hostname", () => {
      [["https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String", "developer.mozilla.org"], ["https://github.com/mitodl/open-discussions", "github.com"], ["https://mobile.github.com/mitodl/open-discussions", "mobile.github.com"], ["https://mail.google.com/", "mail.google.com"], ["https://www.nytimes.com/2017/10/16/us/politics/trump-mcconnell-bannon.html", "nytimes.com"], ["http://www.org/Security", "www.org"], ["http://www.www.org/Security", "www.org"], ["www.mit.edu", "mit.edu"]].forEach(([url, expectation]) => {
  ...
  describe("postPermalink", () => {
    it("should return a good url", () => {
      ...
      assert.ok(url.includes(postDetailURL(post.channel_name, post.id, post.slug)));
    });
  });
  ...
  describe("blankThumbnailUrl", () => {
    it("should return a good blank thumbnail url", () => {
      assert.equal(blankThumbnailUrl(), `${window.location.origin}/static/images/blank.png`);
    });
  });

  describe("defaultResourceImageURL", () => {
    it("should return a default course image url", () => {
      assert.equal(defaultResourceImageURL(), `${window.location.origin}/static/images/default_resource_thumb.jpg`);
    });
  });
  ...
  describe("learningResourcePermalink", () => {
    LR_TYPE_ALL.forEach(objectType => {
      it(`learningResourcePermalink should return a good link for ${objectType}`, () => {
        ...
        const isPodcastObject = object.object_type === LR_TYPE_PODCAST || object.object_type === LR_TYPE_PODCAST_EPISODE;
        assert.equal(learningResourcePermalink(object), `${window.location.origin}/${isPodcastObject ? "podcasts" : "learn/"}?lr_id=${object.id}&type=${object.object_type}`);
      });
    });
  });
});