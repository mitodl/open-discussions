...
import { validation, validate, validatePostCreateForm, validateChannelAppearanceEditForm, validateChannelBasicEditForm, validateContentReportForm, validateEmailForm, validateNewEmailForm, validateMembersForm, validatePasswordForm, validatePasswordResetForm, validatePasswordChangeForm, PASSWORD_LENGTH_MINIMUM, validNotMIT, validateSearchQuery, validateWidgetDialog, validateCreateUserListForm } from "./validation";
import { WIDGET_CREATE, WIDGET_EDIT, WIDGET_TYPE_SELECT } from "../components/widgets/WidgetEditDialog";
import { makeWidgetInstance } from "../factories/widgets";
import { WIDGET_TYPE_RSS, WIDGET_TYPE_URL, LR_TYPE_USERLIST, LR_PUBLIC } from "../lib/constants";
...
describe("validation library", () => {
  describe("validation", () => {
    it("returns a Just if the validation passes", () => {
      const result = validation(R.equals("potato"), R.lensPath(["foo", "bar", "baz"]), "SHUD BE POTATO YUP!", { foo: { bar: { baz: "potato" } } });
    ...
    it("returns Nothing if the validation doesnt pass", () => {
      assertIsNothing(validation(R.equals("not equal"), R.lensPath(["wait", "up"]), "These should be not equal", { wait: { up: "equal!" } }));
    });
  });
  ...
  describe("validate", () => {
    let validations;
    beforeEach(() => {
      validations = [validation(R.equals("beep"), R.lensProp("topLevel"), "top-level validation failed"), validation(R.isEmpty, R.lensPath(["nested", "prop"]), "simple nested validation failed"), validation(R.equals("blippy boop doop"), R.lensPath(["nested", "array", 2]), "nested array validation failed")];
    });

    it("should take an array of validations and return Just if they apply", () => {
      const curriedValidate = validate([validation(R.equals("hey"), R.lensProp("topLevel"), "validation failed")]);
  ...
  describe("validatePostCreateForm", () => {
    ...
    ;[[[], true], [[{ attributes: {}, children: [] }], true], [[{ attributes: {}, children: [{ hey: "there" }] }], false]].forEach(([articleContent, shouldGiveError]) => {
      it(`${shouldIf(shouldGiveError)} if articleContent = ${JSON.stringify(articleContent)}`, () => {
        const post = {
          value: {
        ...
        assert.deepEqual(validatePostCreateForm(post), shouldGiveError ? {
          value: {
  ...
  describe("validateChannelBasicEditForm", () => {
    it("should complain about no link types being selected", () => {
      ...
      assert.deepEqual(validateChannelBasicEditForm(channel), {
        value: {
          allowed_post_types: "At least one of the post type options must be selected"
        }
      });
  ...
  describe("validNotMIT", () => {
    [["user@mit.edu", true], ["user@MIT.EDU", true], ["user@test.mit.edu", true], ["user@TEST.MIT.edu", true], ["user@ALUM.MIT.edu", false], ["user@alum.mit.edu", false], ["user@other.alum.mit.edu", false], ["user@other.school.mit.edu", true], ["user@aluminum.mit.edu", true], ["user@amit.edu", false], ["user@mit.eduu", false], ["user@mit.com", false]].forEach(([email, invalid]) => {
  ...
  describe("email validator", () => {
    [[validateEmailForm, "existing email", "", "Email is required"], [validateEmailForm, "existing email", "abbbb@ddd", "Email is not formatted correctly"], [validateNewEmailForm, "new email", "", "Email is required"], [validateNewEmailForm, "new email", "abbbb@ddd", "Email is not formatted correctly"], [validateNewEmailForm, "new email", "user@mit.edu", "MIT users please login with Touchstone below"]].forEach(([validatorFunc, validatorDesc, emailInput, expectedErrorMsg]) => {
  ...
  describe("validateSearchQuery", () => {
    [["", false], ["a", false], ["an", true], [null, false], ["a much longer search", true]].forEach(([query, isValid]) => {
  ...
  describe("validateWidgetDialog", () => {
    ...
    ;[true, false].forEach(hasOnlyTwitterURLS => {
      it(`${shouldIf(!hasOnlyTwitterURLS)} yell when non-twitter urls ${!hasOnlyTwitterURLS ? "present" : "not present"} in custom_html`, () => {
        const data = {
          state: WIDGET_CREATE,
          instance: makeWidgetInstance(WIDGET_TYPE_URL),
          validation: {}
        ...
        assert.deepEqual(validateWidgetDialog(data), hasOnlyTwitterURLS ? {} : {
          configuration: {
