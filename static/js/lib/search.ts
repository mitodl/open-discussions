...
import R from "ramda";
import { LR_TYPE_COURSE, LR_TYPE_PROGRAM, LR_TYPE_USERLIST, LR_TYPE_VIDEO, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE, LR_TYPE_LEARNINGPATH, PHONE, TABLET, DESKTOP, LR_TYPE_ALL } from "./constants";
import { SEARCH_FILTER_COMMENT, SEARCH_FILTER_POST, SEARCH_FILTER_PROFILE } from "./picker";

import { CommentInTree, LearningResourceSummary, Post, Profile } from "../flow/discussionTypes";
import { PostResult, CommentResult, ProfileResult, SearchParams, FacetResult, LearningResourceResult } from "../flow/searchTypes";

export const searchResultToComment = (result: CommentResult): CommentInTree => ({
...
export const searchResultToLearningResource = (result: LearningResourceResult, overrideObject: LearningResourceSummary = {}): LearningResourceSummary => ({
  id: result.id,
  title: result.title,
  ...
  offered_by: "offered_by" in result && result.offered_by ? result.offered_by : [],
...
const POST_QUERY_FIELDS = ["text.english", "post_title.english", "plain_text.english"];
const COMMENT_QUERY_FIELDS = ["text.english"];
const PROFILE_QUERY_FIELDS = ["author_headline.english", "author_bio.english", "author_name.english"];
const COURSE_QUERY_FIELDS = ["title.english^3", "short_description.english^2", "full_description.english", "topics", "platform", "course_id", "coursenum^5", "offered_by"];
const VIDEO_QUERY_FIELDS = ["title.english^3", "short_description.english^2", "full_description.english", "transcript.english^2", "topics", "platform", "video_id", "offered_by"];

export const RESOURCE_QUERY_NESTED_FIELDS = ["runs.year", "runs.semester", "runs.level", "runs.instructors^5"];

const LIST_QUERY_FIELDS = ["title.english^3", "short_description.english^2", "topics"];

const PODCAST_QUERY_FIELDS = ["title.english^3", "short_description.english^2", "full_description.english", "topics"];

const PODCAST_EPISODE_QUERY_FIELDS = ["title.english^3", "short_description.english^2", "full_description.english", "topics", "series_title^2"];
...
const _searchFields = (type: string | null | undefined) => {
  ...
  } else if ([LR_TYPE_PROGRAM, LR_TYPE_USERLIST, LR_TYPE_LEARNINGPATH].includes(type)) {
    return LIST_QUERY_FIELDS;
  } else {
    return R.uniq([...POST_QUERY_FIELDS, ...COMMENT_QUERY_FIELDS, ...PROFILE_QUERY_FIELDS]);
  }
};
...
const _channelField = (type: string | null | undefined) => {
  if (type === "post") {
...
const getTypes = (type: (string | Array<string>) | null | undefined) => {
  if (type) {
...
export const buildSearchQuery = ({
  ...
  return emptyOrNil(R.intersection(LR_TYPE_ALL, types)) ? buildChannelQuery(builder, text, types, channelName) : buildLearnQuery(builder, text, types, facets);
};

export const buildFacetSubQuery = (facets: Map<string, Array<string>> | null | undefined, builder: any) => {
  const facetClauses = [];
  if (facets) {
    facets.forEach((values, key) => {
      ...
      if (facetClausesForFacet.length > 0) {
        ...
        builder.agg("filter", key, aggregation => {
          return aggregation.orFilter("bool", filter).agg("terms", key === OBJECT_TYPE ? "object_type.keyword" : key, { size: 10000 }, key);
        });
      } else {
...
const addFacetClauseToArray = (facetClauses: Array<any>, facet: string, values: Array<string>) => {
  if (facet === OBJECT_TYPE && values.toString() === buildSearchQuery.toString()) {
    return;
  }
...
export const buildSuggestQuery = (text: string, suggestFields: Array<string>) => {
  const suggest = {
    text
  };
  suggestFields.forEach(field => // $FlowFixMe: yes the fields are missing and I'm adding them
  suggest[field] = {
    phrase: {
      field: `${field}`,
      size: 5,
...
export const buildDefaultSort = () => {
  return [{ minimum_price: { order: "asc" } }, { default_search_priority: { order: "desc" } }, { created: { order: "desc" } }];
};

export const buildOrQuery = (builder: any, searchType: string, textQuery: any, extraClauses: any) => {
  ...
  builder = builder.orQuery("bool", {
    filter: {
      bool: {
        ...
        }, ...extraClauses, // Add multimatch text query here to filter out non-matching results
        ...textFilter]
      }
    },
...
export const buildChannelQuery = (builder: any, text: string | null | undefined, types: Array<string>, channelName: string | null | undefined) => {
  ...
  if (!emptyOrNil(text)) {
    builder = builder.rawOption("suggest", // $FlowFixMe: if we get this far, text is not null
...
export const buildLearnQuery = (builder: any, text: string | null | undefined, types: Array<string>, facets: Map<string, Array<string>> | null | undefined) => {
  for (const type of LR_TYPE_ALL) {
    ...
    const textQuery = emptyOrNil(text) ? {} : {
      ...
      } : null, type === LR_TYPE_COURSE && SETTINGS.file_search_enabled ? {
        has_child: {
          type: "resourcefile",
          query: {
            [queryType]: {
      ...
      } : null].filter(clause => clause !== null)
    };
...
export const mergeFacetResults = (...args: Array<FacetResult>) => ({
  buckets: args.map(R.prop("buckets")).reduce((buckets, acc) => R.unionWith(R.eqBy(R.prop("key")), buckets, acc))
});
