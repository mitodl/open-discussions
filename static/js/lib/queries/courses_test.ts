...
import { courseListRequestFactory, courseRequest, favoriteCourseMutation } from "./courses";
...
describe("Course Queries", () => {
  ...
  it("courseRequest allows fetching a course", () => {
    ...
    assert.equal(request.url, courseDetailApiURL.param({ courseId: "fake-id" }).toString());
  ...
  ;[true, false].forEach(isFavorite => {
    it(`courseFavoriteMutation does the right stuff if ${isFavorite ? "is" : "is not"} favorite`, () => {
      const course = makeCourse();
      course.is_favorite = isFavorite;
      const mutation = favoriteCourseMutation(course);
      assert.equal(mutation.url, `${courseDetailApiURL.param({ courseId: course.id }).toString()}${isFavorite ? "unfavorite" : "favorite"}/`);
  ...
  describe("courseListRequestFactory", () => {
    ...
    beforeEach(() => {
      [request, selector] = courseListRequestFactory("/my/api/url", "bestCourses");
    });
    ...
    it("request.transform should return the right structure", () => {
      assert.deepEqual(request().transform(transformTestObject), {
        courses: constructIdMap(results.map(result => R.merge({ object_type: LR_TYPE_COURSE }, result))),
    ...
    it("selector should grab the right stuff", () => {
      ...
      assert.deepEqual(selector({ entities: testState }), results.map(result => R.merge({ object_type: LR_TYPE_COURSE }, result)));
    });
  });
});