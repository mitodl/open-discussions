...
import { podcastRequest, podcastEpisodeRequest, favoritePodcastMutation, favoritePodcastEpisodeMutation } from "./podcasts";
...
describe("Podcasts API", () => {
  ...
  it("podcast request allows fetching a podcast", () => {
    ...
    assert.equal(request.url, podcastDetailApiURL.param({ podcastId: "fake-id" }).toString());
  ...
  it("podcast episode request allows fetching a podcast episode", () => {
    ...
    assert.equal(request.url, podcastEpisodeDetailApiURL.param({ episodeId: "fake-id" }).toString());
    assert.deepEqual(request.transform({ id: "foobar" }), {
      podcastEpisodes: {
        foobar: { id: "foobar" }
      }
  ...
  ;[true, false].forEach(isFavorite => {
    podcast = makePodcast();
    it(`podcastFavoriteMutation works correcly if ${isIf(isFavorite)} favorite`, () => {
      podcast.is_favorite = isFavorite;
      const mutation = favoritePodcastMutation(podcast);
      assert.equal(mutation.url, `${podcastDetailApiURL.param({ podcastId: podcast.id }).toString()}${isFavorite ? "/unfavorite" : "/favorite"}/`);
    ...
    it(`podcastFavoriteMutation works correcly if ${isIf(isFavorite)} favorite`, () => {
      ...
      assert.equal(mutation.url, `${podcastEpisodeDetailApiURL.param({ episodeId: podcastEpisode.id }).toString()}${isFavorite ? "/unfavorite" : "/favorite"}/`);
      assert.deepEqual(mutation.transform(), {
        podcastEpisodes: {
          [podcastEpisode.id]: {
            ...podcastEpisode,
