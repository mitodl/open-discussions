...
import { Video } from "../../flow/discussionTypes";
...
export const videosRequest = () => ({
  ...
  transform: (body: {results: Array<Video>;} | null | undefined) => ({
...
export const videosSelector = createSelector(state => state.entities.videos, videos => videos);

export const newVideosSelector = createSelector(state => state.entities.videos, state => state.entities.newVideos, (videos, newVideos) => newVideos && videos ? newVideos.map(id => videos[id]) : null);

export const favoriteVideoMutation = (video: Video) => ({
  queryKey: "videoMutation",
  url: `${videoDetailApiURL.param({ videoId: video.id }).toString()}${video.is_favorite ? "unfavorite" : "favorite"}/`,
  transform: () => {
    const updatedvideo = {
      ...video,
