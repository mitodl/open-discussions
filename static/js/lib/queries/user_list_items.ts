...
import { OBJECT_TYPE_ENTITY_ROUTING, normalizeResourcesByObjectType, updateLearningResources, learningResourceSelector } from "./learning_resources";

import { ListItem, ListItemMember, LearningResourceSummary } from "../../flow/discussionTypes";
import { PaginatedResponse } from "../../flow/restTypes";
...
export const normalizeListItemsByObjectType = R.compose(normalizeResourcesByObjectType, R.map(R.prop("content_data")));
...
export const transformListItem = R.curry((userListId: number, item: ListItem): ListItemMember => ({
...
export const useUserListItemsSelector = (userListId: number) => {
  const selector = useMemo(() => {
    ...
    return createSelector(learningResourceSelector, state => state.entities[`${key}.items`] || [], state => state.entities[`${key}.count`] || 0, (lrSelector, items, count) => {
      return {
        items: items.map((item: ListItemMember) => ({
...
export const useUserListItemsPageSelector = (userListId: number, begin: number, end: number) => {
  const itemsSelector = useUserListItemsSelector(userListId);
  return useMemo(() => createSelector(itemsSelector, ({
    items,
    count
  }) => ({
...
export const userListItemsRequest = (userListId: number, offset: number, limit: number) => {
  const key = userListItemsKey(userListId);
  return {
    ...
    body: { offset, limit },
    force: true,
    transform: (body: PaginatedResponse<ListItem> | null | undefined) => {
      return body ? {
...
export const createUserListItemMutation = (userListId: number, resource: LearningResourceSummary) => {
  const key = userListItemsKey(userListId);
  ...
  return {
    ...
    url: userListItemsApiURL.param({ userListId }).toString(),
    transform: (item: ListItem | null | undefined) => ({
      [entityKey]: item ? transformListItem(userListId, item) : null
    }),
    update: {
      [entityKey]: (prev, next: ListItemMember | null | undefined) => {
        return next ? R.evolve({
          [resource.id]: {
            lists: R.map(R.ifElse(R.propEq("list_id", userListId), R.always(next), R.identity))
          }
        })(prev) : prev;
      }
    },
...
export const updateUserListItemPositionMutation = (userListId: number, itemId: number, oldPosition: number, newPosition: number) => {
  const key = userListItemsKey(userListId);
  return {
...
export const deleteUserListItemMutation = (userListId: number, item: ListItemMember) => {
  const key = userListItemsKey(userListId);
  ...
  return {
    ...
    url: userListItemsDetailApiURL.param({ userListId, itemId: item.item_id }).toString(),
    optimisticUpdate: {
      [entityKey]: R.evolve({
        [item.object_id]: {
          lists: evictItem
