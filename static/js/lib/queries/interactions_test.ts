...
import { interactionMutation, popularContentRequest, popularContentSelector } from "./interactions";
...
import { makeVideo, makeCourse, makeUserList, makeProgram, makeLearningPath } from "../../factories/learning_resources";
...
describe("Interactions API", () => {
  ...
  it("popularContentSelector should return the popular content in order", () => {
    ...
    assert.deepEqual(popularContentSelector({
      entities: {
        courses: {
          [course.id]: course
        },
  ...
  it("popularContentRequest allows fetching of popular resources", () => {
    ...
    const results = [course, video, program, userList, learningPath, podcast, podcastEpisode];
