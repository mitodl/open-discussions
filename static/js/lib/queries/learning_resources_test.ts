...
import { filterFavorites, similarResourcesRequest, learningResourceSelector, normalizeResourcesByObjectType, mapResourcesToResourceRefs, filterObjectType } from "./learning_resources";
import { makeCourse, makeProgram, makeLearningResource, makeUserList, makeVideo } from "../../factories/learning_resources";
import { makePodcast, makePodcastEpisode } from "../../factories/podcasts";
import { LR_TYPE_COURSE, LR_TYPE_ALL, LR_TYPE_LEARNINGPATH, LR_TYPE_USERLIST, LR_TYPE_VIDEO, LR_TYPE_PROGRAM, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE } from "../constants";
...
describe("learning resource queries", () => {
  ...
  beforeEach(() => {
    favorites = R.flatten([[makeCourse, LR_TYPE_COURSE], [makeProgram, LR_TYPE_PROGRAM], [makeUserList, LR_TYPE_USERLIST], [makeVideo, LR_TYPE_VIDEO], [makePodcast, LR_TYPE_PODCAST], [makePodcastEpisode, LR_TYPE_PODCAST_EPISODE]].map(([factory, LRType]) => [{
  ...
  it("mapResourcesToResourceRefs should map learning resources to a ref object", () => {
    assert.deepEqual(mapResourcesToResourceRefs([{
      id: 1,
      object_type: "course"
    }, {
      id: 2,
  ...
  it("filterObjectType returns only objects of the specified object_type when object_type is a string", () => {
    assert.deepEqual(filterObjectType([{ object_type: "abc" }, { object_type: "def" }], "abc"), [{ object_type: "abc" }]);
  });

  it("filterObjectType returns only objects of the specified object_type when object_type is and array", () => {
    assert.deepEqual(filterObjectType([{ object_type: "abc" }, { object_type: "def" }, { object_type: "ghi" }], ["abc", "ghi"]), [{ object_type: "abc" }, { object_type: "ghi" }]);
  });
  ...
  it("should filter out null content_data objects", () => {
    ...
    assert.notInclude(filterFavorites(favorites, LR_TYPE_COURSE).map(item => item.id), favorites[3].id);
  });
  ...
  LR_TYPE_ALL.forEach(resourceType => {
    it(`similarResourcesRequest allows fetching similar resources for a ${resourceType}`, () => {
      ...
      const objectType = resourceType === LR_TYPE_LEARNINGPATH ? LR_TYPE_USERLIST : resourceType;
