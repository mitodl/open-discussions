...
import { userListItemsRequest, createUserListItemMutation, updateUserListItemPositionMutation, deleteUserListItemMutation, normalizeListItemsByObjectType, transformListItem } from "./user_list_items";
import { OBJECT_TYPE_ENTITY_ROUTING } from "./learning_resources";
import { makeUserListItems, makeUserListItem, makeUserList } from "../../factories/learning_resources";
import { userListItemsApiURL, userListItemsDetailApiURL } from "../url";
import { LR_TYPE_ALL, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE, OBJECT_TYPE_MAPPING } from "../constants";
...
describe("UserList Items API", () => {
  ...
  describe("userlistItemsRequest", () => {
    it("allows fetching a user list's items", () => {
      ...
      assert.equal(request.url, userListItemsApiURL.param({ userListId }).toString());
  ...
  describe("createUserListItemMutation", () => {
    ...
    LR_TYPE_ALL.filter(objectType => objectType !== LR_TYPE_PODCAST && objectType !== LR_TYPE_PODCAST_EPISODE).forEach(objectType => {
  ...
  describe("updateUserListItemPositionMutation", () => {
    ...
    it("allows reordering user list items", () => {
      assert.equal(request.url, userListItemsDetailApiURL.param({ userListId, itemId: 1 }).toString());
  ...
  describe("deleteUserListItemMutation", () => {
    it("allows deleting a user list item", () => {
      ...
      assert.equal(request.url, userListItemsDetailApiURL.param({ userListId, itemId }).toString());
      assert.equal(request.options.method, "DELETE");
    });

    // TEMPORARY! when we add full support for podcasts and podcast episodes
    // in user lists we can change this
    LR_TYPE_ALL.filter(objectType => objectType !== LR_TYPE_PODCAST && objectType !== LR_TYPE_PODCAST_EPISODE).forEach(contentType => {
