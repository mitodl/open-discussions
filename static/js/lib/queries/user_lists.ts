...
export const userListsRequest = () => ({
  ...
  transform: (body: {results: Array<UserList>;} | null | undefined) => ({
...
export const userListsSelector = createSelector(state => state.entities.userLists, userLists => userLists);

export const myUserListsSelector = createSelector(userListsSelector, userLists => userLists ? Object.values(userLists).filter((userList: any) => userList.author === SETTINGS.user_id) : []);

export const favoriteUserListMutation = (userList: UserList) => ({
  queryKey: "userListMutation",
  url: `${userListDetailApiURL.param({ userListId: userList.id }).toString()}${userList.is_favorite ? "unfavorite" : "favorite"}/`,
  transform: () => {
...
export const createUserListMutation = (params: Object) => ({
  ...
  body: params,
  url: userListApiURL.toString(),
  transform: (newUserList: UserList | null | undefined) => {
    return newUserList ? {
      userLists: {
