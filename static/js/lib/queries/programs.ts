...
import { Program } from "../../flow/discussionTypes";
import { createSelector } from "reselect";

export const programsSelector = createSelector(state => state.entities.programs, programs => programs);
...
export const favoriteProgramMutation = (program: Program) => ({
  ...
  url: `${programDetailApiURL.param({ programId: program.id }).toString()}${program.is_favorite ? "unfavorite" : "favorite"}/`,
  transform: () => {
