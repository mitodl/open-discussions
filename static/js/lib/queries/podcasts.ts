...
import { podcastApiURL, recentPodcastApiURL, podcastDetailApiURL, podcastEpisodeDetailApiURL, podcastDetailEpisodesApiURL } from "../url";

import { DEFAULT_POST_OPTIONS, constructIdMap } from "../redux_query";

import { Podcast, PodcastEpisode } from "../../flow/podcastTypes";
...
export const podcastsSelector = createSelector(state => state.entities.podcasts, podcasts => podcasts);
...
export const recentEpisodesSelector = createSelector(state => state.entities.podcastEpisodes, state => state.entities.recentEpisodes, (episodes, recentEpisodes) => episodes && recentEpisodes ? recentEpisodes.map(id => episodes[id]) : []);

export const podcastEpisodesSelector = createSelector(state => state.entities.podcastEpisodes, podcastEpisodes => podcastEpisodes);

export const favoritePodcastMutation = (podcast: Podcast) => ({
  queryKey: "podcastMutation",
  url: `${podcastDetailApiURL.param({ podcastId: podcast.id }).toString()}${podcast.is_favorite ? "/unfavorite" : "/favorite"}/`,
  transform: () => {
...
export const favoritePodcastEpisodeMutation = (podcastEpisode: PodcastEpisode) => ({
  queryKey: "podcastEpisodeMutation",
  url: `${podcastEpisodeDetailApiURL.param({ episodeId: podcastEpisode.id }).toString()}${podcastEpisode.is_favorite ? "/unfavorite" : "/favorite"}/`,
  transform: () => {
...
export const podcastEpisodesKey = (podcastId: number) => `podcast${podcastId}episodes`;

export const podcastEpisodesRequest = (podcastId: number, offset: number, limit: number) => {
