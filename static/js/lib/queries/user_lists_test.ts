...
import { userListRequest, favoriteUserListMutation, createUserListMutation, deleteUserListMutation, userListMutation, userListsSelector, myUserListsSelector } from "./user_lists";
...
describe("UserLists API", () => {
  ...
  it("userList request allows fetching a userList", () => {
    ...
    assert.equal(request.url, userListDetailApiURL.param({ userListId: 45 }).toString());
  ...
  ;[true, false].forEach(isFavorite => {
    it(`userListFavoriteMutation does the right stuff if ${isFavorite ? "is" : "is not"} favorite`, () => {
      userList.is_favorite = isFavorite;
      const mutation = favoriteUserListMutation(userList);
      assert.equal(mutation.url, `${userListDetailApiURL.param({ userListId: userList.id }).toString()}${isFavorite ? "unfavorite" : "favorite"}/`);
  ...
  it("deleteUserListMutation should return what we expect", () => {
    ...
    assert.equal(query.url, userListDetailApiURL.param({ userListId: userList.id }).toString());
    assert.deepEqual(query.options, {
      method: "DELETE",
      ...DEFAULT_POST_OPTIONS
    });
    assert.deepEqual(query.update.userLists({
      [userList.id]: userList
    }), {});
  });
  ...
  it("myUserListsSelector should grab only user's userList entities", () => {
    ...
    assert.deepEqual(myUserListsSelector(testState), Object.keys(results).map(key => results[key]).filter(result => result.author === author));
  });
});