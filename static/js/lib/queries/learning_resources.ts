...
import { LR_TYPE_COURSE, LR_TYPE_PROGRAM, LR_TYPE_USERLIST, LR_TYPE_VIDEO, LR_TYPE_LEARNINGPATH, LR_TYPE_PODCAST, LR_TYPE_PODCAST_EPISODE } from "../constants";
...
export const normalizeResourcesByObjectType = R.compose(R.map(constructIdMap), renameKeys(OBJECT_TYPE_ENTITY_ROUTING), R.groupBy(R.prop("object_type")));
...
export const mapResourcesToResourceRefs = R.map(R.compose(renameKeys({
  id: "object_id",
  object_type: "content_type"
}), R.pick(["id", "object_type"])));

export const filterObjectType = (results: Array<Object>, objectType: string | Array<string>): Array<Object> => {
...
export const filterFavorites = (results: Array<Object>, contentType: string): Array<Object> => results.filter(result => result.content_data !== null).filter(result => result.content_type === contentType).map(({
  content_data,
  content_type
...
export const favoritesRequest = () => ({
  ...
  transform: (responseJson: Object) => {
    ...
    return {
      ...
      podcastEpisodes: constructIdMap(filterFavorites(results, LR_TYPE_PODCAST_EPISODE)),
      next: next
    };
  },
  update: {
...
export const getSimilarResources = createSelector(state => state.entities.similarResources, similarResources => similarResources);

const filterFavorite = (entities: Object) => R.filter(R.propEq("is_favorite", true), entities || {});

export const favoritesSelector = createSelector(state => state.entities.courses, state => state.entities.programs, state => state.entities.userLists, state => state.entities.videos, state => state.entities.podcasts, state => state.entities.podcastEpisodes, (courses, programs, userLists, videos, podcasts, podcastEpisodes) => ({
...
export const favoritesListSelector = createSelector(favoritesSelector, ({
  courses,
  programs,
  userLists,
  videos,
  podcasts,
  podcastEpisodes
}) => [...Object.values(courses), ...Object.values(programs), ...Object.values(userLists), ...Object.values(videos), ...Object.values(podcasts), ...Object.values(podcastEpisodes)]);

export const getResourceRequest = (objectId: number | null | undefined, objectType: string | null | undefined) => {
  if (!objectId) {
    return null;
  }
...
export const learningResourceSelector = createSelector(state => state.entities.courses, state => state.entities.programs, state => state.entities.userLists, state => state.entities.videos, state => state.entities.podcasts, state => state.entities.podcastEpisodes, (courses, programs, userLists, videos, podcasts, podcastEpisodes) => memoize((objectId, objectType) => {
  switch (objectType) {
    case LR_TYPE_COURSE:
