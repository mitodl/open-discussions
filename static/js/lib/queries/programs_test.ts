...
describe("Programs API", () => {
  ...
  it("program request allows fetching a program", () => {
    ...
    assert.equal(request.url, programDetailApiURL.param({ programId: "fake-id" }).toString());
  ...
  ;[true, false].forEach(isFavorite => {
    it(`programFavoriteMutation does the right stuff if ${isFavorite ? "is" : "is not"} favorite`, () => {
      program.is_favorite = isFavorite;
      const mutation = favoriteProgramMutation(program);
      assert.equal(mutation.url, `${programDetailApiURL.param({ programId: program.id }).toString()}${isFavorite ? "unfavorite" : "favorite"}/`);
