...
describe("Videos API", () => {
  ...
  it("video request allows fetching a video", () => {
    ...
    assert.equal(request.url, videoDetailApiURL.param({ videoId: "fake-id" }).toString());
  ...
  ;[true, false].forEach(isFavorite => {
    it(`videoFavoriteMutation does the right stuff if ${isFavorite ? "is" : "is not"} favorite`, () => {
      video.is_favorite = isFavorite;
      const mutation = favoriteVideoMutation(video);
      assert.equal(mutation.url, `${videoDetailApiURL.param({ videoId: video.id }).toString()}${isFavorite ? "unfavorite" : "favorite"}/`);
