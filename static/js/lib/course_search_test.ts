...
describe("course search library", () => {
  describe("deserializeSearchParams", () => {
    it("should deserialize text from the URL", () => {
      assert.deepEqual(deserializeSearchParams({ search: "q=The Best Course" }), {
        activeFacets: {
          audience: [],
          certification: [],
          offered_by: [],
    ...
    it("should deserialize topics from the URL", () => {
      assert.deepEqual(deserializeSearchParams({
        search: "t=Science&t=Physics&t=Chemistry&t=Computer%20Science&t=Electronics"
      }), {
        activeFacets: {
          audience: [],
          certification: [],
          offered_by: [],
          topics: ["Science", "Physics", "Chemistry", "Computer Science", "Electronics"],
          type: []
        },
        text: undefined
      });
    ...
    it("should deserialize audience from the URL", () => {
      assert.deepEqual(deserializeSearchParams({ search: "a=Open%20Content" }), {
        activeFacets: {
  ...
  describe("serializeSearchParams", () => {
    it("should serialize text to URL", () => {
      assert.deepEqual(serializeSearchParams({
        text: "my search text",
        activeFacets: {}
