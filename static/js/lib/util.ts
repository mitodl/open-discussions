...
export const getCommentID = (props: {match: Match;}): string | void => props.match.params.commentID || undefined;

export const getUserName = (props: {match: Match;}): string => props.match.params.userName || "";
...
export const preventDefaultAndInvoke = R.curry((invokee: Function, e: Event) => {
  if (e) {
    e.preventDefault();
  }
  invokee();
...
export function isProfileComplete(profile: Profile | null | undefined): boolean {
  if (!profile || (profile.name && profile.bio && profile.headline && (profile.image_file || profile.image))) {
    return true;
  }
  return false;
}
...
export const isMobileGridWidth = () => getViewportWidth() < GRID_MOBILE_BREAKPOINT;

export const truncate = (text: string | null | undefined, length: number): string => text ? _.truncate(text, { length: length, separator: " " }) : "";
...
export const makeUUID = (len: number) => Array.from(window.crypto.getRandomValues(new Uint8Array(len))).map(int => int.toString(16)).join("").slice(0, len);

export const removeTrailingSlash = (str: string) => str.length > 0 && str[str.length - 1] === "/" ? str.substr(0, str.length - 1) : str;

export const emptyOrNil = R.either(R.isEmpty, R.isNil);
export const allEmptyOrNil = R.all(emptyOrNil);

export const spaceSeparated = (strings: Array<string | null | undefined>): string => strings.filter(str => str).join(" ");
...
export const isValidUrl = (url: string): boolean => _.isString(url) ? isURL(url, { allow_underscores: true, require_protocol: true }) : false;

export const toArray = (obj: any) => Array.isArray(obj) ? obj : obj ? [obj] : undefined;

export const languageName = (langCode: string | null | undefined): string => LocaleCode.getLanguageName(`${langCode ? langCode.split("-")[0].toLowerCase() : "en"}-US`);

export const flatZip = R.compose(R.flatten, R.zip);

export const capitalize = R.converge(R.concat(), [R.compose(R.toUpper, R.head), R.tail]);

export const formatPrice = (price: (string | null | undefined) | number | Decimal): string => {
...
export const sortBy = (property: string) => R.sortWith([R.ascend(R.prop(property))]);

export const isDoubleQuoted = (text: string | null | undefined) => !emptyOrNil(R.match(/^".+"$/, text || ""));