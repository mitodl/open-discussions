...
import { wait, enumerate, isEmptyText, preventDefaultAndInvoke, userIsAnonymous, isProfileComplete, notNil, truncate, getTokenFromUrl, defaultProfileImageUrl, makeUUID, spaceSeparated, isValidUrl, flatZip, formatPrice } from "./util";
...
describe("utility functions", () => {
  ...
  it("isEmptyText works as expected", () => {
    [[" ", true], ["", true], ["\n\t   ", true], ["                   \t ", true], ["foo \n", false], ["foo", false], ["   \n\tfoo", false]].forEach(([text, exp]) => {
      assert.equal(isEmptyText(text), exp);
    });
  });

  it("truncate works as expected", () => {
    [["", ""], [null, ""], ["A random string", "A random string"], ["A random string with many words.", "A random string..."]].forEach(([text, expected]) => {
  ...
  it("isProfileComplete returns false if any required fields are missing", () => {
    [[null, "bio"], [null, "headline"], [null, "image"]].forEach(([bio, headline, image]) => {
      const profile = {
        name: "Test User",
      ...
      assert.equal(isProfileComplete(profile), bio !== null && headline !== null && image !== null);
    });
  });

  it("notNil works as expected", () => {
    [[null, false], [undefined, false], [0, true], ["", true], ["abc", true]].forEach(([val, exp]) => {
      assert.equal(notNil(val), exp);
    });
  });

  it("getTokenFromUrl gets a token value from a url match or the querystring", () => {
    [["url_token", undefined, "url_token"], [undefined, "?token=querystring_token", "querystring_token"], ["url_token", "?token=querystring_token", "url_token"], [undefined, "?not_token=whatever", ""], [undefined, undefined, ""]].forEach(([urlMatchTokenValue, querystringValue, exp]) => {
      const props = {
        match: {
          params: {
  ...
  describe("spaceSeparated", () => {
    it("should return a space separated string when given an array of strings or nulls", () => {
      [[["a", "b", "c"], "a b c"], [[null, null], ""], [[null, "a", "b"], "a b"], [["a", "b", null], "a b"]].forEach(([inputArr, expectedStr]) => {
