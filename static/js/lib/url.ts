...
export const channelAboutURL = (channelName: string) => `/c/${channelName}/about`;

export const channelMembersURL = (channelName: string) => `/c/${channelName}/members`;

export const channelModerationURL = (channelName: string) => `/manage/c/edit/${channelName}/moderation/`;

export const editChannelBasicURL = (channelName: string) => `/manage/c/edit/${channelName}/basic/`;

export const editChannelAppearanceURL = (channelName: string) => `/manage/c/edit/${channelName}/appearance/`;

export const editChannelModeratorsURL = (channelName: string) => `/manage/c/edit/${channelName}/members/moderators/`;

export const editChannelContributorsURL = (channelName: string) => `/manage/c/edit/${channelName}/members/contributors/`;

export const postDetailURL = (channelName: string, postID: string, postSlug?: string) => `/c/${channelName}/${postID}${postSlug ? `/${postSlug}` : ""}`;

export const newPostURL = (channelName: string | null | undefined) => channelName ? `/create_post/${channelName}` : "/create_post/";

export const profileURL = (username: string, selectedTab: string | null | undefined) => selectedTab ? `/profile/${username}/${selectedTab}` : `/profile/${username}/`;

export const editProfileURL = (username: string) => `/profile/${username}/edit`;

export const commentPermalink = R.curry((channelName: string, postID: string, postSlug: string, commentID: string) => `${postDetailURL(channelName, postID, postSlug)}/comment/${commentID}/`);

export const channelSearchURL = (channelName: string) => `/c/${channelName}/search/`;

export const blankThumbnailUrl = () => new URL("/static/images/blank.png", window.location.origin).toString();

export const defaultResourceImageURL = () => new URL("/static/images/default_resource_thumb.jpg", window.location.origin).toString();

export const embedlyThumbnail = (key: string, url: string, height: number, width: number) => `https://i.embed.ly/1/display/crop/?key=${key}&url=${encodeURIComponent(url)}&height=${height}&width=${width}&grow=true&animate=false&errorurl=${blankThumbnailUrl()}`;

export const embedlyResizeImage = (key: string, url: string, height: number) => `https://i.embed.ly/1/display/resize/?key=${key}&url=${encodeURIComponent(url)}&height=${height}&grow=false&animate=false&errorurl=${blankThumbnailUrl()}`;

export const absolutizeURL = (url: string) => new URL(url, window.location.origin).toString();

export const postPermalink = (post: Post): string => absolutizeURL(postDetailURL(post.channel_name, post.id, post.slug));
...
export const getChannelNameFromPathname = R.compose(R.defaultTo(null), R.view(R.lensIndex(2)), R.match(/c\/(edit\/)?([^/]+)\/?/));
...
export const PODCAST_GOOGLE_URL = "https://podcasts.google.com/feed/aHR0cHM6Ly9vcGVuLm1pdC5lZHUvcG9kY2FzdHMvcnNzX2ZlZWQ";

export const toQueryString = (params: Object) => R.isEmpty(params || {}) ? "" : `?${qs.stringify(params)}`;

export const urlHostname = (url: string | null | undefined) => {
  if (!url) {
    return "";
  }
...
export const COURSE_SEARCH_BANNER_URL = "/static/images/course_search_banner.png";
...
export const podcastDetailEpisodesApiURL = podcastDetailApiURL.segment("episodes/");

export const podcastEpisodeApiURL = api.segment("podcastepisodes/");
export const podcastEpisodeDetailApiURL = podcastEpisodeApiURL.segment(":episodeId");

export const userListIndexURL = "/learn/lists/";
export const userListDetailURL = (id: number) => `/learn/lists/${id}`;

export const learningResourcePermalink = (object: Object) => absolutizeURL(`${object.object_type === LR_TYPE_PODCAST || object.object_type === LR_TYPE_PODCAST_EPISODE ? PODCAST_URL : COURSE_URL}${toQueryString({
  lr_id: object.id,
