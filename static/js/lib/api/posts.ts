...
export async function createPost(channelName: string, payload: CreatePostPayload): Promise<Post> {
  ...
  return JSON.parse((await fetchWithAuthFailure(`/api/v0/channels/${channelName}/posts/`, {
    method: "POST",
    body: formData
  })));
}
...
export async function editPost(postId: string, post: Object): Promise<Post> {
  const formData = objectToFormData(R.pick(["text", "title", "stickied", "ignore_reports", "subscribed"], post));
  ...
  return JSON.parse((await fetchWithAuthFailure(`/api/v0/posts/${postId}/`, {
    method: PATCH,
    body: formData
  })));
}
