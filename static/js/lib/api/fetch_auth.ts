...
export const withAuthFailure = (fetchFunc: Function) => async (...args: any) => {
  try {
...
export const fetchJSONWithAuthFailure = withAuthFailure((...args) => fetchJSONWithCSRF(...args));

export const fetchWithAuthFailure = withAuthFailure((...args) => fetchWithCSRF(...args));

// Fetch an api endpoint with an anonymous token
// NOTE: this is NOT a JWT token
export const fetchJSONWithToken = async (url: string, token: string, body: Object | null | undefined = {}) => {
  try {
