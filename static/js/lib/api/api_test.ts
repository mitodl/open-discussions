...
import { getUserPosts, getUserComments, getSettings, patchFrontpageSetting, patchCommentSetting, getProfile, patchProfileImage, updateProfile, postEmailLogin, postPasswordLogin, postEmailRegister, postConfirmRegister, postDetailsRegister, postPasswordResetEmail, postPasswordResetNewPassword, postSetPassword, search, getRelatedPosts } from "./api";
...
describe("api", function () {
  ...
  describe("REST functions", () => {
    ...
    it("updates a profile", async () => {
      ...
      assert.ok(fetchJSONStub.calledWith(`/api/v0/profiles/${profile.username}/`));
    ...
    describe("updates profile image", () => {
      ...
      it("fails to update a user profile image", () => {
        ...
        return assert.isRejected(patchProfileImage(userName, blob, imageName)).then(() => {
          checkArgs();
        });
      });
    });
    ...
    it("gets user posts", async () => {
      ...
      assert.ok(fetchJSONStub.calledWith(`/api/v0/profiles/someuser/posts/?after=def&before=abc&count=5`));
    ...
    it("gets user comment", async () => {
      ...
      assert.ok(fetchJSONStub.calledWith(`/api/v0/profiles/someuser/comments/?after=def&before=abc&count=5`));
    ...
    describe("postEmailLogin", () => {
      it("should pass email", async () => {
        ...
        assert.ok(fetchJSONStub.calledWith("/api/v0/login/email/", {
          method: POST,
    ...
    describe("postPasswordLogin", () => {
      it("should pass password and partial_token", async () => {
        ...
        assert.ok(fetchJSONStub.calledWith("/api/v0/login/password/", {
          method: POST,
          body: JSON.stringify({
            flow,
    ...
    describe("postEmailRegister", () => {
      it("should pass email", async () => {
        ...
        assert.ok(fetchJSONStub.calledWith("/api/v0/register/email/", {
          method: POST,
          body: JSON.stringify({ flow, email })
        }));
      });
    ...
    describe("postConfirmRegister", () => {
      it("should pass code", async () => {
        ...
        assert.ok(fetchJSONStub.calledWith("/api/v0/register/confirm/", {
          method: POST,
          body: JSON.stringify({
            flow,
            partial_token: token,
    ...
    describe("postDetailsRegister", () => {
      it("should pass name, password, and partial_token", async () => {
        ...
        assert.ok(fetchJSONStub.calledWith("/api/v0/register/details/", {
          method: POST,
          body: JSON.stringify({
            flow,
            partial_token: partialToken,
    ...
    describe("postPasswordResetEmail", () => {
      it("should pass email", async () => {
        ...
        assert.ok(fetchJSONStub.calledWith("/api/v0/password_reset/", {
          method: POST,
    ...
    describe("postPasswordResetNewPassword", () => {
      it("should pass password with confirmation, token, and uid", async () => {
        ...
        const uid = "AA";
        await postPasswordResetNewPassword(newPassword, reNewPassword, token, uid);
        assert.ok(fetchJSONStub.calledWith("/api/v0/password_reset/confirm/", {
          method: POST,
          body: JSON.stringify({
    ...
    describe("postSetPassword", () => {
      it("should pass a current and new password", async () => {
        ...
        assert.ok(fetchJSONStub.calledWith("/api/v0/set_password/", {
          method: POST,
          body: JSON.stringify({
    ...
    describe("search", () => {
      it("should execute a search and return results", async () => {
        const body = { a: "body" };
        const buildStub = sandbox.stub(searchFuncs, "buildSearchQuery").returns(body);
  ...
  describe("settings functions", () => {
    ...
    describe("getSettings", () => {
      it("calls fetchJSONWithToken when passed a token", async () => {
        ...
        assert.ok(fetchTokenStub.calledWith("/api/v0/notification_settings/", "mygreattoken"));
      });

      it("calls fetchJSONWithAuthFailure when not passed a token", async () => {
        await getSettings();
        assert.isNotOk(fetchTokenStub.called);
        assert.ok(fetchAuthFailureStub.calledWith("/api/v0/notification_settings/"));
      });
    });
    ...
    describe("patchFrontpageSetting", () => {
      it("should call fetchJSONWithToken when passed a token", async () => {
        ...
        assert.isNotOk(fetchAuthFailureStub.called);
        assert.ok(fetchTokenStub.calledWith("/api/v0/notification_settings/frontpage/", "great token", {
          method: PATCH,
      ...
      it("calls fetchJSONWithAuthFailure when not passed a token", async () => {
        ...
        assert.isNotOk(fetchTokenStub.called);
        assert.ok(fetchAuthFailureStub.calledWith("/api/v0/notification_settings/frontpage/", {
          method: PATCH,
          body: JSON.stringify(setting)
        }));
      });
    ...
    describe("patchCommentSetting", () => {
      it("should call fetchJSONWithToken when passed a token", async () => {
        ...
        assert.isNotOk(fetchAuthFailureStub.called);
        assert.ok(fetchTokenStub.calledWith("/api/v0/notification_settings/comments/", "great token", {
          method: PATCH,
          body: JSON.stringify(setting)
        }));
      });
      ...
      it("calls fetchJSONWithAuthFailure when not passed a token", async () => {
        ...
        assert.isNotOk(fetchTokenStub.called);
        assert.ok(fetchAuthFailureStub.calledWith("/api/v0/notification_settings/comments/", {
          method: PATCH,
          body: JSON.stringify(setting)
        }));
      });
