...
import { NOT_AUTHENTICATED_ERROR_TYPE, AUTHENTICATION_FAILED_ERROR_TYPE } from "../../util/rest";
...
describe("fetch_auth", function () {
  ...
  beforeEach(() => {
    ...
    loginRedirectUrlStub = sandbox.stub(authHelpers, "generateLoginRedirectUrl").returns(fakeRedirectUrl);
  });

  afterEach(() => {
    sandbox.restore();
    fetchMock.restore();
  });[[fetchJSONWithAuthFailure, "fetchJSONWithAuthFailure", "fetchJSONWithCSRF"], [fetchWithAuthFailure, "fetchWithAuthFailure", "fetchWithCSRF"]].forEach(([authFunc, funcName, djangoCSRFFunc]) => {
    describe(funcName, () => {
      beforeEach(() => {
      ...
      for (const [message, error] of [["a 500 error", error500], ["an unexpected exception", typeError]]) {
  ...
  describe("fetchJSONWithToken", () => {
    ...
    it("should return and redirect if 401 error", async () => {
      ...
      const err = await assert.isRejected(fetchJSONWithToken("/beep/boop/", "mygreatsecuretoken=="));
