...
import { getChannel, getChannels, getPostsForChannel, createChannel, updateChannel, getChannelContributors, addChannelContributor, deleteChannelContributor, addChannelInvitation } from "./channels";
import { makeChannel, makeChannelList, makeContributors, makeContributor, makeChannelInvite } from "../../factories/channels";
...
describe("Channels API", () => {
  ...
  it("gets channel posts with pagination params", async () => {
    ...
    assert.ok(fetchJSONStub.calledWith(`/api/v0/channels/channelone/posts/?after=def&before=abc&count=5`));
    assert.deepEqual(result.posts, posts);
  });
  ...
  it("updates a channel", async () => {
    ...
    assert.ok(fetchJSONStub.calledWith(`/api/v0/channels/${input.name}/`, {
      method: PATCH,
      body: JSON.stringify({
        title: input.title,
  ...
  describe("contributors", () => {
    it("gets a list of contributors", async () => {
      ...
      assert.ok(fetchJSONStub.calledWith(`/api/v0/channels/${channelName}/contributors/`));
    });
    ...
    it("adds a contributor", async () => {
      ...
      assert.deepEqual(contributor, (await addChannelContributor(channelName, contributor.email)));
    ...
    it("removes a contributor", async () => {
      ...
      assert.ok(fetchStub.calledWith(`/api/v0/channels/${channelName}/contributors/${contributor.contributor_name}/`));
  ...
  describe("invitations", () => {
    it("adds a channel invite", async () => {
      ...
      assert.deepEqual(invitation, (await addChannelInvitation(channelName, invitation.email)));

      assert.ok(fetchJSONStub.calledWith(`/api/v0/channels/${channelName}/invites/`));
      assert.deepEqual(fetchJSONStub.args[0][1], {
        method: POST,
        body: JSON.stringify({
