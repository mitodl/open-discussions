...
import { getComments, getComment, createComment, updateComment, getMoreComments, deleteComment } from "./comments";
import { makePost } from "../../factories/posts";
import { makeCommentsResponse, makeMoreCommentsResponse } from "../../factories/comments";
...
describe("Channels API", () => {
  ...
  describe("getComments", () => {
    ...
    it("includes the sort parameter when getting comments", async () => {
      ...
      assert.deepEqual(resp, response);
      assert.ok(fetchJSONStub.calledWith(`/api/v0/posts/${post.id}/comments/?sort=new`));
    });
  });
  ...
  describe("retrieves more comments", () => {
    it("at the root level", async () => {
      ...
      assert.ok(fetchJSONStub.calledWith(`/api/v0/morecomments/?${qs.stringify(payload)}`));
