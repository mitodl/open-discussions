...
import { getChannelModerators, addChannelModerator, deleteChannelModerator, reportContent, getReports } from "./moderation";
import { makeModerators, makeModerator } from "../../factories/channels";
import { makeReportRecord } from "../../factories/reports";

import { CommentReport, PostReport } from "../../flow/discussionTypes";
...
describe("Moderation API", () => {
  ...
  it("reports a comment", async () => {
    ...
    assert.ok(fetchJSONStub.calledWith(`/api/v0/reports/`, {
      method: POST,
  ...
  describe("channel moderators", () => {
    it("gets a list of moderator", async () => {
      ...
      assert.ok(fetchJSONStub.calledWith(`/api/v0/channels/${channelName}/moderators/`));
    });
    ...
    it("adds a moderator", async () => {
      ...
      assert.deepEqual(moderator, ( // $FlowFixMe: flowt thinks .email can be undefined
    ...
    it("removes a moderator", async () => {
      ...
      assert.ok(fetchStub.calledWith(`/api/v0/channels/${channelName}/moderators/${moderator.moderator_name}/`));
