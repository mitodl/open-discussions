...
describe("initials", () => {
  it("should return capitalized first letter of first two words", () => {
    [["Test user", "TU"], ["testuser", "T"], ["test Thurston Howell III", "TT"], ["", ""]].forEach(([name, expected]) => {
...
describe("website getter", () => {
  ...
  it("getSocialSites should return all social sites from a Profile object", () => {
    ...
    const expectedSocialTypes = userWebsiteTypes.filter(siteType => siteType !== PERSONAL_SITE_TYPE);
