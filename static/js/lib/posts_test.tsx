...
import { newPostForm, formatCommentsCount, PostTitleAndHostname, formatPostTitle, mapPostListResponse, postFormIsContentless, isPostContainingText, getPlainTextContent, isEditablePostType, getThumbnailSrc } from "./posts";
...
describe("Post utils", () => {
  ...
  it("should let us check if a post form is empty of post content", () => {
    ...
    assert.isFalse(postFormIsContentless({ ...newPostForm(), article: [{ boop: "doop" }] }));
  });

  it("should correctly format comments", () => {
    const post = makePost();[[0, "0 comments"], [1, "1 comment"], [10, "10 comments"]].forEach(([num, expectation]) => {
  ...
  it("should include a domain and link icon for link posts", () => {
    ...
    assert.ok(postTitle.find(".expanded-url-hostname").text().startsWith(`(${urlHostname(post.url)}`));
  });

  describe("postTitleAndHostname", () => {
    const renderPostTitle = post => shallow(<PostTitleAndHostname post={post} />);
  ...
  it("isPostContainingText returns true for posts that contain text", () => {
    ...
    assert.isTrue(isPostContainingText({ ...post, post_type: LINK_TYPE_ARTICLE }));
    assert.isFalse(isPostContainingText({ ...post, post_type: LINK_TYPE_LINK }));
  });

  describe("getPlainTextContent", () => {
    const txt = "magnets, how do they work?";[[LINK_TYPE_LINK, {}, null, "link post"], [LINK_TYPE_ARTICLE, { plain_text: txt }, txt, "article posts"], [LINK_TYPE_TEXT, { plain_text: txt }, txt, "text posts"], [LINK_TYPE_TEXT, { text: txt }, null, "text posts w/ empty plain text"]].forEach(([postType, updatedProperties, expReturnValue, desc]) => {
      it(`${shouldIf(!!expReturnValue)} return some text content for ${desc}`, () => {
        const post = {
  ...
  ;[[LINK_TYPE_LINK, false], [LINK_TYPE_TEXT, true], [LINK_TYPE_ARTICLE, true]].forEach(([postType, expectation]) => {
    it(`isEditablePostType should return ${String(expectation)} when ${postType}`, () => {
      assert.equal(isEditablePostType({ ...makePost(), post_type: postType }), expectation);
    });
  });

  describe("getThumbnailSrc", () => {
    const src = "img.jpg";[[LINK_TYPE_LINK, { thumbnail: src }, src, "link posts w/ thumbnail"], [LINK_TYPE_ARTICLE, { cover_image: src }, src, "articles w/ cover image"], [LINK_TYPE_LINK, { thumbnail: "" }, null, "link posts w/ no thumbnail"], [LINK_TYPE_ARTICLE, { cover_image: "" }, null, "articles w/ no cover image"], [LINK_TYPE_TEXT, {}, null, "text posts"]].forEach(([postType, updatedProperties, expReturnValue, desc]) => {
      it(`${shouldIf(!!expReturnValue)} return an image source for ${desc}`, () => {
        const post = {
          ...makePost(postType === LINK_TYPE_LINK),
          ...updatedProperties,
          post_type: postType
