...
import { Sandbox } from "../flow/sinonTypes";
...
export default class IntegrationTestHelper {
  ...
  constructor() {
    ...
    this.dispatchThen = this.store.createDispatchThen() // stub all the api functions
    ...
    ;[api, channelAPI, ckeditorAPI, moderationAPI, embedlyAPI, frontpageAPI, postAPI, commentAPI, livestreamAPI, widgetAPI].forEach(apiModule => {
      for (const methodName in apiModule) {
        if (typeof apiModule[methodName] === "function") {
          const stubName = `${methodName}Stub`;
          this[stubName] = this.sandbox.stub(apiModule, methodName).throws(() => new Error(`${stubName} not implemented`));
        }
      }
    });
    ...
    this.debounceStub = this.sandbox.stub(_, "debounce").callsFake((func: Function) => () => func());
    ...
    this.getViewportWidthStub = this.sandbox.stub(utilFuncs, "getViewportWidth").returns(1000);
    this.embedlyPlatformStub = this.sandbox.stub(embedUtil, "loadEmbedlyPlatform");
    ...
    this.sandbox.stub(networkInterfaceFuncs, "makeRequest").callsFake((url, method, options) => ({
      execute: callback => {
    ...
    this.handleRequestStub.withArgs(userListApiURL.toString()).returns(queryListResponse([]));
   ...
   */
  renderComponent(url: string = "/", typesToAssert: Array<string> | null = null): Promise<any> {
    let expectedTypes = [];
  ...
  stubComponent(module, displayName, name = "default") {
    ...
    this[`${displayName}Stub`] = this.sandbox.stub(module, name).callsFake(props => <Stubber {...props} />);
  }
  ...
  configureReduxQueryRenderer(Component, defaultProps = {}, defaultState) {
    ...
    return async (extraProps = { history }, beforeRenderActions = [], perRenderDefaultState) => {
  ...
  configureHOCRenderer(WrappedComponent: Class<React.Component<any, any>>, InnerComponent: Class<React.Component<any, any>>, defaultState: Object, defaultProps = {}) {
    const history = this.browserHistory;
    return async (extraState = {}, extraProps = {
      ...
      const wrapper = await mount(<Router history={this.browserHistory} store={this.store}>
          <WrappedComponent store={store} dispatch={store.dispatch} {...defaultProps} {...extraProps} />
        </Router>);

      this.wrapper = wrapper;

      const inner = wrapper.find(InnerComponent).exists() ? wrapper.find(InnerComponent) : wrapper.findWhere(component => component.type() && component.type().WrappedComponent === InnerComponent).last();
