...
import { anyProcessing, allLoaded, anyError, anyErrorExcept404, any404Error, anyNotAuthorizedErrorType, isNotAuthenticatedErrorType, NOT_AUTHORIZED_ERROR_TYPE, NOT_AUTHENTICATED_ERROR_TYPE, AUTHENTICATION_FAILED_ERROR_TYPE } from "./rest";
...
describe("rest utils", () => {
  describe("anyProcessing", () => {
    ...
    it("should return true if one object has processing true", () => {
      assert.isTrue(anyProcessing([{}, { bip: "bop" }, { processing: false }, { processing: true }]));
    });

    it("should return true if all objects have processing true", () => {
      assert.isTrue(anyProcessing([{ bip: "bop", processing: true }, { processing: true }, { processing: true }]));
    });
  });

  describe("allLoaded", () => {
    it("should return false if one does not have loaded true", () => {
      assert.isFalse(allLoaded([{}, { bip: "bop" }, { loaded: true }, { loaded: false }]));
    });
    ...
    it("should return true if all have loaded true", () => {
      assert.isTrue(allLoaded([{ loaded: true }, { loaded: true, other: "prop" }]));
    });
  });
  ...
  describe("any404Error", () => {
    ...
    it("should return false if all codes are not 404", () => {
      assert.isFalse(any404Error([makeError(303), makeError(400), makeError(500), { error: {} }]));
    });

    it("should return true if some or all codes are 404", () => {
      assert.isTrue(any404Error([makeError(404), makeError(500), makeError(4932)]));

      assert.isTrue(any404Error([makeError(404), makeError(404), makeError(404)]));
    });
  });
  ...
  describe("anyNotAuthorizedErrorType", () => {
    ...
    it("should return false if all codes are not NotAuthorized", () => {
      assert.isFalse(anyNotAuthorizedErrorType([makeErrorType("Some"), makeErrorType("Other"), makeErrorType("Error"), { data: {} }]));
    });

    it("should return true if some or all codes are NotAuthorized", () => {
      assert.isTrue(anyNotAuthorizedErrorType([makeErrorType(NOT_AUTHORIZED_ERROR_TYPE), makeErrorType("Not"), makeErrorType("It")]));

      assert.isTrue(anyNotAuthorizedErrorType([makeErrorType("Not"), makeErrorType("It"), makeErrorType(NOT_AUTHORIZED_ERROR_TYPE)]));
    });[NOT_AUTHENTICATED_ERROR_TYPE, AUTHENTICATION_FAILED_ERROR_TYPE].forEach(error => {
