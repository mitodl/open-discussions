...
export const allLoaded = R.both(R.complement(R.isEmpty), R.all(R.propEq("loaded", true)));

const hasError = R.propSatisfies(R.complement(R.isNil), "error");

export const anyError = R.any(hasError);

export const anyErrorExcept = (codes: Array<number>) => R.compose(R.any(R.either(R.propSatisfies(R.isNil, "errorStatusCode"), R.propSatisfies(R.complement(R.contains(R.__, codes)), "errorStatusCode"))), R.map(R.prop("error")), R.filter(hasError));

const anySpecificError = code => R.compose(R.any(R.propSatisfies(R.equals(code), "errorStatusCode")), R.map(R.prop("error")), R.filter(hasError));
...
export const AUTHENTICATION_ERRORS = [NOT_AUTHENTICATED_ERROR_TYPE, AUTHENTICATION_FAILED_ERROR_TYPE];

const anyErrorTypes = errorTypes => R.propSatisfies(R.contains(R.__, errorTypes), "error_type");

const anySpecificErrorTypes = errorTypes => R.compose(R.any(anyErrorTypes(errorTypes)), R.map(R.prop("error")), R.filter(hasError));

export const isNotAuthenticatedErrorType = anyErrorTypes(AUTHENTICATION_ERRORS);
export const anyNotAuthorizedErrorType = anySpecificErrorTypes([NOT_AUTHORIZED_ERROR_TYPE]);