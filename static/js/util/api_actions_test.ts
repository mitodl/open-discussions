...
import { approvePost, removePost, approveComment, removeComment, toggleFollowComment, toggleFollowPost, leaveChannel } from "../util/api_actions";
...
describe("api_actions util", () => {
  ...
  describe("toggleFollowPost", () => {
    beforeEach(() => {
      ...
      it(`should set subscribed from ${subscribed} to ${!subscribed} and show a snackbar message`, async () => {
        ...
        await helper.listenForActions([requestType, successType, SET_SNACKBAR_MESSAGE], () => {
  ...
  describe("approvePost", () => {
    ...
    it("should patch the post and set the updated value", async () => {
      ...
      const {
        requestType,
        successType
      } = actions.postRemoved.patch;
      const state = await helper.listenForActions([requestType, successType, SET_POST_DATA], () => {
  ...
  describe("approveComment", () => {
    ...
    it("should patch the comment and set the updated value", async () => {
      ...
      const state = await helper.listenForActions([actions.comments.get.requestType, actions.comments.get.successType, actions.comments.patch.requestType, actions.comments.patch.successType], async () => {
      ...
      const resultComment = R.view( // $FlowFixMe: flow can't reason about comment.id properly
      findComment(stateTree, comment.id), stateTree);

      assert.isOk( // $FlowFixMe: flow can't reason about comment.id properly
  ...
  describe("leaveChannel", () => {
    ...
    it("should make requests to remove the user as a subscriber and contributor", async () => {
      await helper.listenForActions([actions.channelSubscribers.delete.requestType, actions.channelSubscribers.delete.successType, actions.channelContributors.delete.requestType, actions.channelContributors.delete.successType], async () => {
        await leaveChannel(helper.store.dispatch, channelName, username);
      });
      sinon.assert.calledWithExactly(helper.deleteChannelSubscriberStub, channelName, username);
      sinon.assert.calledWithExactly(helper.deleteChannelContributorStub, channelName, username);
    });
  });
});