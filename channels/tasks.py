...
def subscribe_all_users_to_default_channel(*, channel_name):
    ...
    query = User.objects.exclude(username=settings.INDEXING_API_USERNAME).values_list('username', flat=True).iterator()

    for usernames in chunks(query, chunk_size=chunk_size):
        subscribe_user_range_to_default_channel.delay(channel_name=channel_name, usernames=usernames)
