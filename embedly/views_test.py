...
def test_get_embedly(client, user, mocker, settings):
    ...
    embed_return_value.configure_mock(**{'json.return_value': {'some': 'json', 'thumbnail_url': external_thumb}})
...
def test_get_embedly_no_key(client, user, settings):
    ...
    client.force_login(user)
    embedly_url = reverse(
        'embedly-detail', kwargs={"url": "https%253A%252F%252Fen.wikipedia.org%252Fwiki%252FGiant_panda/"})
    resp = client.get(embedly_url)
    assert resp.json() == {}
