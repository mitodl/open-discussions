...
from search.task_helpers import delete_user_list, upsert_user_list, upsert_staff_list, delete_staff_list
    ...
    def update(self, instance, validated_data):
        ...
        if request and hasattr(request, "user") and isinstance(request.user, User):
            ...
            with transaction.atomic():
                ...
                if topics_data is not None:
                    stafflist.topics.set(CourseTopic.objects.filter(id__in=topics_data))
                if instance.items.exists() and instance.privacy_level == PrivacyLevel.public.value:
